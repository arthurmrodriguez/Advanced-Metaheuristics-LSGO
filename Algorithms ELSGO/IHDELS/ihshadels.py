...
class PoolLast:
    ...
    def update_prob(self):
        ...
        if np.all([value == 0 for value in self.improvements.values()]):
            return np.random.choice(self.improvements.keys())
...
#                import ipdb; ipdb.set_trace()
                ...
                # Increase a 10% of values
                new_solution = np.random.uniform(-0.05, 0.05, dim)*(upper-lower)+best_global_solution

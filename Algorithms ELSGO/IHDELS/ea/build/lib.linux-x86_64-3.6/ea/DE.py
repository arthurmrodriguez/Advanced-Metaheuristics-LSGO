...
def random_population(lower, upper, dimension, size):
    return  np.random.uniform(lower, upper, dimension*size).reshape((dimension,size))
    ...
    if name_output is None:
        ...
        if not replace and os.path.isfile(name_output):
            ...
            if lines:
                ...
                return EAresult(fitness=bestSolutionFitness, solution=bestSol, evaluations=evaluations)
...
def DE(fun, run_info, dimension, max_evals, name_output=None, run=25, replace=True, debug=True, F=0.6, CR=0.9, popsize=60, crossoverFunction=DEcrossover.classicalBinFunction, population=None,initial_solution=None):
    """
    ...
    for numrun in range(1, run+1):
        ...
        probabilityRecombination = get_parameter('CR', CR, normal_values['CR'], PS)
        ...
        for i in range(sizePopulation):
            ...
            if bestSolutionFitness > populationFitness[i]:
                ...
                msg = "Best solution Find: %e at %d" %(bestSolutionFitness, currentEval)
                dprint(msg, debug)

        while not shouldEnd(currentEval, maxEval, bestSolutionFitness, fun_best, threshold):
            # Mutate the current population
            trialVector = np.zeros((sizePopulation, dimension))
            trialVectorFitness = np.zeros(sizePopulation)

            for i in xrange(sizePopulation):
                noisyVector = crossover.apply(population, i, indexBest, get_value(valueMutation))
                ...
                trialVector[i] = noisyVector*changed+population[i]*np.invert(changed)
                ...
                if populationFitness[i] < bestSolutionFitness:
                    ...
                    dprint("Best solution Find: %e at %d" %(bestSolutionFitness, position), debug)

            if fid is not None and currentEval >= check_eval:
                fid.write("[%.0e]: %e,%d\n" %(check_eval, abs(bestSolutionFitness-threshold), numEvalFound))
                fid.flush()
        ...
        msg = "The best solution is: %e in %d evaluation %s" %(bestSolutionFitness, numEvalFound, crossover.stats())

        dprint(msg, debug)

        if fid is not None:
            fid.write("%s\n" %msg)
            fid.write("%e,%s,%d,%d\n" %(abs(bestSolutionFitness-threshold), ' '.join(map(str, bestSolution)), numEvalFound, currentEval))
            fid.flush()

    if fid is not None:
        fid.close()

    return EAresult(fitness=bestSolutionFitness, solution=bestSolution, evaluations=currentEval)
...
def initializeVariables(F_n):
    ...
    bounds = [(-100, 100), (-10, 10), (-100, 100), (-100, 100), (-30, 30), 
            (-100, 100), (-1.28, 1.28), (-500, 500), (-5.12, 5.12), (-32, 32), 
            (-600, 600), (-50, 50), (-50, 50)]
...
def shouldEnd(currentEval, maxEval, bestSolutionFitness, objectiveValue, threshold):
    """

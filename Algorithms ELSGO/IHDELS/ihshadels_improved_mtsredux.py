...
from mtsred import mtsls, mtsreset
"""
...
class PoolLast:
    ...
    def improvement(self, obj, account, freq_update, minimum=0.15):
        ...
        if not self.new:
            ...
            rests = list(set(self.improvements.keys()) - set(self.new))
            self.new = np.random.choice(rests)
        elif account < previous:
            self.new = self.update_prob()
...
def apply_localsearch(name, method, fitness_fun, bounds, current_best,
    ...
    if fit <= current_best_fitness:
        ...
        return EAresult(
            solution=np.array(sol), fitness=fit, evaluations=funcalls)
    else:
        return EAresult(
...
def applySHADE(crossover,
    ...
    result, bestId = SHADE.improve(
        ...
        MemF=applySHADE.MemF,
        MemCR=applySHADE.MemCR)
    fid.write(
        get_improvement("SHADE partial", current_best.fitness, result.fitness))
...
def ihshadels(fitness_fun,
    ...
    while totalevals < maxevals:
        ...
        if not pool_global.is_empty():
            ...
            current_best = apply_localsearch(
                "Global", method_global, fitness_fun, bounds,
                current_best_solution, current_best.fitness, evals_gs, fid)
        ...
        for i in range(1):
            ...
            if apply_de:
                result, bestInd = applyDE(
                    crossover, fitness_fun, funinfo, dim, evals_de, population,
                    populationFitness, bestId, current_best, fid, info_de)
            ...
            if apply_ls:
                result = apply_localsearch(
                    "Local", method, fitness_fun, bounds_partial,
            ...
            fid.write(
                "TotalImprovement[{:d}%] {:.3e} => {:.3e} ({})\tRestart: {}\n".
                format(
                    int(100 * ratio_improvement), previous_fitness,
                    result.fitness, num_worse, num_restarts))
            ...
            if num_worse >= 3:
                ...
                #                import ipdb; ipdb.set_trace()
                fid.write(
                    "Restart:{0:.2e} for {1:.2f}: with {2:d} evaluations\n".
                    format(current_best.fitness, ratio_improvement,
                           totalevals))
                # Increase a 1% of values
                posi = np.random.choice(popsize)
                new_solution = np.random.uniform(-0.01, 0.01, dim) * (
                    upper - lower) + population[posi]
    ...
    fid.write(
        "%e,%rests = list(set(self.improvements.keys())-set(self.new)),%d\n" %
        (abs(best_global_fitness), ' '.join(map(str, best_global_solution)),
         totalevals))
    fid.flush()
    return result
...
"""
    ...
    parser.add_argument(
        "-e", required=False, type=int, dest="maxevals", help='maxevals')
    parser.add_argument(
        "-t", default=0.01, type=float, dest="threshold", help='threshold')
    parser.add_argument(
        "-p", default=100, type=int, dest="popsize", help='population size')
    parser.add_argument(
        "-H",
        default=None,
        type=int,
    ...
    fname = name + "_pop{args.popsize}_H{args.shade_h}_t{args.threshold:.2f}_F{args.function}_{args.seed}r{args.run}.txt".format(
        args=args)
    ...
    # Parameter commons
    bench.set_algname("ihshadels_mtsredux")

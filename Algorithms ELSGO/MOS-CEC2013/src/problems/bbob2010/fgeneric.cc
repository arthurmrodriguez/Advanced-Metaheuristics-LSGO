...
double fgeneric_initialize(ParamStruct PARAMS)
{
    ...
    idxEvalsTrigger = 0;
    evalsTrigger = floor(pow(10.,(double)idxEvalsTrigger/(double)nbPtsEvals)); /* = 1 ! */
...
double fgeneric_evaluate(double * X)
{
    ...
    if ( (LastEval.num+1 >= evalsTrigger) || (Ftrue-Fopt < fTrigger) ) 
    {
        ...
        if (evalsj >= evalsTrigger)
        {
            ...
            evalsTrigger = fmin(floor(pow(10., (double)idxEvalsTrigger/(double)nbPtsEvals)), DIM * pow(10., idxDIMEvalsTrigger));
        }
        ...
        if (Ftrue - Fopt < fTrigger)
        {
            ...
            else
            {
                ...
                while ( (Ftrue-Fopt) <= pow(10., (double)idxFTrigger/(double)nbPtsF) )

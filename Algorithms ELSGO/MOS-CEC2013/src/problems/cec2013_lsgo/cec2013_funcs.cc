...
// functions.
// ---------------------------------------------------------------------------------
arma::mat T_asy(const arma::mat& f, double beta) {
  arma::mat g=f;
...
// ---------------------------------------------------------------------------------
arma::mat T_diag(const arma::mat& f, double alpha) {
  double alphasq=sqrt(alpha);
...
// ---------------------------------------------------------------------------------
arma::uvec checkBounds(const arma::mat& x, double lb, double ub) {
...
// Sphere Function
// ---------------------------------------------------------------------------------
double sphere(const arma::mat& x) {
...
// Elliptic Function
// ---------------------------------------------------------------------------------
double elliptic(const arma::mat& x) {
  double condition=1e+06;
  ...
  double fit=dot(coefficients, square(T_irreg(x)));
  return fit;
}
...
// ---------------------------------------------------------------------------------
double rastrigin(const arma::mat& x) {
  double A=10;
...
// Ackley's Function
// ---------------------------------------------------------------------------------
double ackley(const arma::mat& x) {
  arma::mat tmpsq =sum(square(x), 0);
  arma::mat tmpcos=sum(cos(2*arma::datum::pi*x), 0);
  double fit=20-20*exp(-0.2*sqrt(tmpsq(0)/x.n_rows))-exp(tmpcos(0)/x.n_rows)+exp(1);
  return fit;
}
...
// ---------------------------------------------------------------------------------
double schwefel(const arma::mat& x) {
...
// ---------------------------------------------------------------------------------
double rosenbrock(const arma::mat& x) {
...
double f1(const arma::mat& x) {
  ...
  double fit=elliptic(newX);

  if (idx.n_rows*idx.n_cols > 0) {
    fit=std::numeric_limits<double>::max();

...
public:
  ...
  int              route     ;
    ...
    void checkProbsFromGen(vector <vector<int> >& all_values) {
      ...
      vector<double> probs_def = DARPVNSShaker::getSelectionProbs(routecosts_max);
      vector<double> probs_max = DARPVNSShaker::getSelectionProbs(routecosts_max,GAGenome::MAXIMIZATION);
      vector<double> probs_min = DARPVNSShaker::getSelectionProbs(routecosts_min,GAGenome::MINIMIZATION);
    ...
    void checkBestRouteForInsertion(vector< vector<int> >& gen_values, list<int>& ins_seq, int orig_route, double score_orig, int expected_value) {
...
TEST_F(GreedyMoveTest,specificValuesProbsTest) {
  ...
  vector<double> probs = DARPVNSShaker::getSelectionProbs(values);
  for (int i=0; i<size; i++) {
    double prev_value = (i==0) ? 0.0 : probs[i-1];
    ASSERT_GT(0.001,fabs( prev_value+(double)values[i].score/(double)sum - probs[i]) );
  }

}

TEST(SelectPos,TestingProbabilities) {
  double probs[] = {0.10,0.20,0.30,0.48,0.78,0.93,1.0}; vector<double> probsv (probs, probs + sizeof(probs)/ sizeof(double) );
  ...
  for (int i=0; i<freqs.size(); i++) {
    double estim_prob = (i==0) ? probs[0] : probs[i]-probs[i-1];
    ASSERT_GT(0.01, fabs ( (double) estim_prob - (double)freqs[i]/ATTEMPTS) );
  }

...
double DARPEvaluator::computePickupDelay(map<int,long> beginningServiceTimes) {
  double res = 0;
  map<int,long>::const_iterator it;
  int vert_id;
...
double DARPEvaluator::computeDeliveryDelay(map<int,long> beginningServiceTimes, map<int, long> arrivalTimes) {
  double res = 0;
  map<int,long>::const_iterator it;
...
void DARPEvaluator::printVertexInfo2(int vertex, int nextvert, map<int,long>& A, map<int,long>& W, map<int,long>& B, map<int,long>& D, map<int,long>& F, map<int,long>& L, map<int,long>& y) {
  ...
  cout << " | L = "  << Ltime;
  cout << " | d = "  << dtime;
  if (Vertex::isRideTimeRelative() ) cout << " | L/d = " << (double) L[vertex] / dist;
  cout << " | next = "  << dtimenext;
  cout << " | W = "  << Wtime;
...
double DARPEvaluator::nonPenalizedScore(const DARPGenome& gen) const {
  double result = 0.0;
...
double DARPEvaluator::nonPenalizedScore(const DARPGenome& gen, int route) const {
  return nonPenalizedScore( gen.costPerRoute(route), gen.pickupDelayPerRoute(route), gen.deliveryDelayPerRoute(route) );
}

double DARPEvaluator::nonPenalizedScore(long cost, double pickupdelay, double deliverydelay) const {
  double value=-1;
...
double DARPEvaluator::score(DARPGenome& gen) {
...
double DARPEvaluator::scalePickupValue(long begintime, Vertex& vertex) {
  return pow( (double) (begintime - vertex.fbegin_) / (double) (vertex.fend_ - vertex.fbegin_) , timeSumExp_ );
}

double DARPEvaluator::scaleDeliveryDelay(Vertex& pick_vert, Vertex& del_vert, long begin_time, long del_time) {
  ...
  double scaled_value = ( (service_time < direct_dist && service_time >= direct_dist - 100) ||
                          direct_dist == 0 ) ?
                            0 :
                            (double) (service_time                                                - direct_dist) /
                            (double) (Vertex::maxRideTime(pick_vert.id_,del_vert.id_,costMatrix_) - direct_dist);

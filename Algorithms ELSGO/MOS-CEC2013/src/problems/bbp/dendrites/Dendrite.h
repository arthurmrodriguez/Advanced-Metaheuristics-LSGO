...
public:
	...
	boost::unordered_map<string, double> Metrics( void );


	// Comunicación con el sistema de salida
	void printBranch( std::string mesg );
	// Comunicación con los modelos
	double predict(boost::unordered_map<string, double>& data, string param);
	// Cálculo de métricas
	void maxEucDist(double dist);
	int incrementNumberOfBranches();
	void incrementLength(double l);

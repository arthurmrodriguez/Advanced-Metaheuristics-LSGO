...
int main_test (int argc, char** argv) {
  ...
  for (unsigned i = 0; i < iters; i++) {
    ...
    for (int p = 1; p <= nIters; p++) {
      ...
      double pvalueBranches = KolmogorovTest(data[i].second.numBranches,    newNumBranches, "");
      double pvalueEuc      = KolmogorovTest(data[i].second.maxEucDistance, newMaxEucDist,  "");
      double pvalueLength   = KolmogorovTest(data[i].second.totalLength,    newTotalLength, "");

...
static int           shared__;
static int           rest__;
static GA1DArrayAlleleGenome<double>* ind_part1__;
static GA1DArrayAlleleGenome<double>* ind_part2__;
static GA1DArrayAlleleGenome<double>* ind_partrest__;

void setIndPart1AndPart2Values(GAGenome& ind);

void setInitValues(GAGenome& ind, double param_m) {
  GA1DArrayAlleleGenome<double>& ind_r = dynamic_cast<GA1DArrayAlleleGenome<double>&>(ind);
  m__ = param_m;

  ind_part1__ = dynamic_cast<GA1DArrayAlleleGenome<double>*>(ind_r.clone());
  ind_part2__ = dynamic_cast<GA1DArrayAlleleGenome<double>*>(ind_r.clone());
...
double composeFuncs(GAGenome& ind, double (*f1)(GAGenome&), double bias1, double (*f2)(GAGenome&), double bias2 ) {
  ...
  double res_f1 = f1(*ind_part1__) - bias1;
  if (res_f1 < 0) {
      std::cout << "res_f1: " << res_f1 << std::endl;
      exit(0);
   }
   //assert(res_f1 >= 0.0);
  double res_f2 = f2(*ind_part2__) - bias2; assert(res_f2 >= 0.0);
...
void setIndPart1AndPart2Values(GAGenome& ind) {
  GA1DArrayAlleleGenome<double>& orig_ind = dynamic_cast<GA1DArrayAlleleGenome<double>&>(ind);

...
ConstantParticipation::ConstantParticipation(double minPart, unsigned restoreGen, void* pfData)
...
DynamicParticipation::DynamicParticipation(double adjust, double minPart, unsigned restoreGen, void* pfData)
...
double DynamicParticipation::setBaseQual (MOSEA2& alg) {
...
void DynamicParticipation::update (MOSEA2& alg) {
  ...
  double bestQuality = techSet.getTechnique(bestTechIds[0])->getQuality();
  ...
  for (MOSTechniqueSet::MOSTechniqueSetIterator it =techSet.begin(); it != techSet.end(); it++) {
    if (std::find(bestTechIds.begin(), bestTechIds.end(), it->first) == bestTechIds.end()) {
      double diff = (bestQuality - it->second->getQuality()) / (bestQuality - _base);

      double currentTechPart = techSet.getPartRatio(it->first);
      double ratioInc = currentTechPart * _adjust * diff;

      if ((currentTechPart - ratioInc) > _minPart) {
        double sharedRatio = ratioInc / (double) bestTechIds.size();
        ...
        double sharedRatio = (techSet.getPartRatio(it->first) - _minPart) / (double) bestTechIds.size();
        for (unsigned i = 0; i < bestTechIds.size(); i++)
          techSet.setPartRatio(bestTechIds[i], techSet.getPartRatio(bestTechIds[i]) + sharedRatio);

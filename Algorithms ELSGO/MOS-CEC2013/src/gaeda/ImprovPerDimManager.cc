...
static void swap (vector<int>& dims, vector<double>& values, int p1, int p2) {

   double tmp_val = values [p1];
   int    tmp_dim = dims   [p1];
...
static void quicksort (vector<int>& keys, vector<double>& values, int l, int r) {
...
void ImprovPerDimManager::getDimsWhichProbSumAThreshold(vector<int>& selected_dims, double prob_percent, int num_other_dims) const {
  assert(num_other_dims >= 0 && num_other_dims < numDims() );

  selected_dims.clear();
  if (num_other_dims == 0) num_other_dims = 1;

  vector<double>* probs = getProbs();
...
vector<double>* ImprovPerDimManager::getProbs() const {
  vector<double> dim_values = dimPerfValues();
  double sum = 0.0; for (int i=0; i<dim_values.size(); i++) sum += dim_values[i];
  assert( sum >= 0.0 );

  vector<double>* probs = new vector<double>(dim_values.size(),0.0);
...
vector<double>* ImprovPerDimManager::getAccumProbs() const {
  vector<double> dim_values = dimPerfValues();
  ...
  if (sum > 0.0) { // If all performance values are = 0.0 nothing is done
    //int    num_zeros = 0;
    //double min_non_zero_prob_value = 1.1; // initialized with a bigger value than the ones reached
...
vector<double> ImprovPerDimManager::dimPerfValues() const {
...
vector<int>* getDimsWithVSmallProb(vector<double>& probs) {
...
void ImprovPerDimManager::getDimsWithRep (vector<int>& dims) const {
  ...
  vector<double>* probs = getAccumProbs();
...
int ImprovPerDimManager::posBasedOnProbs(vector<double>& probs) const {
  int selected_pos = -1;
  ...
  if (probs.back() == 0.0) {
    ...
    double rnd_value  = GARandomDouble(0.0,1.0);
...
//void ImprovPerDimManager::updateValues (vector<double>& new_values) {
//  // /*LOG*/ vector<double>  old_values = dimPerfValues();
...
//  // /*LOG*/ vector<double>  new_comp_values = dimPerfValues();
//  // /*LOG*/ for (int i=0; i<numDims(); i++) msg << new_comp_values[i] << ", "; msg << endl;
//
//  // /*LOG*/ GALogger::instance()->appendLogMessage("ImprovPerDimManager", msg.str());
//}

void ImprovPerDimManager::updateValue(int pos, double new_value) {
  // Needed to be implemented in order to be called from the posBasedOnProbs method
  throw runtime_error("Method dimPerfValues from ImprovPerDimManager should not be called");
}

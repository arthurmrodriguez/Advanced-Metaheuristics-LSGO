...
void EvolutionStrategy::initialize () {
  ...
  for (unsigned i = 0; i < pop->size(); i++) {
    ...
    GA1DArrayAlleleGenome<double>& g = DYN_CAST(GA1DArrayAlleleGenome<double>&, pop->individual(i));
...
void EvolutionStrategy::offspring (GAPopulation* offpop){
  ...
  for (unsigned i = 0; i < _lambda; i++) {
    ...
    GA1DArrayAlleleGenome<double>& g = DYN_CAST(GA1DArrayAlleleGenome<double>&, offpop->individual(i));

    // Selection of parents for recombination
    static std::vector<GA1DArrayAlleleGenome<double>*> parents (_ro, (GA1DArrayAlleleGenome<double>*)0);

    for (unsigned j = 0; j < _ro; j++) {
      int indiv = GARandomInt(0, pop->size()-1);
      parents[j] = DYN_CAST(GA1DArrayAlleleGenome<double>*, &(pop->individual(indiv)));
    }
    ...
    double F = exp(_tau0*normal_random(0,1));
...
int EvolutionStrategy::IntermediateRecombination(const std::vector<GA1DArrayAlleleGenome<double>*>& parents,
						 GA1DArrayAlleleGenome<double>& child) const {

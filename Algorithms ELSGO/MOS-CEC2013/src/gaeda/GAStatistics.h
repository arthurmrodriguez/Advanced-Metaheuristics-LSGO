...
public:
  ...
  double online() const {return on;}
  double offlineMax() const {return offmax;}
  double offlineMin() const {return offmin;}
  double initial(int w=Maximum) const;
  double current(int w=Maximum) const;
  double maxEver() const {return maxever;}
  double minEver() const {return minever;}
  ...
  double getScaledFitness(double max_score, double min_score, double genome_score);
...
protected:
  ...
  double maxever;		// maximum score since initialization
  double minever;		// minimum score since initialization
  double on;			// "on-line" performance (ave of all scores)
  double offmax;			// "off-line" performance (ave of maximum)
  double offmin;			// "off-line" performance (ave of minimum)

  double aveInit;		// stats from the initial population
  double maxInit;
  double minInit;
  double devInit;
  double divInit;

  double aveCur;			// stats from the current population
  double maxCur;
  double minCur;
  double devCur;
  double divCur;
  ...
  double * cscore;		// best score of last n generations

  unsigned int nscrs, Nscrs;	// how many scores do we have?
  int * gen;			// generation number corresponding to scores
  double * aveScore;		// average scores of each generation
  double * maxScore;		// best scores of each generation
  double * minScore;		// worst scores of each generation
  double * devScore;		// stddev of each generation
  double * divScore;		// diversity of each generation
...
inline double GAStatistics::convergence() const {
  double cnv = 0.0;
...
inline double GAStatistics::initial(int w) const {
  double val = 0.0;
  switch(w){

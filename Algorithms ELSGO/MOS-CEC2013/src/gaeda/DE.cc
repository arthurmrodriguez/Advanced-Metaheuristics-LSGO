...
DE::DE(const GAGenome& c, double F, double CR) : GAGeneticAlgorithm(c){
  F_ = F;
  CR_ = CR;
  oldPop = pop->clone();
  deCross_ = c.de ();
...
DE::DE(const GAPopulation& p, double F, double CR) : GAGeneticAlgorithm(p){
  F_ = F;
  CR_ = CR;
  oldPop = pop->clone();
...
void DE::offspring(GAPopulation* pop){
  int           dim    = dynamic_cast<GA1DArrayAlleleGenome<double>&>(pop->individual(0)).length();
  GA1DArrayAlleleGenome<double>* t1     = NULL;
  GA1DArrayAlleleGenome<double>* t2     = NULL;
  GA1DArrayAlleleGenome<double>* t3     = NULL;
  GA1DArrayAlleleGenome<double>* x_i    = NULL;
  GA1DArrayAlleleGenome<double>* x_new  = NULL;
  ...
  for(unsigned ind_pos=0; ind_pos<pop->size(); ind_pos++){
    x_i   = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->individual( ind_pos ) );
    x_new = dynamic_cast <GA1DArrayAlleleGenome<double>*> (&    pop->individual( ind_pos ) );
    ...
    unsigned attempts=0;
    do {
      t1 = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->select() );
      attempts += 1;
    ...
    do {
       t1 = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->individual(GARandomInt(0,pop->size()-1)) );
    }while (t1 == x_i);

    do{
      t2 = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->select() );
      attempts+=1;
      if (attempts >= pop->size() ){
           attempts=0;
           break;
    ...
    do{
       t2 = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->individual(GARandomInt(0,pop->size()-1)) );
    }while (t2 == x_i or t2 == t1);

    do{
      t3 = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->select() );
      attempts+=1;
      if (attempts >= pop->size() ){
           attempts=0;
           break;
    ...
    do{
        t3 = dynamic_cast <GA1DArrayAlleleGenome<double>*> (& oldPop->individual(GARandomInt(0,pop->size()-1)) );
    }while (t3 == x_i or t3 == t2 or t3 == t1);

    for(int gene_pos=0; gene_pos<dim; gene_pos++){
      double new_val = t1->gene(gene_pos) + F_ * (t2->gene(gene_pos) - t3->gene(gene_pos));

...
protected:
  ...
  double cost(const RoutingGenome& g) { return g.nonPenalizedScore(); }
...
public:
  ...
  double getBestSolScore()   const { return (& (bestSol())   ) ? bestSol().score()   : 0.0;}
  double getActualSolScore() const { return (& (actualSol()) ) ? actualSol().score() : 0.0;}

...
GAProblemStruct* GAloadProblem (char* libpath) {
   ...
   // These are optional
   res->individualInit = (modGenomeFuncPtr)      dlsym (handle, "individualInit");
   res->populationInit = (populationInitFuncPtr) dlsym (handle, "populationInit");

   res->postprocess = (postprocessFuncPtr) dlsym (handle, "postprocess");

   res->VNSLocalSearch = ( (getLSFuncPtr)         dlsym (handle, "VNSLocalSearch")   );
   res->VNSShakers     = ( (getVNSShakersFuncPtr) dlsym (handle, "getVNSShakersList"));

   // Chapuzillas rapidas que habra que cambiar en un futuro y no repetir codigo, por ej
   // con una macro

   void* sym  =  dlsym (handle, "configureAlg");
   if (!sym) res->configureAlg = NULL;
   else      res->configureAlg = (configureAlgPtr) sym;

   sym  =  dlsym (handle, "getOptimumGenome");
   if (!sym) res->optimum = NULL;
   else      res->optimum =  (optFuncPtr) sym;

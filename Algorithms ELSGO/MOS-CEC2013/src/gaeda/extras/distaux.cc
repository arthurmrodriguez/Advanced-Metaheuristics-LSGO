...
vector<double> computeDistanceMatrix(vector<GAGenome*>& inds) {
  ...
  vector<double> dist_matrix( num_inds * num_inds, 0.0 );
...
MinDist minDistOfPos(int pos, vector<double>& dist_matrix, vector<bool>* deleted_inds=NULL){
  ...
  mindist.value = std::numeric_limits<double>::max();
...
vector<MinDist> computeMinDists(vector<GAGenome*>& inds, vector<double>& dist_matrix) {
  int             num_inds = inds.size();
...
void removeInd(int pos, vector<MinDist>& mindists, vector<double>& dist_matrix, vector<bool>& deleted_inds) {
  assert(mindists.size() == deleted_inds.size());
...
void maxminD2Method (vector<GAGenome*>& all_inds, vector<GAGenome*>& slctd_inds) {
  ...
  if (slctd_inds.size() != all_inds.size() ) {
    ...
    vector<double>  dist_matrix = computeDistanceMatrix(all_inds);
...
//
//vector<double> computeSumDists(vector<GAGenome*>& inds, vector<double>& dist_matrix) {
//  int num_inds = inds.size();
//
//  vector<double> dist_sums (num_inds,0.0);
//  for (int i=0; i<num_inds; i++) {
//    double sum_i = 0.0;
...
//                  vector<double>&    dist_sums,
//                  vector<bool>&      deleted_inds) {
//
//  int    min_setdist_pos = -1;
//  double min_setdist     = std::numeric_limits<int>::max();
//  double tmp_setdist;
//

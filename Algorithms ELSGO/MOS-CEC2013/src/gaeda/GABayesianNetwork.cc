...
void GABayesianNetwork::initialize () {
   ...
   mProbs = new double * [getIndividualSize ()];

   for (i = 0; i < getIndividualSize (); i++) {

      mProbs [i] = new double [getState (i) - 1];

      // The final probability is found subtracting
      // all the others to 1.

      for (j = 0; j < getState (i) - 1; j++)
         mProbs [i][j] = 1 / (double) getState (i);
   ...
   mActualMetric = new double [getIndividualSize ()];
...
void GABayesianNetwork::learn (const GAPopulation& p, double sel_pctg) {
...
double* GABayesianNetwork::probabilities (int ordered_node, GAGenome& g) {
...
void GABayesianNetwork::learnProbabilities (int**& cases) {
   ...
   for (i = 0; i < getIndividualSize (); i++) {
      ...
      if (mLearningMethod != PBIL) {
         ...
         delete [] mProbs [i];
         mProbs [i] = new double [no_j * (getState (i) - 1)];
      ...
      for (j=0;j<no_j;j++) {
         ...
         double BDeu = (double) 1 / getState (i) / no_j;
         ...
         if ((mLearningMethod != PBIL) )
            for (k = 0; k < getState (i) - 1; k++)
               mProbs [i][j * (getState (i) - 1) + k] = (double) (nijk [j][k] + BDeu) / (double) (nij + getState (i) * BDeu);

         if (mLearningMethod == PBIL)
            for (k = 0; k < getState (i) - 1; k++)
               mProbs [i][j * (getState (i) - 1) + k] = (1.0 - ALPHA) * (mProbs [i][j * (getState (i) - 1) + k]) +
                                                        ALPHA * ((double) (nijk [j][k]) / (double) (nij));
...
void GABayesianNetwork::learnTree (int**& cases) {
   ...
   double** mi = new double* [getIndividualSize ()];

   for (j = 0; j < getIndividualSize (); j++) {

      mi [j] = new double [getIndividualSize ()];
   ...
   // A boolean double matrix containing if there is an undirected
...
double GABayesianNetwork::computeMI (int bat, int bi, int**& cases) {
   ...
   for (int i = 0; i < getState (i); i++)      // for bat
      for (int j = 0; j < getState (j); j++) { // for bi
         ...
         pbat   = pbat   / (double) (mSelectedSize);
         pbi    = pbi    / (double) (mSelectedSize);
         pbatbi = pbatbi / (double) (mSelectedSize);

         if (pbatbi != (double) 0.0 && pbat != (double) 0.0 && pbi != (double) 0.0)
            mi += pbatbi * (log ((double) ((pbatbi) / (pbat * pbi))));
...
double GABayesianNetwork::entropy (int node, int**& cases) {
   ...
   for (x = 0; x < getState (node); x++)
      if (n_x [x] > 0)
         entropy += n_x [x] * log ((double) n_x [x]);

   entropy /= -mSelectedSize;
   entropy += log ((double) mSelectedSize);
...
double GABayesianNetwork::entropy (int node, int parent, int**& cases) {
...
double GABayesianNetwork::BICMetric (int node, int**& cases) {
   ...
   for (j = 0; j < no_j; j++) {
      ...
      for (k = 0; k < getState (node); k++) {
         ...
         if (nijk [j][k] != 0)
            bic += nijk [j][k] * log ((double) (nijk [j][k]));

      }

      // For rounding problems...
      if (nij != 0)
         bic -= nij * log ((double) nij);

   }

   bic -= log ((double) mSelectedSize) * no_j / 2;
...
double GABayesianNetwork::deltaBIC (int node, int nparent, int**& cases) {
   ...
   for (j = 0; j < no_j; j++) {
      ...
      for (k = 0; k < getState (node); k++) {
         ...
         // For rounding problems...
         if (nijk [j][k] != 0)
            deltabic += nijk [j][k] * log ((double) (nijk [j][k]));

      }

      // For rounding problems...
      if (nij != 0)
         deltabic -= nij * log ((double) nij);

   }

   deltabic -= log ((double) mSelectedSize) * no_j / 2;
...
double GABayesianNetwork::K2Metric (int node, int**& cases) {
   ...
   k2 -= log ((double) mSelectedSize) * no_j / 2;
...
double GABayesianNetwork::deltaK2 (int node, int nparent, int**& cases) {
...
double GABayesianNetwork::logFact (int n) {

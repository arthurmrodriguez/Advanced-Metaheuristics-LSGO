...
RLImprovPerDimManager::RLImprovPerDimManager(int num_dims, double alpha) : ImprovPerDimManager(num_dims),
                                                                           alpha_(alpha),
                                                                           values_(num_dims),
...
vector<double> RLImprovPerDimManager::dimPerfValues() const {
  if (all_dims_explored_) return values_;
  else {
    vector<double> mod_values = values_; 

    double max_value = values_[0];
...
void RLImprovPerDimManager::updateValue (int pos, double new_value) {

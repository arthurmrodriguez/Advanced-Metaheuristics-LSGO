...
bool GAEDAConfig::parse(int argc, char **argv, bool embedded) {
   ...
   try {
     ...
     general_opts.add_options()
       ...
       ("elitism-percentage", po::value<double>()->default_value(1.0), "Set the percentage of the old population that is brought into the new one.")
       ...
       ("reset-percentage", po::value<double>()->default_value(1.0), "Percentage of the population that will be re-initialized after a population restart.")

       ("rerun-percentage", po::value<double>()->default_value(1.0), "Percentage of the population that will be re-initialized in multiple runs.")
     ...
     convergence_opts.add_options()
       ...
       ("convergence,C", po::value<double>(), "Set the fitness convergence ratio. Use 'convergence-iters' to set the number of generations considered to compute the convergence"
       ...
       ("pop-convergence,U", po::value<double>(), "Set the population convergence ratio.")

       ("partial-pop-convergence,V", po::value<double>(), "Set a partial population convergence criterion.")
       ...
       ("timeConv", po::value<double>(), "Set maximum number of seconds for the algorithm to execute.")

       ("precission", po::value<double>()->default_value(1e-20), "Set required precission for fitness to consider convergence (in combination with evaluations).")
     ...
     ga_opts.add_options()
       ...
       ("mut-prob,M", po::value<double>()->default_value(0.01), "Set mutation probability.")

       ("cross-prob,X", po::value<double>()->default_value(0.9), "Set crossover probability.")
     ...
     de_opts.add_options()
       ...
       ("F,F", po::value<double>(), "Set the DE mutation factor Phi.")

       ("CR", po::value<double>(), "Set the DE crossover probability.")
     ...
     vns_opts.add_options()
       ("vnstinitratio", po::value<double>()->default_value(0.005),"Set the VNS t init ratio")

       ("vnstinitprob",  po::value<double>()->default_value(0.2),  "Set the VNS t init probability")

       ("vnslsratio1",   po::value<double>()->default_value(1.02), "Set the VNS 1st worsening ratio for executing the LS")

       ("vnslsprob",     po::value<double>()->default_value(0.021), "Set the VNS probability of executing the LS")

       ("vnslsratio2",   po::value<double>()->default_value(1.05), "Set the VNS 2nd worsening ratio for executing the LS")
     ...
     mts_opts.add_options()
       ...
       ("mts-adjustFailed", po::value<double>()->default_value(0.5), "Set the adjustment factor (*) to be used after one iteration of the LS without any improvement")

       ("mts-adjustMin",    po::value<double>()->default_value(0.4), "Set the adjustment factor (*) to be used if SR reaches its minimum vlue (1e-14)")

       ("mts-moveLeft",     po::value<double>()->default_value(1.0), "Set the movement factor (*) to be used when moving a solution left")

       ("mts-moveRight",    po::value<double>()->default_value(0.5), "Set the movement factor (*) to be used when moving a solution right")
     ...
     mtsred_opts.add_options()
       ...
       ("mtsred-searchProb", po::value<double>()->default_value(0.9), "Set the cummulated percentage of dimensions that will be searched (instead of exploring all of them , we just explore those cummulating this percentage")

       ("mtsred-minProb", po::value<double>()->default_value(0.05), "Set the minimum percentage of dimensions to explore");
     ...
     sw_opts.add_options()
       ...
       ("sw-adjustSuccess", po::value<double>()->default_value(2.0), "Set the adjustment factor (*) to be used after 'sw-maxSuccess' iterations of the LS with improvements")

       ("sw-adjustFailed",  po::value<double>()->default_value(0.5), "Set the adjustment factor (*) to be used after 'sw-maxFailed' iterations of the LS without improvements")

       ("sw-delta",         po::value<double>()->default_value(1.2), "Set the delta factor of Solis-Wets algorithm")
     ...
     adapde_opts.add_options()
       ...
       ("adapde-Fl",    po::value<double>()->default_value(0.1), "Set the lower bound for the adjustment interval of the F parameter of a DE algorithm")

       ("adapde-Fu",    po::value<double>()->default_value(0.9), "Set the upper bound for the adjustment interval of the F parameter of a DE algorithm")

       ("adapde-tauF",     po::value<double>()->default_value(0.1), "Set the threshold to choose if an adjustment on the F parameter should be done")

       ("adapde-tauCR",     po::value<double>()->default_value(0.1), "Set the threshold to choose if an adjustment on the CR parameter should be done")
     ...
     stsde_opts.add_options()
       ...
       ("stsde-prob",    po::value<double>()->default_value(0.4), "Set the probability to apply STSDE")
     ...
     mos_opts.add_options()
       ...
       ("minimum-part", po::value<double>()->default_value(0.05) , "Set the minimum participation of a MOS technique.")
       ...
       ("mosql-pmax", po::value<double>()->default_value(0.9), "Sets the selection pressure for actions in MOSEA-QL.")

       ("mosrl-alfa", po::value<double>()->default_value(0.7), "Sets the learning rate for MOSEA-RL.")

       ("mosrl-gamma", po::value<double>()->default_value(0.8), "Sets the discount rate for MOSEA-RL.")

       ("mosrl-beta", po::value<double>()->default_value(0.3), "Sets the reward rate for MOSEA-RL.")

       ("mosrl-delta", po::value<double>()->default_value(0.1), "Sets the Delta rate for MOSEA-RL (HPC only).")

       ("mosrl-deltal", po::value<double>()->default_value(0.2), "Sets the Delta Lose for MOSEA-RL (WoLF only).")

       ("mosrl-deltaw", po::value<double>()->default_value(0.05), "Sets the Delta Win for MOSEA-RL (WoLF only).")
       ...
       ("mos-bonus", po::value<double>()->default_value(0.01), "Sets the reward bonus parameter.")
       ...
       ("mos-shared-evals-percent", po::value<double>()->default_value(0.01), "Sets the percentage of shared evaluations to distribute in a MOS HRH algorithm.")

       ("mos-shared-evals-factor", po::value<double>(), "Sets the factor to compute the number of shared evaluations to distribute in a MOS HRH algorithm from the population size.")
     ...
     if( vm.count("elitism-percentage") ){
       elitism_percentage = vm["elitism-percentage"].as<double>();
     ...
     if (vm.count("convergence")) {
       ...
       mConv = vm["convergence"].as<double>();
     ...
     if (vm.count("pop-convergence")) {
       mConvCrit = POPCONV;
       mConv = vm["pop-convergence"].as<double>();
     ...
     if (vm.count("partial-pop-convergence")) {
       ...
       mConv = vm["partial-pop-convergence"].as<double>();
     ...
     if (vm.count("timeConv")) {
       mConvCrit      = TIMECONV;
       maxExecSeconds = vm["timeConv"].as<double>();
     ...
     if (vm.count("precission")) {
       mPrecission = vm["precission"].as<double>();
     }
     ...
     if (vm.count("reset-percentage")) {
       mResetPerc = vm["reset-percentage"].as<double>();
     ...
     if (vm.count("rerun-percentage")) {
       mRerunPerc = vm["rerun-percentage"].as<double>();
     ...
     if (vm.count("mut-prob")) {
       mMutProb = vm["mut-prob"].as<double>();
     ...
     if (vm.count("cross-prob")) {
       mCrossProb = vm["cross-prob"].as< double>();
     ...
     if (vm.count("F")) {
       if (vm["F"].as<double>() <= 0 || vm["F"].as<double>() >= 2 ){
         std::cerr << "[GAEDAConfig] Error: Wrong Phi constant value: " << vm["F"].as<double>() << ". Please select a value within (0, 2)." << std::endl;
         return false;
       }
       DE_F_factor = vm["F"].as<double>();
     }


     if (vm.count("CR")) {
       if (vm["CR"].as<double>() <= 0 || vm["CR"].as<double>() >= 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong crossover probability: " << vm["CR"].as<double>() << ". Please select a value within (0, 1) " << std::endl;
         return false;
       }
       DE_CR_factor = vm["CR"].as<double>();
     }
...
#define VNSOPT(optname,type) \
     ...
     VNSOPT(vnstinitratio,double);
     VNSOPT(vnstinitprob,double);
     VNSOPT(vnslsratio1,double);
     VNSOPT(vnslsprob,double);
     VNSOPT(vnslsratio2,double);
     ...
     if (vm.count("mts-adjustFailed")) {
       mts_adjustFailed = vm["mts-adjustFailed"].as<double>();
     }

     if (vm.count("mts-adjustMin")) {
       mts_adjustMin = vm["mts-adjustMin"].as<double>();
     }

     if (vm.count("mts-moveLeft")) {
       mts_moveLeft = vm["mts-moveLeft"].as<double>();
     }

     if (vm.count("mts-moveRight")) {
       mts_moveRight = vm["mts-moveRight"].as<double>();
     }
     ...
     if (vm.count("mtsred-searchProb")) {
       mtsred_searchProb = vm["mtsred-searchProb"].as<double>();
     }

     if (vm.count("mtsred-minProb")) {
       mtsred_minProb = vm["mtsred-minProb"].as<double>();
     }
     ...
     if (vm.count("sw-adjustSuccess")) {
       sw_adjustSuccess = vm["sw-adjustSuccess"].as<double>();
     }

     if (vm.count("sw-adjustFailed")) {
       sw_adjustFailed = vm["sw-adjustFailed"].as<double>();
     }

     if (vm.count("sw-delta")) {
       sw_delta = vm["sw-delta"].as<double>();
     }
     ...
     if (vm.count("adapde-Fl")) {
       adapde_Fl = vm["adapde-Fl"].as<double>();
     }

     if (vm.count("adapde-Fu")) {
       adapde_Fu = vm["adapde-Fu"].as<double>();
     }

     if (vm.count("adapde-tauF")) {
       adapde_tauF = vm["adapde-tauF"].as<double>();
     }

     if (vm.count("adapde-tauCR")) {
       adapde_tauCR = vm["adapde-tauCR"].as<double>();
     }
     ...
     if (vm.count("stsde-prob")) {
       stsde_prob = vm["stsde-prob"].as<double>();
     }
     ...
     if (vm.count("minimum-part")) {
       if (vm["minimum-part"].as<double>() < 0 || vm["minimum-part"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong minimum participation for MOS techniques: " << vm["minimum-part"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       minPart = vm["minimum-part"].as<double>();
     }
     ...
     if (vm.count("mosql-pmax")) {
       if (vm["mosql-pmax"].as<double>() < 0 || vm["mosql-pmax"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong PMax value for MOSQL: " << vm["mosql-pmax"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSQLPmax = vm["mosql-pmax"].as<double>();
     }


     if (vm.count("mosrl-alfa")) {
       if (vm["mosrl-alfa"].as<double>() < 0 || vm["mosrl-alfa"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong learning rate for MOSRL: " << vm["mosrl-alfa"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSRLAlfa = vm["mosrl-alfa"].as<double>();
     }


     if (vm.count("mosrl-gamma")) {
       if (vm["mosrl-gamma"].as<double>() < 0 || vm["mosrl-gamma"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong discount rate for MOSRL: " << vm["mosrl-gamma"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSRLGamma = vm["mosrl-gamma"].as<double>();
     }


     if (vm.count("mosrl-beta")) {
       if (vm["mosrl-beta"].as<double>() < 0 || vm["mosrl-beta"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong reward rate for MOSRL: " << vm["mosrl-beta"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSRLBeta = vm["mosrl-beta"].as<double>();
     }


     if (vm.count("mosrl-delta")) {
       if (vm["mosrl-delta"].as<double>() < 0 || vm["mosrl-delta"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong Delta rate for MOSRL (HPC): " << vm["mosrl-delta"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSRLDelta = vm["mosrl-delta"].as<double>();
     }


     if (vm.count("mosrl-deltal")) {
       if (vm["mosrl-deltal"].as<double>() < 0 || vm["mosrl-deltal"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong Delta Lose rate for MOSRL (WoLF): " << vm["mosrl-deltal"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSRLDeltaL = vm["mosrl-deltal"].as<double>();
     }


     if (vm.count("mosrl-deltaw")) {
       if (vm["mosrl-deltaw"].as<double>() < 0 || vm["mosrl-deltaw"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong Delta Win rate for MOSRL (WoLF): " << vm["mosrl-deltaw"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mMOSRLDeltaW = vm["mosrl-deltaw"].as<double>();
     }
     ...
     if (vm.count("mos-bonus")) {
       if (vm["mos-bonus"].as<double>() < 0 || vm["mos-bonus"].as<double>() > 1 ){
         std::cerr << "[GAEDAConfig] Error: Wrong reward bonus for MOS: " << vm["mos-bonus"].as<double>() << ". Please select a value within [0, 1]. " << std::endl;
         return false;
       }
       mBonus = vm["mos-bonus"].as<double>();
     }
     ...
     if (vm.count("mos-shared-evals-percent")) {
       if (vm["mos-shared-evals-percent"].as<double>() < 0 || vm["mos-shared-evals-percent"].as<double>() > 1){
         std::cerr << "[GAEDAConfig] Error: Wrong number of shared evaluations for MOS HRH: " << vm["mos-shared-evals-percent"].as<double>() << ". Please select a value within (0, 1]. " << std::endl;
         return false;
       }
       mSharedEvalsPercent = vm["mos-shared-evals-percent"].as<double>();
     }


     if (vm.count("mos-shared-evals-factor")) {
       if (vm["mos-shared-evals-factor"].as<double>() < 1.0){
         std::cerr << "[GAEDAConfig] Warning: The value for the factor to compute the shared evaluations for MOS HRH is smaller than the population size: " << vm["mos-shared-evals-factor"].as<double>() << ". Be sure that this is what you actually want. " << std::endl;
       }
       mSharedEvalsFactor = vm["mos-shared-evals-factor"].as<double>();
...
double GAEDAConfig::getVNSTinitRatio()              const { return vnstinitratio_;              }
double GAEDAConfig::getVNSTinitProb()               const { return vnstinitprob_;               }
double GAEDAConfig::getVNSLSRatio1()                const { return vnslsratio1_;                }
double GAEDAConfig::getVNSLSProb()                  const { return vnslsprob_;                  }
double GAEDAConfig::getVNSLSRatio2()                const { return vnslsratio2_;                }

...
/* AUX FUNCTIONS */
void ludcmp (int nn, double* matrix, double* d);
...
long double Matrix::Determinant () {
   ...
   if (DETERMINANT_CALCU == 0) {
      ...
      m_data2 = (double*) calloc (m_rows * m_columns, sizeof (double));
      ...
      for (i = 0; i < m_rows; i++)
         for (int j = 0; j < m_columns; j++)
            m_data2 [i * m_rows + j] = (double) m_data [i][j];
   ...
   *d = (double) 1.0;
   ...
   for (i = 0; i < nn; i++) {
      ...
      for (j = i; j < nn; j++) {
         ...
         temp = (double) fabs (matrix [i * nn + j]);
   ...
   for (j = 0; j < nn; j++) {
      ...
      if (matrix [j * nn + j] == (double) 0.0)
         matrix [j * nn + j] = TINY;

      if (j != nn) {

         dum = (double) ((double) 1.0 / matrix [j * nn + j]);

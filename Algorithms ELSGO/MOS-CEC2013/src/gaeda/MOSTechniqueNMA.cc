...
MOSGenome* MOSTechniqueNMA::centroidOf(const GAPopulation& pop) const {
  ...
  GA1DArrayAlleleGenome<double>& centroidReal = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*centroid->getGenome(_encoding));
  ...
  for (unsigned dim = 0; dim < centroidReal.length(); dim++) {
    ...
    for (unsigned ind_pos = 0; ind_pos < pop.size(); ind_pos++) {
      ...
      new_val           += dynamic_cast<GA1DArrayAlleleGenome<double>&>(*tmp_ind.getGenome(_encoding)).gene(dim);
    }
...
MOSGenome* MOSTechniqueNMA::reflection(const MOSGenome& centroid, const MOSGenome& worst_ind, double rho) const {
  ...
  GA1DArrayAlleleGenome<double>& reflx_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*reflx_ind->getGenome(_encoding));
  GA1DArrayAlleleGenome<double>& centroid_real  = dynamic_cast<GA1DArrayAlleleGenome<double>&>(  *centroid.getGenome(_encoding));
  GA1DArrayAlleleGenome<double>& worst_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>( *worst_ind.getGenome(_encoding));
  ...
  double bestParentFit = worst_ind.score();
...
void MOSTechniqueNMA::expansion(const MOSGenome& centroid, const MOSGenome& reflx_ind, MOSGenome& worst_ind, double chi, stringstream& msg) const {
  ...
  GA1DArrayAlleleGenome<double>& expan_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*expan_ind->getGenome(_encoding));

  double max_value = centroid_real.alleleset(0).upper();
  double min_value = centroid_real.alleleset(0).lower();

  for (unsigned dim = 0; dim < expan_ind_real.length(); dim++) {
    double new_gene = centroid_real.gene(dim) + chi * (reflx_ind_real.gene(dim) - centroid_real.gene(dim));
  ...
  double bestParentFit = reflx_ind.score();
...
MOSGenome* MOSTechniqueNMA::outsideContraction(const MOSGenome& centroid, const MOSGenome& reflx_ind, double gamma) const {
  MOSGenome*                     contr_ind      = dynamic_cast<MOSGenome*>(centroid.clone());
  GA1DArrayAlleleGenome<double>& contr_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*contr_ind->getGenome(_encoding));
  GA1DArrayAlleleGenome<double>& centroid_real  = dynamic_cast<GA1DArrayAlleleGenome<double>&>(  *centroid.getGenome(_encoding));
  GA1DArrayAlleleGenome<double>& reflx_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>( *reflx_ind.getGenome(_encoding));
...
MOSGenome* MOSTechniqueNMA::insideContraction(const MOSGenome& centroid, const MOSGenome& worst_ind, double gamma) const {
  MOSGenome*                     contr_ind      = dynamic_cast<MOSGenome*>(centroid.clone());
  GA1DArrayAlleleGenome<double>& contr_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*contr_ind->getGenome(_encoding));
  GA1DArrayAlleleGenome<double>& centroid_real  = dynamic_cast<GA1DArrayAlleleGenome<double>&>(  *centroid.getGenome(_encoding));
  GA1DArrayAlleleGenome<double>& worst_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>( *worst_ind.getGenome(_encoding));
...
void MOSTechniqueNMA::shrinkPop(const GAPopulation& pop, unsigned& used_evals, unsigned max_evals, double sigma) const {
  ...
  GA1DArrayAlleleGenome<double>& best_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*best_ind.getGenome(_encoding));

  for (int ind_pos=1; ind_pos<pop.size() && used_evals <= max_evals; ind_pos++, used_evals++) { // First pos = 1 not an error

    MOSGenome&                     tmp_ind      = dynamic_cast<MOSGenome&>(pop.individual(ind_pos));
    GA1DArrayAlleleGenome<double>& tmp_ind_real = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*tmp_ind.getGenome(_encoding));

    double bestParentFit = tmp_ind.score();
...
void MOSTechniqueNMA::contraction(const MOSGenome& centroid, const MOSGenome& reflx_ind, MOSGenome& worst_ind, const GAPopulation& pop, unsigned& used_evals, unsigned max_evals, unsigned& branch, double gamma, double sigma, stringstream& msg) const {
...
bool MOSTechniqueNMA::NMA (GAPopulation& pop, /* out */ unsigned& used_evals, unsigned max_evals, unsigned& branch, double rho, double chi, double gamma, double sigma) {
  ...
  double     old_worst_score = worst_ind.score();

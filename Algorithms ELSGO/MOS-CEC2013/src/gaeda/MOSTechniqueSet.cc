 ...
 */
bool MOSTechniqueSet::setPartRatio (const techIdType techniqueId, const double partRatio) {
 ...
 *
 * @param techniqueID string que identifica la técnica
 * @return double ratio de participación
 */
double MOSTechniqueSet::getPartRatio (const techIdType techniqueId) const {
 ...
 */
double MOSTechniqueSet::sumPartRatios() {
  double ratiosAcum = 0.0;
  TechniqueSet::iterator it;
  for (it=_techniqueSet.begin(); it!=_techniqueSet.end(); it++) {
 ...
 */
double MOSTechniqueSet::getTechQuality(techIdType techniqueId) {
 ...
 */
void MOSTechniqueSet::setTechQuality(techIdType techniqueId, double quality) {
...
void MOSTechniqueSet::offspringAll(GAPopulation* origPop, GAPopulation* destPop, unsigned maxEvals) {
  ...
  unsigned offset = 0;
  double ratiosAcum = sumPartRatios();
...
void MOSTechniqueSet::offspringAll(std::vector<GAPopulation*>& origPops, std::vector<GAPopulation*> destPops) {
  ...
  unsigned offset = 0;
  double ratiosAcum = (GAEDAConfig::handle()->getPartFunction() != dynQualityMaxPartPF) ? sumPartRatios() : 1.0;
  MOSTechnique* currentTechnique = NULL;
...
MOSProbVector& MOSTechniqueSet::bonusTechnique(MOSProbVector& probVector, techIdType techniqueId) {
  ...
  double incBonusTech = probVector[techniqueId] * _bonus;
  ...
  double penNotAply = 0.0;
  // Calcular penalización inicial
  double decTech = incBonusTech / decList;
...
void MOSTechniqueSet::initPartRatios() {
  TechniqueSet::iterator it;
  double partRatio = 1.0 / _techniqueSet.size();
 ...
 * @return std::vector<double>* Vector con los ratios de participacion
 */
std::vector<double>* MOSTechniqueSet::getPartRatiosVector(std::vector<double>* ratiosVector) const {
 ...
 * PRE: Presuponemos que el vector que nos pasan es del mismo tamano que el numero de tecnicas
 * @return std::vector<double>* Vector con la calidad
 */
std::vector<double>* MOSTechniqueSet::getQualityVector(std::vector<double>* qualityVector) const {
  TechniqueSet::const_iterator it;
  unsigned i = 0;
...
techIdType MOSTechniqueSet::getWorstTechniqueId() const {
  TechniqueSet::const_iterator it;
  double worstQuality = 999999999999.0;
...
bool MOSTechniqueSet::updateStrategies(const std::vector<GAGenome*>& parents, GAGenome* child) const {
  ...
  if (_esTechs.size() > 0) {
    ...
    double prob=GARandomDouble(0,1);

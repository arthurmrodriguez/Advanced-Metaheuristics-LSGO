...
VNS::VNS(const  GAGenome& g,
         ...
         double ls_ratio_4improv1,    double ls_prob, double ls_ratio_4improv2,
...
bool VNS::feasible(const RoutingGenome& g) {
  ...
  if (f and tinit_ == 0) {
    ...
    ratioPerEval_ = t_ / (double) GAEDAConfig::handle()->getEvals();
  }
...
void VNS::step(){
  ...
  double prand = GARandomDouble(0, 1);
  ...
  // Line 14: Move or not
  double pSA = (t_ > 0) ? (1.0 / (exp((s1->score() - bestsol_->score()) / t_))) : -1;

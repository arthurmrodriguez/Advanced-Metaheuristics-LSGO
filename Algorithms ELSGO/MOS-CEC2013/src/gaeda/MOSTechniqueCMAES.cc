...
MOSTechniqueCMAES::MOSTechniqueCMAES(techIdType id, std::string description, GAGenome::Initializer init,
  ...
  double stddevs [psize];
  double initialX[psize];

  std::fill_n(stddevs,  psize, 0.3);
  std::fill_n(initialX, psize, 0.5);

  GA1DArrayAlleleGenome<double>& tmp_gen = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*genomeBase);
...
bool MOSTechniqueCMAES::restartInnerData(GAPopulation* pop) {
  ...
  GA1DArrayAlleleGenome<double>& tmp_gen = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*dynamic_cast<MOSGenome&>(pop->individual(GARandomInt(0, pop->size()-1))).getGenome(GAID::RealEncoding));
  for (unsigned i = 0; i < psize; i++)
    initialX[i] = tmp_gen.gene(i);
...
unsigned MOSTechniqueCMAES::offspring (unsigned maxEvals, unsigned& usedEvals, GAPopulation* pop, GAPopulation* auxPop, bool& converged) {
  ...
  double fits[(int)cmaes_Get(&_cmaes_evo, "lambda")];
  ...
  for (unsigned i = 0; i < cmaes_Get(&_cmaes_evo, "lambda"); i++) {
    ...
    GA1DArrayAlleleGenome<double>& g = dynamic_cast<GA1DArrayAlleleGenome<double>&> (*gen.getGenome(GAID::RealEncoding));

...
double gaDefLinearScalingMultiplier   = 1.2;
double gaDefSigmaTruncationMultiplier = 2.0;
double gaDefPowerScalingFactor        = 1.0005;
double gaDefSharingCutoff             = 1.0;
...
void GAStandardScaling::evaluate(const GAPopulation& p) {
  ...
  double best  = GAGenome::optCriterion() == GAGenome::MAXIMIZATION ? p.max() : p.min();
  double worst = GAGenome::optCriterion() == GAGenome::MAXIMIZATION ? p.min() : p.max();
...
// a negative value, dump an error message then set all of the scores to 0.
  ...
  for(unsigned i=0; i<p.size(); i++){
    ...
    p.individual(i).fitness((double)f);       // might lose information here!
  }
}
...
double
GALinearScaling::multiplier(double fm) {
  if(fm <= 1.0){
...
// equal to zero.
double
GASigmaTruncationScaling::multiplier(double fm) {
  if(fm < 0.0){

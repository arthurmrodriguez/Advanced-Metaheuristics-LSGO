...
MOSTechniqueAdapSTSDE::MOSTechniqueAdapSTSDE(techIdType id, std::string description, GAGenome::Initializer init,
              ...
              : MOSTechniqueSTSDE(id, description, init, evaluator, encoding, genomeBase, crossover, F, CR, selector ) {}

void MOSTechniqueAdapSTSDE::offspring(MOSGenome& dad, MOSGenome& mom, GAPopulation* origPop, GAPopulation* destPop, unsigned size, unsigned offset) {
  selfAdjustDEParams(*origPop);
  MOSTechniqueSTSDE::offspring(dad, mom, origPop, destPop, size, offset);
}

void MOSTechniqueAdapSTSDE::offspring(GAPopulation* origPop, GAPopulation* destPop, unsigned size, unsigned offset) {
  selfAdjustDEParams(*destPop);
  MOSTechniqueSTSDE::offspring(origPop, destPop, size, offset);
}

unsigned MOSTechniqueAdapSTSDE::offspring (unsigned maxEvals, unsigned& usedEvals, GAPopulation* pop, GAPopulation* auxPop, bool& converged) {
  selfAdjustDEParams(*pop);
  return MOSTechniqueSTSDE::offspring (maxEvals, usedEvals, pop, auxPop, converged);
}

void MOSTechniqueAdapSTSDE::selfAdjustDEParams(const GAPopulation& pop) {
  static bool first_call = true;

  GAEDAConfig* cfg = GAEDAConfig::handle();

  long double tau1 = cfg->getAdapDETauF ();
  long double tau2 = cfg->getAdapDETauCR();
  long double Fl   = cfg->getAdapDEFl   ();
  long double Fu   = cfg->getAdapDEFu   ();
  ...
  for (unsigned i = 0; i < sz; i++) {
    long double r1 = GARandomDouble(0, 1);
    long double r2 = GARandomDouble(0, 1);
    long double r3 = GARandomDouble(0, 1);
    long double r4 = GARandomDouble(0, 1);
...
void MOSTechniqueAdapSTSDE::offspring_internal(MOSGenome& genomeM, MOSGenome& newGenomeM, MOSGenome& genome1M, MOSGenome& genome2M, MOSGenome& genome3M) {
  ...
  for (unsigned i = 0; i < dim; i++) {
    long double new_val = genome1->gene(i) + _Fv[_curTarget] * (genome2->gene(i) - genome3->gene(i));

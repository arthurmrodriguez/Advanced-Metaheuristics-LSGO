...
unsigned MOSTechniqueLSObj::offspring (unsigned maxEvals, unsigned& usedEvals, GAPopulation* pop, GAPopulation* auxPop, bool& converged) {
  ...
  if (!_swparams) _swparams = _sw.getInitOptions(dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*g.getGenome(_encoding)), pop);
  ...
  do {
    long double fit_inc_acum = 0.0;
    unsigned evals = 0;
    unsigned res = _sw.apply(_swparams, dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*g.getGenome(_encoding)), fitBest, evals, fit_inc_acum, _times_improved, maxEvals-usedEvals);
    g.score(g.getGenome(_encoding)->score());
    fit_inc_acum_total += fit_inc_acum;
    usedEvals+=evals;

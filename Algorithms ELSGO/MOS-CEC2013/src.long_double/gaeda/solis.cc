...
void SolisWets::setDelta(long double maxdelta) {
  assert(maxdelta > 0);
  m_maxdelta = maxdelta;
}

void SolisWets::setDelta(long double mindelta, long double maxdelta) {
...
long double SolisWets::normal(long double desv) {
  long double u1, u2;
  long double result;
...
long double SolisWets::distreal(const tChromosomeReal &x, const tChromosomeReal &y) {
  long double dist = 0;
...
long double SolisWets::distanceMin(const tChromosomeReal &x, GAPopulation *pop, unsigned *posmin) {
  long double dist, lowest;
  unsigned i;
...
SolisParams *SolisWets::getInitOptions(tChromosomeReal &sol, GAPopulation* m_pop) {
  ...
  //    */
  //   long double dist = distanceMin(sol, m_pop, &nearest);
...
static long double increm_bias(const long double &bias, const long double &dif) {
  return 0.2*bias+0.4*(dif+bias);
}

static long double dec_bias(const long double &bias, const long double &dif) {
  return bias-0.4*(dif+bias);
}

unsigned SolisWets::apply(SolisParams *params, tChromosomeReal &sol, tFitness &sol_perf, unsigned& evals, long double& fit_inc_acum, unsigned& improvements, unsigned maxeval) {

  static const long double maxSuccess    = GAEDAConfig::handle()->getSWMaxSuccess   ();
  static const long double maxFailed     = GAEDAConfig::handle()->getSWMaxFailed    ();
  static const long double adjustSuccess = GAEDAConfig::handle()->getSWAdjustSuccess();
  static const long double adjustFailed  = GAEDAConfig::handle()->getSWAdjustFailed ();
  ...
  for (numEval = 0; numEval < maxeval;) {
    ...
    //else if (numEval < maxeval && !m_problem->isBetter(newsol_perf, sol_perf) && !m_running->isFinish()) {
      ...
      if (GAGenome::compareScores(newsol_perf, sol_perf) == GAGenome::BETTER) {
        ...
        transform(p->bias.begin(), p->bias.end(), p->bias.begin(), bind2nd(multiplies<long double>(), 0.5));
        p->numFailed++;
        p->numSuccess = 0;
      }

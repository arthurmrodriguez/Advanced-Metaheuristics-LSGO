...
inline long double GARandomDouble(){
  return _GA_RND(); }
// Changed from the original version for using the better random system 
// functions. Old code is commented below
inline long double GARandomDouble(long double low, long double high){
  long double val=high-low; val*=_GA_RND(); return val+low;
}

/* inline long double GARandomDouble(long double low, long double high){ */
/*   long double temp; */

/*   if (low > high) {           // swap low & high around if the user makes no sense  */
/*     temp = low; */
/*     low = high; */
/*     high = temp; */
/*   } */

/*   temp = (random() / (static_cast<long double>(RAND_MAX) + 1.0)) * (high - low) + low; */
/*   return temp; */
/* } */
...
inline long double GARandomDouble(){
  long double val=_GA_RND(); val/=_GA_RND_MAX; return val;
}
inline long double GARandomDouble(long double low, long double high){
  long double val=high-low; val*=_GA_RND(); val/=_GA_RND_MAX; val+=low; return val;
}
...
unsigned int GAGetRandomNoRankSeed();
void GARandomSeed(unsigned int seed=0, int rank=0);
void GAResetRNG(unsigned int seed);
int GARandomBit();
long double GAUnitGaussian();
...
inline long double GAGaussianDouble(long double dev){ return GAUnitGaussian() * dev; }

const char* GAGetRNG();

// With this definitions we can redefine the random functions (for example, for returning always an specific value)
// for conducting the tests
#if defined USEDUMMYRANDOM
#define GARandomInt GARandomIntDummy
#define GARandomDouble GARandomDoubleDummy
#define GARandomFloat GARandomFloatDummy

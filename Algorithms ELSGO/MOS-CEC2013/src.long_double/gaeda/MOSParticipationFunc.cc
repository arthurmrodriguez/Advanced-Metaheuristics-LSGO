...
extern "C" void alternatingQualityPF (MOSEA& alg) {
  ...
  static long double gensPerTechCont [10];

  // Cache for storing the cumulated quality of each technique
  static long double qualPerTech [10];
  ...
  if (curGen == 0) {
    ...
    for (unsigned i = 0; i < nTechs; i++) {
      gensPerTechCont [i] = (long double) gens / (long double) sharedGens;
      gensPerTech [i] = gens;
      total += gens;
      if (i == 0)
  ...
  long double avgQual [nTechs];

  // Compute the average quality obtained by each technique in its assigned iterations
  for (unsigned i = 0; i < nTechs; i++)
    avgQual [i] = (gensPerTech[i] > 0) ? qualPerTech [i] / (long double) gensPerTech [i] : 0.0;
  ...
  long double adjust = (tmp == NULL) ? 0.05 : *((long double*) tmp);

  if (adjust <= 0 || adjust > 1)
    adjust = 0.05;

  // Get the min participation value
  long double minGens = 0.10;

  long double bestQuality = avgQual [bestTechnique];

  for (unsigned i = 0; i < nTechs; i++) {

    long double diff = (bestQuality == 0.0) ? 1.0 : (bestQuality - avgQual [i]) / (bestQuality - base);
    long double gensInc = gensPerTechCont [i] * adjust * diff;
...
extern "C" void dynQualityMOSPF (MOSEA& alg) {
  ...
  if (tmp == NULL)
    ...
    adjust = *((long double*) tmp);

  if (adjust <= 0 or adjust > 1)
    adjust = 0.05;

  // Get the min participation value
  long double minPart = alg.getMinParticipation();
  ...
  for (it=techSet.begin(); it!=techSet.end(); it++) {
    if (it->first != bestTechId) {
      long double bestQuality = techSet.getTechnique(bestTechId)->getQuality();
      long double diff;
      if (bestQuality == 0.0)
         diff = 1.0;
      else
      ...
      long double ratioInc = currentTechPart * adjust * diff;
...
extern "C" void dynQualityMaxPartPF (MOSEA& alg) {
  ...
  static long double maxPart = 1.0 / (long double) techSet.nTechniques();
  ...
  techIdType bestTechId    = techSet.getBestTechniqueId();
  long double     bestQuality   = techSet.getTechnique(bestTechId)->getQuality();
  long double     bestPartRatio = techSet.getPartRatio(bestTechId);
  ...
  long double sumRatios = 0.0;
  long double diff = 0.0;

  for (it=techSet.begin(); it!=techSet.end(); it++) {

    long double alfa = it->second->getQuality() / bestQuality;

    long double currentTechPart = techSet.getPartRatio(it->first);
    long double newRatio = alfa * currentTechPart;
...
extern "C" void dynQualityMOSPF2 (MOSEA& alg) {
  ...
  std::vector<long double> quals (nTechs, 0);
  ...
  long double bestQuality = techSet.getTechnique(pos[nTechs-1])->getQuality();

  long double diff = 0.0;

  for (unsigned i = 0; i < nLose; i++) {

    long double alfa = techSet.getTechnique(pos[i])->getQuality() / bestQuality * 0.05;

    long double currentTechPart = techSet.getPartRatio(pos[i]);
    long double newRatio = currentTechPart - alfa;
  ...
  for (unsigned i = nLose; i < nTechs; i++) {
    ...
    long double currentTechPart = techSet.getPartRatio(pos[i]);
    long double inc = techSet.getTechnique(pos[i])->getQuality() / sumQualities * diff;

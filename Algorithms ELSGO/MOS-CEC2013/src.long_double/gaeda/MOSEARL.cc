 ...
 */
MOSEARL::MOSEARL(const GAGenome& genome, const long double elitismPercent)
...
MOSEARL::MOSEARL(const GAPopulation& population, const long double elitismPercent)
...
MOSEARL::MOSEARL(const GAGeneticAlgorithm& alg, const long double elitismPercent)
...
void MOSEARL::step() {
  ...
  for (unsigned i = 0; i < pop->size(); i+=2) {
    ...
    fracPart = modfl (((long double)_Inds[action]/(long double)_totalInds) * 10.0, &intPart);
    ...
    if (discretePart > _parts[action] && stats.generation() > 25) {
      ...
      std::vector< std::pair<unsigned, long double> > decimals (_parts.size());
      unsigned totalDisc = 0;

      for (unsigned j = 0; j < _parts.size(); j++) {

         fracPart = modfl (((long double)_Inds[j]/(long double)_totalInds) * 10.0, &intPart);
    ...
    long double popAvgFit = stats.current(GAStatistics::Mean);
    long double popMaxFit = stats.current(GAStatistics::Maximum);
    long double fitThreshold = (popAvgFit+popMaxFit)/2;

    long double fit, r, oldQ, maxQ;
  ...
  for (unsigned i = 0; i < _auxPop->size(); i++) {
    ...
    _avgFit[id] += (gen.score() / (long double) _partialInds[id]);
  }
...
unsigned MOSEARL::registerState(const std::vector<unsigned>& parts) {
  ...
  if (_QLStates.find(key) != _QLStates.end())
    ...
    std::vector<long double> v (nTechs, 0.0);
    ...
    std::vector<long double> va (nTechs, 1.0 / (long double) nTechs);
...
MOSTechnique* MOSEARL::selectTechniquePMax(unsigned& action) const {
  ...
  long double prob = GARandomDouble (0.0, 1.0);
  ...
  if (prob > PMAX) {
    ...
    std::vector<long double> probs = _QLearner[_state];
    ...
    long double suma = (long double)(probs.size() * (probs.size() + 1))/2.0;
    ...
    for (unsigned i = 0; i< probs.size();) {
      ...
      while (j < probs.size() && probs[pos[j]]==probs[pos[i]]) {
        ...
        unsigned end=j+1;
        for(;i<j;i++)
          probs[pos[i]] = (((long double)(end - 1 + base)/2.0))/suma;
    ...
    // select a technique
    std::vector< std::pair<unsigned, long double> > probsPairs;
    for (unsigned i = 0; i < probs.size(); i++)
      probsPairs.push_back(std::pair<unsigned, long double> (i, probs[i]));

    // Sort the vector
    std::sort(probsPairs.begin(), probsPairs.end(), pairCmp);

    // New random number to select a technique
    long double rnd = GARandomDouble(0.0, 1.0);
...
void MOSEARL::updateQualityAndParticipation() {
   ...
   for (it = _techs.begin(); it != _techs.end(); it++) {
      ...
      it->second->setPartRatio((long double) _partialInds[it->first] / (long double) pop->size());
 ...
 */
long double MOSEARL::scaleFitness(long double fit) const {

   long double popMaxFit = stats.current(GAStatistics::Maximum);
   long double popMinFit = stats.current(GAStatistics::Minimum);
...
void MOSEARL::updateActionProbabilityWoLF(unsigned action) {
  ...
  long double currentSum = 0.0, averageSum = 0.0;
  ...
  long double delta = (currentSum > averageSum) ? DELTA_W : DELTA_L;
...
void MOSEARL::updateAveragePolicyEstimateWoLF() {
  ...
  for (unsigned i = 0; i < _averagePolicyEstimate[_state].size(); i++) {
    _averagePolicyEstimate[_state][i] += (1.0 / (long double) _counter[_state]) *
...
bool MOSEARL::readRLFile() {
  ...
  if (f.good()) {
    ...
    std::vector<long double> v (maxj, 0.0);
    ...
    for (unsigned i = 0; i < maxi; i++)
      for (unsigned j = 0; j < maxj; j++) {
         long double val;
         f.read ((char*) &val, sizeof(long double));
         _QLearner[i][j] = val;
      }
  ...
  if (f.good()) {
    ...
    for (unsigned i = 0; i < _QLearner.size(); i++)
      for (unsigned j = 0; j < _QLearner[i].size(); j++) {
        long double val = _QLearner[i][j];
        f.write ((char*) &val, sizeof(long double));
      }
    ...
    for (unsigned i = 0; i < _actionProb.size(); i++)
      for (unsigned j = 0; j < _actionProb[i].size(); j++) {
        long double val = _actionProb[i][j];
        f.write ((char*) &val, sizeof(long double));
      }
    ...
    for (unsigned i = 0; i < _averagePolicyEstimate.size(); i++)
      for (unsigned j = 0; j < _averagePolicyEstimate[i].size(); j++) {
        long double val = _averagePolicyEstimate[i][j];
        f.write ((char*) &val, sizeof(long double));
      }

...
vector<GAGenome*>* RandomHighSeparatedCentGenerator::generateCentroids() const {
  ...
  long double             sum_cents     = sumMinDistancesBetweenCentroids(*centroids);

  for (unsigned int i=0; i<max_tries; i++){
    vector<GAGenome*>* centroids_temp = createARandomCentroidsDistribution();
    long double             sum_cents_temp = sumMinDistancesBetweenCentroids(*centroids_temp);
...
long double RandomHighSeparatedCentGenerator::sumMinDistancesBetweenCentroids(vector<GAGenome*>& centroids) const{
  assert(centroids.size() == cent_number);
  vector<long double> min_distances(cent_number);
...
//  vector<GAGenome*>* centroids     = createARandomCentroidsDistribution();
//  long double             sum_cents     = sumDistancesBetweenCentroids(*centroids);
//
//  for (unsigned int i=0; i<max_tries; i++){
//    vector<GAGenome*>* centroids_temp = createARandomCentroidsDistribution();
//    long double             sum_cents_temp = sumDistancesBetweenCentroids(*centroids_temp);
//
//    if (sum_cents_temp > sum_cents){                //We do the swap so centroids_temp at the end point to centroids
//      vector<GAGenome*>* centroids_aux = centroids;
//      centroids      = centroids_temp;
...
//long double RandomHighSeparatedCentGenerator::sumDistancesBetweenCentroids(vector<GAGenome*>& centroids){
//  long double total_sum = 0.0;
//
//  for(unsigned int i=0; i<cent_number; i++){
...
//    for (unsigned int j=i+1; j<cent_number; j++){
//      GAGenome& centroid_j = *centroids[j];
//      long double distance       = ( centroid_i.comparator() ) (centroid_i, centroid_j);
...
long double RandomHighSeparatedCentGenerator::findMinDistanceToOtherCentroids(vector<GAGenome*>& centroids, unsigned int pos) const{
  GAGenome&    centroid      = *centroids[pos];
  unsigned int first_pos     = (pos == 0) ? 1 : 0;
  long double       min_distance  = centroid.compare(*centroids[first_pos]);
  long double       temp_distance = 0.0;

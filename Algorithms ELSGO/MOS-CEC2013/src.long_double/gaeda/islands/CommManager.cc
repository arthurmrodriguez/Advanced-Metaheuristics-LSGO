...
long double CommManager::getTime () const{
  return MPI_Wtime();
}
...
void CommManager::sendDoubleVectorToMaster( vector<long double>& data ) const {
...
vector<long double> CommManager::receiveDoubleVectorFromIsland(int rank, int maxsize) {
  vector<long double> data(maxsize);
...
//void CommManager::sendFitness(long double fitness, int dest_island_rank){
//  MPI_Send (&fitness, 1, MPI_DOUBLE,d est_island_rank, TAG_SEND_DATA, MPI_COMM_WORLD);
//}

//long double CommManager::receiveFitness(int source){
//  long double result;

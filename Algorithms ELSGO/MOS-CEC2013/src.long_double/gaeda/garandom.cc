...
/* ----------------------------------------------------------------------------
  random.C
  mbwall 5sep95
  Copyright (c) 1995 Massachusetts Institute of echnology
                     all rights reserved
...
static unsigned int norankseed=0;

unsigned int GAGetRandomSeed()       { return seed; }
unsigned int GAGetRandomNoRankSeed() { return norankseed; }

void GARandomSeed(unsigned int s, int rank) {
...
GAUnitGaussian(){
  ...
  static long double cachevalue;
  if(cached == gaTrue){
    cached = gaFalse;
    return cachevalue;
  }

  long double rsquare, factor, var1, var2;
  do{
  ...
  long double val = -2.0 * log(rsquare) / rsquare;

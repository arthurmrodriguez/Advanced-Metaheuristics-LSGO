 ...
 * Constructors
 */
MOSEA::MOSEA (const GAGenome& genome, const long double elitismPercent, const long double minPart, const evolutionType evolutiveApproach)
...
MOSEA::MOSEA (const GAPopulation& population, const long double elitismPercent, const long double minPart, const evolutionType evolutiveApproach)
...
MOSEA::MOSEA (const GAGeneticAlgorithm& alg, const long double elitismPercent, const long double minPart, const evolutionType evolutiveApproach)
...
void MOSEA::fitnessAverageQuality(GAPopulation* evalPop) {
  ...
  std::map<techIdType,long double>   acumQualityVector;
...
void MOSEA::diversityAvgQuality(GAPopulation* evalPop) {
   ...
   std::map<techIdType,long double> diversities_avg = computeDiversitiesAvg(evalPop);
   MOSTechniqueSet::MOSTechniqueSetIterator it;
...
void MOSEA::compassQuality(GAPopulation* evalPop) {
   ...
   long double tmp_fitavg, tmp_divavg;
   MOSTechniqueSet::MOSTechniqueSetIterator it;
   ...
   for (it=_techniqueSet->begin(); it!=_techniqueSet->end(); it++) {
      ...
      long double compassvalue = (tmp_fitavg + tmp_divavg) / 2.0;
...
void MOSEA::NSCQuality(GAPopulation* evalPop) {
  ...
  MOSTechniqueSet::MOSTechniqueSetIterator it;
  for (it=_techniqueSet->begin(); it!=_techniqueSet->end(); it++) {
    long double quality = _nsc->computeNSC(evalPop, it->first);
...
std::map<techIdType,long double> MOSEA::computeDiversitiesAvg(GAPopulation* evalPop) {

   std::map<techIdType,long double>              diversities_avg;
...
GAPopulation* MOSEA::stepAutonomic(unsigned maxEvals) {
  ...
  std::vector<long double> partialInds (_techniqueSet->nTechniques(), 0);
  ...
  for (it = _techniqueSet->begin(); it != _techniqueSet->end(); it++)
    it->second->setPartRatio((long double) partialInds[it->first] / (long double) pop->size());
...
MOSTechnique* MOSEA::selectTechnique(MOSProbVector& probVector) {
  ...
  long double randomValue = GARandomDouble(0.0, 1.0);
  long double acumProb = 0.0;

...
class VNS : public RoutingAlg {
  ...
  long double   t_;
  long double   tinit_;
  long double   ratioPerEval_;
  unsigned shakerpos_;

  int initialEvalRouteCalls_; // Used for the update of the t_ parameter of the Simulated Anhealing behavior.

  // Set by user
  long double tinitratio_;
  long double tinitprob_;
  long double ls_ratio_4improv1_;
  long double ls_prob_;
  long double ls_ratio_4improv2_;
...
public:
  ...
  VNS(const  GAGenome& g,
      ...
      long double ls_ratio_4improv1,    long double ls_prob, long double ls_ratio4improv2,

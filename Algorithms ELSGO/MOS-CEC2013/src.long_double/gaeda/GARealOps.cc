...
void RealUniformInitializer (GAGenome& g) {
 ...
 void RealOrderedInitializer (GAGenome& g) {
   return OrderedInitializer<long double> (g);
 }
...
long double RealElementComparator (const GAGenome& g1, const GAGenome& g2) {
  return ElementComparator<long double> (g1, g2);
}
...
long double RealEuclideanLinearizedComparator (const GAGenome& g1, const GAGenome& g2) {

   const GA1DArrayAlleleGenome<long double>& genome = dynamic_cast <const GA1DArrayAlleleGenome<long double>&> (g1);

   long double max_diff  = genome.alleleset(1).upper () - genome.alleleset(1).lower ();
   long double max_value = sqrt (max_diff * max_diff) * genome.length ();

   assert(max_value != 0.0);

   long double return_value = (RealEuclideanComparator (g1, g2) + max_value) / (2.0 * max_value);
...
long double RealChebyshevComparator (const GAGenome& g1, const GAGenome& g2) {

   const GA1DArrayGenome<long double>& sis = DYN_CAST (const GA1DArrayGenome<long double>&, g1);
   const GA1DArrayGenome<long double>& bro = DYN_CAST (const GA1DArrayGenome<long double>&, g2);
...
long double RealManhattanComparator (const GAGenome& g1, const GAGenome& g2) {
...
int RealOnePointCrossover     (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return OnePointCrossover<long double>     (p1, p2, c1, c2);}
int RealTwoPointsCrossover    (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return TwoPointsCrossover<long double>    (p1, p2, c1, c2);}
int RealEvenOddCrossover      (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return EvenOddCrossover<long double>      (p1, p2, c1, c2);}
int RealPartialMatchCrossover (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return PartialMatchCrossover<long double> (p1, p2, c1, c2);}
int RealUniformCrossover      (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return UniformCrossover<long double>      (p1, p2, c1, c2);}
int RealOrderCrossover        (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return OrderCrossover<long double>        (p1, p2, c1, c2);}
int RealCycleCrossover        (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return CycleCrossover<long double>        (p1, p2, c1, c2);}

int RealAlternativeOrderCrossover (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return AlternativeOrderCrossover<long double> (p1, p2, c1, c2);}
int RealAlternativeCycleCrossover (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {return AlternativeCycleCrossover<long double> (p1, p2, c1, c2);}
...
int RealArithmeticCrossover (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, GAGenome* c2) {
  ...
  const GA1DArrayGenome<long double> &mom=
    DYN_CAST(const GA1DArrayGenome<long double> &, p1);
  const GA1DArrayGenome<long double> &dad=
    DYN_CAST(const GA1DArrayGenome<long double> &, p2);
...
int RealBinomialCrossover    (const GAGenome& p1, const GAGenome& p2, GAGenome* c1, const long double prob) {return BinomialCrossover<long double>    (p1, p2, c1, prob);}



// Mutators

int RealSwapMutator             (GAGenome& g, long double pmut) {return SwapMutator<long double>             (g, pmut);}
int RealFlipMutator             (GAGenome& g, long double pmut) {return FlipMutator<long double>             (g, pmut);}
int RealRepeatedExchangeMutator (GAGenome& g, long double pmut) {return RepeatedExchangeMutator<long double> (g, pmut);}
int RealSimpleInversionMutator  (GAGenome& g, long double pmut) {return SimpleInversionMutator<long double>  (g, pmut);}

int RealUniformMutator (GAGenome& g, long double pmut) {
...
//    nMut = 0;
//    for(i=length; i>=0; i--){
//      long double gene_value = child.gene(i);
//
//      if(GAFlipCoin(pmut)){
...
//	        long double allele_region = child.alleleset(i).upper() - child.alleleset(i).lower();
//	        do {
...
int RealIntermediateCrossover (const std::vector<GAGenome*> parents, GAGenome* child) {
  ...
  GA1DArrayGenome<long double>& g = DYN_CAST(GA1DArrayGenome<long double>&, *child);

  unsigned ro = parents.size();

  std::vector<GA1DArrayAlleleGenome<long double>*> pars (ro, (GA1DArrayAlleleGenome<long double>*)0);

  for (unsigned i = 0; i < ro; i++)
    pars [i] = DYN_CAST(GA1DArrayAlleleGenome<long double>*, parents[i]);
...
int RealIsotropicMutator (GAGenome& g) {
  ...
  GA1DArrayGenome<long double>& gen = DYN_CAST(GA1DArrayGenome<long double>&, g);

  // Initialization of Tau vars
  unsigned N    = GAEDAConfig::handle()->getProblemSize();
  long double   tau  = 1 / (sqrt(2*sqrt(N)));
...
int RealNonIsotropicMutator (GAGenome& g) {
  ...
  // Initialization of Tau vars
  unsigned N    = GAEDAConfig::handle()->getProblemSize();
  long double   tau0 = 1 / (sqrt(2*N));
  long double   tau  = 1 / (sqrt(2*sqrt(N)));

  // Update mutation rates
  long double F = exp(tau0*normal_random(0,1));
...
void computeDiffInd(GA1DArrayAlleleGenome<long double>& main_ind, GA1DArrayAlleleGenome<long double>& other_ind, long double step_size, /* inout */ unsigned& used_evals) {
...
bool DiffsLS (GAPopulation& pop, unsigned& used_evals, long double& fit_inc_acum, unsigned& improvements, unsigned max_evals) {

  GA1DArrayAlleleGenome<long double>& best_ind = dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*dynamic_cast<MOSGenome&>(pop.best()).getGenome(GAID::RealEncoding));
  GA1DArrayAlleleGenome<long double>* tmp_ind  = dynamic_cast<GA1DArrayAlleleGenome<long double>*>(best_ind.clone());

  long double SRLSDiff = best_ind.SR();
  bool   improved = false;

  for (unsigned i = 0; i < pop.size() && used_evals < max_evals; i++) {

    GA1DArrayAlleleGenome<long double>& pop_ind = dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*dynamic_cast<MOSGenome&>(pop.individual(i)).getGenome(GAID::RealEncoding));
    if (&pop_ind == &best_ind) continue;

    long double step_size = SRLSDiff * GARandomDouble(0.1, 1.0);
...
long double MTS_LS1 (GAGenome& Xk, long double& fitBest, unsigned& evals, long double& fit_inc_acum, unsigned& improvements, long double& SR, unsigned maxIters) {
  ...
  static const long double adjustFailed = GAEDAConfig::handle()->getMTSAdjustFailed();
  static const long double adjustMin    = GAEDAConfig::handle()->getMTSAdjustMin   ();
  static const long double moveLeft     = GAEDAConfig::handle()->getMTSMoveLeft    ();
  static const long double moveRight    = GAEDAConfig::handle()->getMTSMoveRight   ();
  ...
  for (unsigned i = g.lastIterPos(); i < sz; i++) {
    ...
    if (!isgene_allele_min && !ignoreFirstStep) {
      long double new_gene = old_gene - (moveLeft * SR);
    ...
    if (!isgene_allele_min && !ignoreFirstStep && GAGenome::compareScores(new_score, old_score) == GAGenome::EQUAL) {
      ...
      if (isgene_allele_min || ignoreFirstStep || GAGenome::compareScores(new_score, old_score) == GAGenome::WORSE) {
        ...
        if (old_gene != allele_max) {
          long double new_gene = old_gene + (moveRight * SR);
...
long double MTS_LS1_Reduced_Dims (GAGenome& Xk, long double& fitBest, unsigned& evals, long double& fit_inc_acum, unsigned& improvements, unsigned maxIters) {
  static const long double adjustFailed = GAEDAConfig::handle()->getMTSAdjustFailed();
  static const long double adjustMin    = GAEDAConfig::handle()->getMTSAdjustMin   ();
  static const long double moveLeft     = GAEDAConfig::handle()->getMTSMoveLeft    ();
  static const long double moveRight    = GAEDAConfig::handle()->getMTSMoveRight   ();
  ...
  // BEGIN: Cambios Santi
  std::vector<long double> improvements_per_dim(sz);

  std::vector<int> dimensions;
  static const long double searchProb=GAEDAConfig::handle()->getMTSRedSearchProb();
  static const long double minProb   =GAEDAConfig::handle()->getMTSRedMinProb   ();
  ...
  for (unsigned i = 0, dim; i < dimensions.size(); i++) {
    ...
    long double old_gene  = g.gene(dim);
    long double old_score = g.score();
    long double new_score;
...
long double MTS_LS2 (GAGenome& Xk, long double& fitBest, unsigned& evals, long double& fit_inc_acum, unsigned maxIters) {
...
*/
  ...
  for (unsigned i = 0; i < sz; i++) {
    ...
    std::vector<std::pair<unsigned,long double> > old_genes;
    ...
    long double new_score = g.evaluate();
    evals++;
...
long double MTS_LS3 (GAGenome& Xk, long double& fitBest, unsigned& evals, long double& fit_inc_acum, unsigned maxIters) {

  GA1DArrayAlleleGenome<long double>& g = dynamic_cast <GA1DArrayAlleleGenome<long double>&> (Xk);

  // Store a copy of the original individual to restore it if not improvement
  // has been obtaine at the end of the LS
  GA1DArrayAlleleGenome<long double>* X_copy = dynamic_cast <GA1DArrayAlleleGenome<long double>*> (g.clone());

  unsigned sz = g.size();
  long double grade = 0.0;
  long double old_score = g.score();
  long double X_score = old_score;
  ...
  for (unsigned i = 0; i < sz; i++) {
    ...
    long double X1_score = X_score = g.evaluate();
    evals++;
    ...
    // Y1
    g.gene(i, old_gene - 0.1);
    long double Y1_score = X_score = g.evaluate();
    evals++;
    ...
    long double X2_score = X_score = g.evaluate();
    evals++;
    ...
    // Check BONUS2 for X1
    long double D1 = fabs(X_score - X1_score);
    ...
    // Check BONUS2 for Y1
    long double D2 = fabs(X_score - Y1_score);
    ...
    // Check BONUS2 for X2
    long double D3 = fabs(X_score - X2_score);
    if (GAGenome::compareScores(X2_score, X_score) == GAGenome::BETTER) // X2 is better than X
      grade += BONUS2;
    else
      D1 *= -1; // This way we respect the original behavior of the LS, regardless the problem is minimization or maximization

    long double a = GARandomDouble(0.4, 0.5);
    long double b = GARandomDouble(0.1, 0.3);
    long double c = GARandomDouble(0, 1);
...
long double RandomGreedyLS (GAGenome& Xk, long double& fitBest, unsigned& evals, long double& fit_inc_acum, unsigned& improvements, long double& SR, unsigned maxIters) {
  ...
  while (evals < maxIters) {
    ...
    long double old_value, new_value, old_score;

    int pos = GARandomInt(0, numdims-1);

    long double min_value = g.alleleset(pos).lower();
    long double max_value = g.alleleset(pos).upper();

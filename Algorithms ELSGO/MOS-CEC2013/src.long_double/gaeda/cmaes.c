...
*/
/* --- Changes : --- 
  03/03/21: argument const long double *rgFunVal of
...
long double random_Gauss(random_t *); /* (0,1)-normally distributed */
long double random_Uniform(random_t *);
long   random_Start(random_t *, long unsigned seed /* 0==1 */);

void   timings_start(timings_t *timing); /* fields totaltime and tictoctime */
long double timings_update(timings_t *timing);
void   timings_tic(timings_t *timing);
long double timings_toc(timings_t *timing);

void readpara_init (readpara_t *, int dim, int seed,  const long double * xstart, 
		    const long double * sigma, int lambda, const char * filename);
...
void cmaes_output_dir(cmaes_t * t, const char * dir) {
  int len = strlen(dir);
  output_dir = malloc((len+1)*sizeof(char));
  strncpy(output_dir, dir, len);
  output_dir[len] = '\0';
}
...
static void Eigen( int N,  long double **C, long double *diag, long double **Q, 
		   long double *rgtmp);
static int  Check_Eigen( int N,  long double **C, long double *diag, long double **Q);
static void QLalgo2 (int n, long double *d, long double *e, long double **V); 
static void Householder2(int n, long double **V, long double *d, long double *e); 
...
static void ERRORMESSAGE(char const *sz1, char const *s2, 
			 char const *s3, char const *s4);
static void   Sorted_index( const long double *rgFunVal, int *index, int n);
static int    SignOfDiff( const void *d1, const void * d2);
static long double rgdouMax( const long double *rgd, int len);
static long double rgdouMin( const long double *rgd, int len);
static long double douMax( long double d1, long double d2);
static long double douMin( long double d1, long double d2);
static int    intMin( int i, int j);
static int    MaxIdx( const long double *rgd, int len);
static int    MinIdx( const long double *rgd, int len);
static long double myhypot(long double a, long double b);
static long double * new_double( int n);
...
cmaes_init(cmaes_t *t, /* "this" */
		int dimension, 
		long double *inxstart,
	        long double *inrgstddev, /* initial stds */
		long int inseed,
		int lambda, 
		const char *input_parameter_filename) 
{
  int i, j, N;
  long double dtest, trace;

  t->version = "3.02.03.beta";
  ...
  t->chiN = sqrt((long double) N) * (1. - 1./(4.*N) + 1./(21.*N*N));
  ...
  t->rgD = new_double(N);
  t->C = (long double**)new_void(N, sizeof(long double*));
  t->B = (long double**)new_void(N, sizeof(long double*));
  ...
  t->arFuncValueHist[0] = (long double)(10+(int)ceil(3.*10.*N/t->sp.lambda));
  t->arFuncValueHist++; 
  ...
  t->rgrgx = (long double **)new_void(t->sp.lambda, sizeof(long double*));
  for (i = 0; i < t->sp.lambda; ++i) {
...
cmaes_resume_distribution(cmaes_t *t, char *filename)
{
  ...
  char full_name [1000];
  strncpy(full_name, filename, strlen(filename)+1);
...
/* --------------------------------------------------------- */
long double const * 
cmaes_SetMean(cmaes_t *t, const long double *xmean)
/*
...
cmaes_SamplePopulation(cmaes_t *t)
{
  ...
  long double sum;
  long double const *xmean = t->rgxmean; 
...
/* --------------------------------------------------------- */
long double const * 
cmaes_ReSampleSingle_old( cmaes_t *t, long double *rgx)
{
  int i, j, N=t->sp.N;
  long double sum; 

  if (rgx == NULL)
    FATAL("cmaes_ReSampleSingle(): Missing input long double *x",0,0,0);
...
/* --------------------------------------------------------- */
long double * 
cmaes_SampleSingleInto( cmaes_t *t, long double *rgx)
{
  int i, j, N=t->sp.N;
  long double sum; 
...
/* --------------------------------------------------------- */
long double *
cmaes_UpdateDistribution( cmaes_t *t, const long double *rgFunVal)
{
  ...
  */
  ...
  if(psxps/N > 1.5 + 10.*sqrt(2./N) 
    ...
    long double tfac = sqrt((1 + douMax(0, log(psxps/N))) * N / psxps);
    for (i=0; i<N; ++i) 
      t->rgps[i] *= tfac;
    psxps *= tfac*tfac; 
  }
...
Adapt_C2(cmaes_t *t, int hsig)
{
  ...
  if (t->sp.ccov != 0. && t->flgIniphase == 0) {
    ...
    long double ccovmu = t->sp.ccov * (1-1./t->sp.mucov); 
    long double sigmasquare = t->sigma * t->sigma; 
...
{ 
  ...
  char full_name [1000];
  strncpy(full_name, output_dir, strlen(output_dir)+1);
  strncat(full_name, "/", 1);
  strncat(full_name, name, strlen(name));

  fp = fopen( full_name, appendwrite); 
...
{ 
  ...
  while (key < keyend)
      ...
      if (strncmp(key, "diag(D)", 7) == 0)
	{
	  for (i = 0; i < N; ++i)
	    t->rgdTmp[i] = t->rgD[i];
	  qsort(t->rgdTmp, (unsigned) N, sizeof(long double), &SignOfDiff); /* superfluous */
	  for (i = 0; i < N; ++i)
	...
	{ 
	  ...
	  for (i = 0; i < N; ++i)
	    t->rgdTmp[i] = t->rgD[i];
	  qsort(t->rgdTmp, (unsigned) N, sizeof(long double), &SignOfDiff);
...
/* --------------------------------------------------------- */
long double * 
cmaes_GetInto( cmaes_t *t, char const *s, long double *res)
{
  int i, N = t->sp.N;
  long double const * res0 = cmaes_GetPtr(t, s);
  if (res == NULL)
    res = new_double(N);
  for (i = 0; i < N; ++i)
    res[i] = res0[i];
...
void cmaes_ReadSignals(cmaes_t *t, char const *filename)
{
  ...
  strncpy(full_name, output_dir, strlen(output_dir)+1);
  strncat(full_name, "/", 1);
  strncat(full_name, filename, strlen(filename));
...
*/
{
  ...
  int flgprinted = 0;
  int flgwritten = 0; 
  long double deltaprinttime = time(NULL)-t->printtime; /* using clock instead might not be a good */
  long double deltawritetime = time(NULL)-t->writetime; /* idea as disc time is not CPU time? */
  long double deltaprinttimefirst = t->firstprinttime ? time(NULL)-t->firstprinttime : 0; /* time is in seconds!? */
  long double deltawritetimefirst = t->firstwritetime ? time(NULL)-t->firstwritetime : 0; 
...
/* ========================================================= */
static int
Check_Eigen( int N,  long double **C, long double *diag, long double **Q) 
/* 
...
/* ========================================================= */
static void 
Eigen( int N,  long double **C, long double *diag, long double **Q, long double *rgtmp)
/* 
 ...
 */
{
  ...
  if (rgtmp == NULL) /* was OK in former versions */
    FATAL("cmaes_t:Eigen(): input parameter long double *rgtmp must be non-NULL", 0,0,0);
...
/* ========================================================= */
static void 
QLalgo2 (int n, long double *d, long double *e, long double **V) {
  /*
    -> n     : Dimension. 
  ...
  */
  ...
  long double eps = 2.22e-16; /* Math.pow(2.0,-52.0);  == 2.22e-16 */
...
/* ========================================================= */
static void 
Householder2(int n, long double **V, long double *d, long double *e) {
  /* 
 ...
 */
  ...
  diffc = (long double)(t->lastclock - lc) / CLOCKS_PER_SEC; /* is presumably in [-21??, 21??] */
...
/* --------------------------------------------------------- */
long double random_Gauss(random_t *t)
{
  long double x1, x2, rquad, fac;
...
/* --------------------------------------------------------- */
long double random_Uniform( random_t *t)
{
  long tmp;
  ...
  return (long double)(t->aktrand)/(2.147483647e9);
}
...
readpara_init (readpara_t *t,
	       int dim, 
	       int inseed, 
	       const long double * inxstart, 
	       const long double * inrgsigma,
	       int lambda, 
	       const char * filename)
{
  int i, N;
  ...
  t->rgp2adr = (long double ***) new_void(11, sizeof(long double **));
		     ...
		     const char *filenamesource)
{
  ...
  strncpy(full_name, output_dir, strlen(output_dir)+1);
  strncat(full_name, "/", 1);
  strncat(full_name, filenamedest, strlen(filenamedest));
  ...
  for (ipara=1; ipara < t->n1outpara; ++ipara) {
	...
	fprintf(fp, " ");
	fprintf(fp, t->rgsformat[ipara]+5, *(long double *)t->rgpadr[ipara]);
      } else
	fprintf(fp, t->rgsformat[ipara], *(long double *)t->rgpadr[ipara]);
    }
    fprintf(fp, "\n");
  } /* for */
  fprintf(fp, "\n");
...
readpara_SupplementDefaults(readpara_t *t)
{
  ...
  if (t->lambda < 2)
    t->lambda = 4+(int)(3*log((long double)N));
  if (t->mu == -1) {
    t->mu = t->lambda/2; 
  ...
  if (t->stopMaxIter == -1)
    t->stopMaxIter = ceil((long double)(t->stopMaxFunEvals / t->lambda)); 
  ...
  t->damps = t->damps 
    ...
    * douMax(0.3, 1. -                                       /* modify for short runs */
                  (long double)N / (1e-6+douMin(t->stopMaxIter, t->stopMaxFunEvals/t->lambda))) 
    + t->cs;                                                 /* minor increment */

  if (t->updateCmode.modulo < 1)
    t->updateCmode.modulo = 1./t->ccov/(long double)(N)/10.;
...
static long double
douMax( long double i, long double j)
{
  return i > j ? i : j;
}
static long double
douMin( long double i, long double j)
{
  return i < j ? i : j;
}
static long double
rgdouMax( const long double *rgd, int len)
{
  int i;
  long double max = rgd[0];
  for (i = 1; i < len; ++i)
...
static long double
rgdouMin( const long double *rgd, int len)
{
  int i;
  long double min = rgd[0];
  for (i = 1; i < len; ++i)
...
static int    
MaxIdx( const long double *rgd, int len)
{
  int i, res;
...
static int    
MinIdx( const long double *rgd, int len)
{
  int i, res;
  for(i=1, res=0; i<len; ++i)
    if(rgd[i] < rgd[res])
...
static long double 
myhypot(long double a, long double b) 
...
static int SignOfDiff(const void *d1, const void * d2) 
{ 
  return *((long double *) d1) > *((long double *) d2) ? 1 : -1; 
} 
...
{
  static char s[170];
  long double *p = (long double *) calloc((unsigned) n, sizeof(long double));
  if (p == NULL) {
    sprintf(s, "new_double(): calloc(%ld,%ld) failed",
	    (long)n,(long)sizeof(long double));
    FATAL(s,0,0,0);
  }
  return p;
}
		   ...
		   char const *s3, char const *s4)
{
  ...
  */
  ...
  strncpy(full_name, output_dir, strlen(output_dir)+1);
  strncat(full_name, "/", 1);
  strncat(full_name, "errcmaes.err", strlen("errcmaes.err"));

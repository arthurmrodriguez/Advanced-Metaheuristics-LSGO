...
// Must do a special case for long double/float.  Any floats that get passed to this
// routine will be cast to long doubles, but then we need to force them back to a
...
int
GAParameterList::set(const char* name, long double v) {
  int found = 0;
  for(unsigned int i=0; i<n; i++){
    if(strcmp(name, p[i]->fullname()) == 0 ||
       strcmp(name, p[i]->shrtname()) == 0){
...
GAParameterList::get(const char* name, void* value) const {
  ...
  for(unsigned int i=0; i<n; i++){
	...
	*((long double*)value) = *((long double*)p[i]->value());
	break;

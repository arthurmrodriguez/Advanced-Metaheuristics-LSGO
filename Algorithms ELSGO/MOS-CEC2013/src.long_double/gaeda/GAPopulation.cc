...
void
GAPopulation::DefaultInitializer(GAPopulation & p, long double perc){
...
void GAPopulation::PercInitializer(GAPopulation & p, long double perc){
...
GAPopulation::copy(const GAPopulation & arg)
{
	...
	if(arg.indDiv) {
		indDiv = new long double[N*N];
		memcpy(indDiv, arg.indDiv, (N*N*sizeof(long double)));
	}
	else {
		indDiv = 0;
	}
...
GAPopulation::grow(unsigned int s) {
	...
	if(indDiv) {
		long double *tmpd = indDiv;
		indDiv = new long double[N*N];
		for(int i=0; i<oldsize; i++)
			memcpy(&(indDiv[i*N]), &(tmpd[i*oldsize]), oldsize*sizeof(long double));
		delete [] tmpd;
	}
...
GAPopulation::compact()
{
	...
	//    indDiv = new long double [n*n];
	//    for(unsigned int i=0; i<n; i++)
	//      memcpy(&(indDiv[i*n]), &(tmpd[i*N]), n*sizeof(long double));
	//    delete [] tmpd;
	//  }
...
GAPopulation::statistics(GABoolean flag) const {
	...
	if(n > 0) {
		...
		unsigned int i;
		for(i=1; i<n; i++){
			long double scr = rind[i]->score();
            tmpsum += scr;
            This->rawMax = GAMax(rawMax, scr);
            This->rawMin = GAMin(rawMin, scr);
		}
		long double tmpave = tmpsum / n;
		This->rawAve = tmpave;
		This->rawSum = tmpsum;	// if scores are huge we'll lose data here

		long double tmpvar = 0.0;
		if(n > 1){
			for(i=0; i<n; i++){
				long double s = rind[i]->score() - This->rawAve;
				s *= s;
				tmpvar += s;
			}
			tmpvar /= (n-1);
		}
		This->rawDev = (long double)sqrt(tmpvar);
...
GAPopulation::scale(GABoolean flag) const {
	...
	if(n > 0) {
		...
		if(n > 1){
			for(i=0; i<n; i++){
				long double s = sind[i]->fitness() - This->fitAve;
				s *= s;
				tmpvar += s;
			}
			tmpvar /= (n-1);
		}
		This->fitDev = (long double)sqrt(tmpvar);
...
GAPopulation::diversity(GABoolean flag) const {
	...
	if(n > 1) {
		if(This->indDiv == 0) This->indDiv = new long double[N*N];
...
long double GAPopulation::SumOfDistanceToPop(unsigned ind_pos) const {
...
/*
bool goesFirstBeforeSecond(const GAPopulation::SortOrder sort_order,long double v1, long double v2){
...
GAPopulation::QuickSortAscendingRaw(GAGenome **c, int l, int r) {
	int i,j; long double v; GAGenome *t;
	if(r > l){
...
void GAPopulation::quickSortPopRec(const SortOrder sort_order, GAGenome** all_inds, int left_pos, int right_pos){
  int        i,j;
  GAGenome*  tmp_ind;
  long double     pivot_value;
  long double     pivot_age;
...
GAGenome& GAPopulation::medoid() const {
  ...
  long double    medoid_distance = SumOfDistanceToPop(0);
  long double    tmp_distance    = 0.0;
...
long double GAPopulation::entropy() const{
  ...
  long double tmp_num_ones, tmp_num_zeros, entropy = 0.0;
...
long double GAPopulation::distAvgPoint() const{
  ...
  GA1DArrayGenome<long double>* gen = dynamic_cast<GA1DArrayGenome<long double>* >(& best());
  unsigned num_genes = gen->length();

  unsigned pop_size  = size();

  long double sum, sqr_sum, avg, var, devs_sum, ind_gene;
  ...
  for (unsigned gen_pos=0; gen_pos<num_genes; gen_pos++){
    ...
    for (unsigned ind_pos=0; ind_pos<pop_size; ind_pos++){
      ...
      if      ( gen_binary_tmp ) ind_gene = gen_binary_tmp->bit(gen_pos);
        ...
        GA1DArrayGenome<long double>* tmp_ind = dynamic_cast<GA1DArrayGenome<long double>* >(& individual(ind_pos) );
...
long double GAPopulation::grefenstetteBias() const {
...
long double GAPopulation::nativePrcnt(unsigned rank) const {
...
long double GAPopulation::avgGenDiv() const {
    diversity();                            // Computes (if not already) the distance between the individuals
      long double* gen_dists = (const_cast<GAPopulation* > (this))->genDistsMatrix();

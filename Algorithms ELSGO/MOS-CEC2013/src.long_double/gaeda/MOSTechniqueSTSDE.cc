...
MOSTechniqueSTSDE::MOSTechniqueSTSDE(techIdType id, std::string description, GAGenome::Initializer init,
  ...
  const GA1DArrayAlleleGenome<long double>* ind = dynamic_cast <const GA1DArrayAlleleGenome<long double>* > (genomeBase); assert(ind != NULL);

  min_range_ = new long double[ind->length()];
  max_range_ = new long double[ind->length()];
...
void MOSTechniqueSTSDE::updateRanges(GAPopulation* pop) {
  ...
  GA1DArrayAlleleGenome<long double> *tmp_ind = dynamic_cast<GA1DArrayAlleleGenome<long double>*>(tmp_indM.getGenome(_encoding));
  assert(tmp_ind != NULL);
  int num_dims = tmp_ind->length();

  for (int dim_pos=0; dim_pos<num_dims; dim_pos++) {

    GA1DArrayAlleleGenome<long double>& first_ind = dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*(dynamic_cast<MOSGenome&>(pop->individual(0)).getGenome(_encoding)));
    min_range_[dim_pos] = max_range_[dim_pos] = first_ind.gene(dim_pos);

    for (int ind_pos=1; ind_pos<pop->size(); ind_pos++) {
      GA1DArrayAlleleGenome<long double>& ind_i = dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*(dynamic_cast<MOSGenome&>(pop->individual(ind_pos)).getGenome(_encoding)));
...
void MOSTechniqueSTSDE::genSTSPop(GAPopulation* pop, GAPopulation* stsPop) {
  ...
  long double k = GARandomDouble(0.0,1.0);
  ...
  for (int ind_pos=0; ind_pos<pop->size(); ind_pos++) {
    ...
    GA1DArrayAlleleGenome<long double>& ind     = dynamic_cast<GA1DArrayAlleleGenome<long double>&>(    *indM.getGenome(_encoding));
    GA1DArrayAlleleGenome<long double>& sts_ind = dynamic_cast<GA1DArrayAlleleGenome<long double>&>(*sts_indM.getGenome(_encoding));

...
GAParameterList& GAIncrementalGA::registerDefaultParameters (GAParameterList& p) {
   ...
   p.add (gaNnOffspring, gaSNnOffspring, GAParameter::INT,
          &DefaultParameters::nOffsprings);
...
GAIncrementalGA::GAIncrementalGA (const GAGenome& c) : GAGeneticAlgorithm (c) {
   ...
   noffspr = DefaultParameters::nOffsprings;
...
GAIncrementalGA::GAIncrementalGA (const GAIncrementalGA& ga) : GAGeneticAlgorithm (ga) {
...
GAIncrementalGA& GAIncrementalGA::operator =(const GAIncrementalGA& ga) {
...
void GAIncrementalGA::copy (const GAGeneticAlgorithm& g) {
...
void GAIncrementalGA::objectiveData (const GAEvalData& v) {
...
GAIncrementalGA::ReplacementScheme GAIncrementalGA::replacement (GAIncrementalGA::ReplacementScheme   n,
...
int GAIncrementalGA::nOffspring (unsigned int value) {
...
void GAIncrementalGA::initialize () {
   pop->initialize ();
   pop->evaluate (gaTrue);

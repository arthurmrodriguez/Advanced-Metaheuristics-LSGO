...
class GAGaussianNetwork : public GAGraphModel {
   ...
   private:
      ...
      long double rejectionRegionBoundary ();
      void learnEE ();

      long double logGamma (long double num);
      long double logC (int l, int alpha);

      long double BGe (CSet& nodes);
      long double BGe (int node);


      // functions for the BIC score
      long double BIC (int node, CSet& nodes);
      long double BIC (int node);
      ...
      long double entropy (int node, int parent);

      // The empirical entropy of node.
      long double entropy (int node);

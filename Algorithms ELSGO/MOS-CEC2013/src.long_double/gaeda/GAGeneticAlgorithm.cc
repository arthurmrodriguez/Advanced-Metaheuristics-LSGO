...
GABoolean GAGeneticAlgorithm::TerminateUponPopConvergenceDiscrete (GAGeneticAlgorithm & ga) {
   ...
   long double best = pop.best().score();
   unsigned nBest = 0;
   ...
   if (((long double) nBest / (long double) pop.size()) >= ga.pConvergence())
     val = gaTrue;

   std::cout << "Converged " << (long double) nBest / (long double) pop.size() << "%, over threshold " << ga.pConvergence() << "%" << std::endl;

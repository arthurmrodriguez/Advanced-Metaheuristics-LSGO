...
void GenealogyLogStat::computeValues(long double &max, long double &min, long double &avg, long double &dev, long double &imperc, long double &immax, long double &immin, long double &imavg, long double &imdev, long double &fac){
  ...
  if(GAGenealogy::handle()->isGenealogyMemory())
	...
	long double tmpfitage, tmpage, tmpdev, tmpimm = 0.0, tmpmax = 0.0, tmpmin = 30000;
	long double tmpvar, tmpimax = 0.0, tmpimin = 30000;
	...
	avg = tmpvar/(long double) pop.size();
	max = tmpmax;
	min = tmpmin;
	if(nimm)
	  {
	    imperc = (long double) nimm/(long double) pop.size();
	    imavg = tmpimm/(long double) nimm;
	    immax = tmpimax;
	    immin = tmpimin;
	  }
	else
	...
	for(tmpfitage=tmpvar=tmpdev=0.0, i=0; i<(int)pop.size(); i++)
	  {
	    ...
	    tmpvar += pow((long double) (tmpage) - avg, 2);
	    if(nimm)
	      if(pop.individual(i).getIsland() != island)
		tmpdev += pow((long double) (tmpage) - imavg, 2);

	    tmpfitage += (pop.individual(i).fitness() - pop.fitave())*(tmpage - avg);
	  }

	dev = sqrt(tmpvar/((long double) pop.size()-1));
	if(nimm && tmpdev != 0.0)
	  imdev = sqrt(tmpdev/((long double) nimm-1));
	else
	  imdev = 0.0;

	if(pop.fitdev() != 0.0 && dev != 0.0)
	  fac = (tmpfitage/(long double) pop.size())/(pop.fitdev()*dev);
	else
	  fac = 0.0;
      }
}
...
void GenealogyLogStat::update(){
  long double max,min,avg,dev,ipr,ima,imi,iav,ide,fac;

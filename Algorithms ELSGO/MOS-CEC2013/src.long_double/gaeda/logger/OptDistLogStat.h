...
class OptDistLogStat : public CollectionLogStat {
  ...
  long double (*dist_func_)(GAGenome&);

  static long double normalOptDistFunc (GAGenome& gen);

public:
	OptDistLogStat(const Algorithm& alg,GAGenome* opt_gen, long double (*dist_func)(GAGenome&) );
  ~OptDistLogStat();

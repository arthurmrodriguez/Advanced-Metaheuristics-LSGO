...
static int    k     = 4;
static int    mstar = 4;
static long double u     = 0.3;
static long double ustar = 1.0;
static long double v     = 0.02;
...
static int     nblocks;
static long double* part_scores;
...
extern "C" long double objective (GAGenome& g) {
   ...
   for (level = 0; level <= k; level++) {
      ...
      if (complete_block_sets > 0)
         fitness += ustar + u * (long double) (complete_block_sets - 1);
...
void initEvaluator (void) {
   ...
   part_scores = (long double*) malloc ((b + 1) * sizeof (long double));
   ...
   for (int i = 0; i <= mstar; i++)
      part_scores [i] = (long double) i * v;

   for (int i = mstar + 1; i < b; i++)
      part_scores [i] = (long double) (i - mstar) * -v;

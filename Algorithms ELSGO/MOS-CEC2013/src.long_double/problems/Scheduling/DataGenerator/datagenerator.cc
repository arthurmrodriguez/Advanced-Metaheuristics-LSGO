 ...
 */
long double weekday [] = {0.25, 0.125, 2.0, 1.0, 4.0, 0.5};

/**
 * @var weekend
 * @brief Frequency of jobs worked on weekend days divided in intervals.
 */
long double weekend [] = {0.0625, 0.03125, 0.25, 0.25, 0.25, 0.125};
 ...
 */
long double poisson_prob (long double avg, long double n) {
 ...
 */
unsigned day (long double timestamp) {
 ...
 */
unsigned number_of_jobs (long double act_time) {

   // Determine current time slot
   int index = ((int) (act_time - (long double) ((((unsigned) act_time) / 24) * 24.0))) / INTERVAL_LEN;
   ...
   long double j = rand () / (RAND_MAX + 1.0);
...
TJob generate_job (void) {
   ...
   long double prob_ms, prob_mn, prob_mb;
   long double prob_ls, prob_ln, prob_lb;
...
void generate_dataset_new (char* filename, char* fileclasses) {
   ...
   long double total_proc_time = 0.0;

...
long double localSearch (GA1DArrayAlleleGenome<int>& g, long double old_score) {

   long double score = old_score;
   unsigned sz = g.size ();
   ...
   for (unsigned i = 0; i < lsiters; i++) {
      ...
      long double  sche_time = simulator->simulate (job_order);
      long double  penalty   = simulator->get_total_penalty ();

      long double  new_score = (proc_time) / ((sche_time + penalty) * cpus);
...
extern "C" long double objectiveInt (GAGenome& g) {
   ...
   long double  score = (proc_time) / ((sche_time + penalty) * cpus);
...
extern "C" GAGenome* defineProblem () {
   ...
   GA1DArrayAlleleGenome<long double>* genomeReal = new GA1DArrayAlleleGenome<long double> (nJobs, real_alleles, objectiveReal);

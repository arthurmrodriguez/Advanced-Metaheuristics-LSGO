...
// Distance functions
long double EuclideanDistance (const long double* huesped, const long double* vecino, unsigned size, const int* genes) {
...
long double distance (const long double* huesped, const long double* vecino, unsigned size, const int* genes) {
...
bool knn (const vector<int>& learn, const vector<int>& test, unsigned learnSize, unsigned testSize, const int* genes,
   ...
   vector<long double> real      (testSize, 0.0);
   vector<long double> predicted (testSize, 0.0);
...
extern "C" long double objective (GAGenome& g) {
   ...
   // No resustitution
   std::vector<long double> ll_nores (iters, 0), auc_nores (iters, 0), bs_nores (iters, 0), acc_nores (iters, 0);
   ...
   case AUC:
     ...
     break;
   case AUC_PIV:
     res =  auc * (1.0 /  (long double) (abs ((long) nGenes - pivot) + 1));
     break;
   case LL:
     res = 1 / - ll;
     break;
   ...
   //res = (auc + (1.0 - ((long double) nGenes / (long double) genome.length ()))) / 2.0; // AUC
   //res =  auc -  0.5 * ((long double) nGenes / (long double) genome.length ()); // AUC

...
void writeFinalData(ParamStruct PARAMS, LastEvalStruct BestFEval, long double lastWriteEval, LastEvalStruct LastEval, long double Fopt);
void writeBestF(char * dataFile, long double BestFEval, long double Fopt);
void sprintData(FILE* fout, long double evals, long double F, long double bestF, long double Fnoisy, long double bestFnoisy, long double * x, long double Fopt);
void writeDataHeader(char * dataFile, long double Fopt);
...
/* The following are some of the global static variables of the Matlab program */
  long double fTrigger;
  long double evalsTrigger;
  unsigned int idxEvalsTrigger, idxDIMEvalsTrigger;
  int idxFTrigger;

  long double maxFunEvalsFactor = 1e6;
...
ParamStruct DefaultParam = {
    ...
    0, /* unsigned int DIM; */
    1e-8, /* long double precision; AKA deltaFTarget */
...
For each benchmark function, the specific init are inteh function itself (test on isInitDone)
*/
...
*/
long double fgeneric_initialize(ParamStruct PARAMS)
{
    ...
    /************************************************************/
    if (PARAMS.DIM == 0)
      ERROR((char*) "You need to set the dimension of the problem greater than 0");

    if (PARAMS.DIM  > DIM_MAX)
        ERROR((char*) "You need to recompile the program to increase DIM_MAX if you want to run with dimension %d\nPlease also pay attention at the definition of lastEvalInit", PARAMS.DIM);
    ...
    else
    {
        if ( (100 < PARAMS.funcId) && (PARAMS.funcId-101 < handlesNoisyLength) )
            ...
            ERROR((char*) "funcId in PARAMS is %d which is not a valid function identifier", PARAMS.funcId);
    }
    ...
    idxEvalsTrigger = 0;
    evalsTrigger = floor(pow(10.,(long double)idxEvalsTrigger/(long double)nbPtsEvals)); /* = 1 ! */
    ...
    if ( strlen(PARAMS.dataPath) == 0 ) 
        ERROR((char*) "PARAMS.DATAPATH is expected to be a non-empty string. To set DATAPATH to the current working directory, input '.'");
    ...
    */
     ...
     * Don't forget to declare X (long double * X;) and res (TwoDoubles res).*/
    X = (long double*) malloc(DIM * sizeof(long double));
    res = (*actFunc)(X);
    free(X);
...
*/
long double fgeneric_finalize()
{
    if (actFunc == NULL)
        ERROR((char*) "Finalization process of fgeneric is called before the initialization process has occurred.");
...
*/
long double fgeneric_evaluations()
{
    if (actFunc == NULL) 
        WARNING((char*) "fgeneric_evaluations: fgeneric_initialized has not been called.");
    return  LastEval.num;
}
...
*/
long double fgeneric_best()
{
    if (actFunc == NULL) 
        WARNING((char*) "fgeneric_best: fgeneric_initialized has not been called.");
    return BestFEval.F;
}
...
*/
long double fgeneric_ftarget()
{
    if (actFunc == NULL) {
        WARNING((char*) "fgeneric_ftarget: fgeneric_initialized has not been called.");
        return 0.0;
    }
    else
...
*/
long double fgeneric_maxevals(unsigned int DIM)
{
    return (long double)maxFunEvalsFactor * DIM;
}
...
*/
void fgeneric_evaluate_vector(long double * XX, unsigned int howMany, long double * result)
{
    unsigned int i;
    long double * XXtmp=XX;    /* moving vector of design variable */
    long double * resultTmp = result ; /* moving result */
...
*/
long double fgeneric_evaluate(long double * X)
{
    unsigned int i, boolImprovement = 0;
    long double Fvalue, Ftrue;
    long double evalsj;
    FILE * dataFileId;
    FILE * hdataFileId;
    TwoDoubles res;
    ...
    if (actFunc == NULL)
        ERROR((char*) "fgeneric has not been initialized. Please call 'fgeneric_initialize' first.");
    ...
    if ( (LastEval.num+1 >= evalsTrigger) || (Ftrue-Fopt < fTrigger) ) 
    {
        ...
        if (evalsj >= evalsTrigger)
        {
            ...
            while (evalsj >= floor(pow(10., (long double)idxEvalsTrigger/(long double)nbPtsEvals)) )
                idxEvalsTrigger = idxEvalsTrigger + 1;

            while ( evalsj >= DIM * pow(10., idxDIMEvalsTrigger))
                idxDIMEvalsTrigger = idxDIMEvalsTrigger + 1;

            evalsTrigger = fmin(floor(pow(10., (long double)idxEvalsTrigger/(long double)nbPtsEvals)), DIM * pow(10., idxDIMEvalsTrigger));
        }
        ...
        if (Ftrue - Fopt < fTrigger)
        {
            ...
            else
            {
                if (idxFTrigger == INT_MAX)
                    idxFTrigger = ceil(log10(Ftrue-Fopt))*nbPtsF;

                while ( (Ftrue-Fopt) <= pow(10., (long double)idxFTrigger/(long double)nbPtsF) )
                    idxFTrigger = idxFTrigger - 1;

                fTrigger = fmin(fTrigger, pow(10., (long double)idxFTrigger/(long double)nbPtsF));
            }
        }
...
void writeNewIndexEntry(ParamStruct PARAMS)
{
    ...
    if (indexFileId == NULL)
        ERROR((char*) "Could not open %s", PARAMS.indexFile);
...
void addIndexEntry(ParamStruct PARAMS)
{
    ...
    if ( !existFile(PARAMS.indexFile) ) /* will be created when opened */
        ERROR((char*) "Could not find %s", PARAMS.indexFile);

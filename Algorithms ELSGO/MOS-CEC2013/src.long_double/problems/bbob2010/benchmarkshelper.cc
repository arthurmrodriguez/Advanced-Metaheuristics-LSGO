...
static long double * gval;
static long double * gval2;
static long double * gvect;
static long double * uniftmp;
...
void unif(long double* r, int N, int inseed)
{
    ...
    for (i = 39; i >= 0; i--)
    {
        tmp = floor((long double)aktseed/(long double)127773);
    ...
    for (i = 0; i < N; i++)
    {
        ...
        if (aktseed < 0)
            aktseed = aktseed + 2147483647;
        tmp = floor((long double)aktrand / (long double)67108865);
        aktrand = rgrand[tmp];
        rgrand[tmp] = aktseed;
        r[i] = (long double)aktrand/2.147483647e9;
        if (r[i] == 0.)
        {
...
void computeRotation(long double ** B, int seed, int DIM)
{
    long double prod;
...
{
    long double Fval = Ftrue * exp(beta * randn());
    Fval += 1.01 * TOL;
    if (Ftrue < TOL) {
        Fval = Ftrue;
    }
...
{
    long double Fval = pow(myrand(), beta) * Ftrue * fmax(1., pow(1e9/(Ftrue+1e-99), alpha * myrand()));
    Fval += 1.01 * TOL;
    if (Ftrue < TOL) {
        Fval = Ftrue;
    }
...
{
    long double Fval;
    long double tmp = randn()/fabs(randn()+1e-199);
...
{
    long double temp = peaks[*(const int*)a] - peaks[*(const int*)b];
    if (temp > 0)
        return 1;
    else if (temp < 0)
        return -1;
...
void initbenchmarkshelper() {
    gval = (long double*) malloc(sizeof(long double) * 1);
    gval2 = (long double*) malloc(sizeof(long double) * 1);
    gvect = (long double*) malloc(sizeof(long double) * DIM * DIM);
    uniftmp = (long double*) malloc(sizeof(long double) * 2 * DIM * DIM);
    tmpvect = (long double*) malloc(sizeof(long double) * DIM);
    Xopt = (long double*) malloc(sizeof(long double) * DIM);
    return;
}
...
if ( (strlen(fileName) + strlen(dirName)) > 1022 )
   ERROR((char*)"FileName will be too long for %s + %s", dirName, fileName);
...
FILE * bbobOpenFile(char * fileName)
{
    ...
    if (fileId == NULL)
        ERROR((char*)"Could not open %s", fileName);
    return fileId;
}

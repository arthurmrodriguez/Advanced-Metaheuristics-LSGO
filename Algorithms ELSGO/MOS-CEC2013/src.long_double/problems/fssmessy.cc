...
void strSplitDouble (string str, string delim, vector<long double>& results) {
...
// Distance functions
long double EuclideanDistance (const long double* huesped, const long double* vecino, unsigned size) {
...
long double ManhattanDistance (const long double* huesped, const long double* vecino, unsigned size) {
...
long double ChevyshevDistance (const long double* huesped, const long double* vecino, unsigned size) {
...
long double distance (const long double* huesped, const long double* vecino, unsigned size) {
...
long double rand2 (long double min, long double max) {

   long double diff = max - min;
   long double n = (rand() / (RAND_MAX + 1.0));
...
long double ROCArea (vector<long double>& YNew, vector<long double>& Y) {
   ...
   long double indiceMedio = 0.0;

   long double rango [len];


   long double sumY = 0.0, sumYNew = 0.0;
   ...
   if ((sumY    == (long double) Y.size    () || sumY    == 0.0) ||
       (sumYNew == (long double) YNew.size () || sumYNew == 0.0)    ) {
   ...
   while (fE < len - 1) {
      ...
      if (YNew [fE + 1] > YNew [fE]) {
         ...
         if (pE == fE) {
            ...
            for (int i = pE; i <= fE ; i++)
               rango [i]= (long double) (indiceMedio + index) / (long double) (fE - pE + 1);
...
//       return 0;
   ...
   indiceMedio /= (long double) numUnos;

   area = (indiceMedio - ((long double) (numUnos + 1) / 2.0)) / (long double) (len - numUnos);
...
long double linear_interpolation (long double distance) {
...
long double logL (vector<long double>& YNew, vector<long double>& Y) {
...
long double brierScore (vector<long double>& YNew, vector<long double>& Y) {
...
long double accuracy (vector<long double>& YNew, vector<long double>& Y) {
...
// Algoritmo KNN
bool knn (const vector< vector<long double> >& learn, const vector< vector<long double> >& test,
          long double& ll_val, long double& auc_val, long double& bs_val, long double& acc_val             ) {

   unsigned learnSize = learn.size ();
   unsigned testSize  = test.size  ();

   int   keys [learnSize];
   long double dist [learnSize];

   vector<long double> real;
   ...
   for (unsigned i = 0; i < testSize; i++) {
      ...
      long double num_zeros = 0;
      long double num_ones  = 0;
...
extern "C" long double objective (GAGenome& g) {
   ...
   long double ll = 0.0, auc = 0.0, bs = 0.0, acc = 0.0;

   // We copy selected instances by the GA to a tmp vector
   std::vector< std::vector<long double> > tmpData;
   ...
   // Resustitution
   long double ll_res, auc_res, bs_res, acc_res;
   knn (tmpData, tmpData, ll_res, auc_res, bs_res, acc_res);

   // No resustitution
   std::vector<long double> ll_nores, auc_nores, bs_nores, acc_nores;

   for (unsigned i = 0; i < iters; i++) {

      std::vector<int> selection;
      std::vector< std::vector<long double> > learn, test;
   ...
   long double ll_avg = 0.0, auc_avg = 0.0, bs_avg = 0.0, acc_avg = 0.0;
   ...
   case AUC:
     ...
     break;
   case AUC_PIV:
     res =  auc * (1.0 /  (long double) (abs ((long) nGens - pivot) + 1));
     break;
   case LL:
     res = 1 / - ll;
     break;
   ...
   //res = (auc + (1.0 - ((long double) nGens / (long double) genome.length ()))) / 2.0; // AUC
   //res =  auc -  0.5 * ((long double) nGens / (long double) genome.length ()); // AUC
...
//vector<long double> prob_f;

   // NOTE: for the Messy encoding
/*
extern "C" long double destProb (int n) {

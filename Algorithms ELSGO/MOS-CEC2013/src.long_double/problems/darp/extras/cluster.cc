...
{ int i, j;
  ...
  { int loop;
    int mid = (lo + hi)/2;
    long double result = x[mid];
    long double xlo = x[lo];
    long double xhi = x[hi];
    if (xhi<xlo)
    { long double temp = xlo;
      xlo = xhi;
      xhi = temp;
    }
    ...
    { if (j==nl && i==nr)
        ...
        { int k;
          ...
          long double xmin = x[n-1];
          for (k = lo; k <= j; k++) xmax = clustermax(xmax,x[k]);
          for (k = i; k <= hi; k++) xmin = clustermin(xmin,x[k]);
...
static const long double* sortdata = NULL; /* used in the quicksort algorithm */
...
{ const int i1 = *(const int*)a;
  ...
  const long double term1 = sortdata[i1];
  const long double term2 = sortdata[i2];
...
static long double* getrank (int n, long double data[])
...
{ int i;
  long double* rank;
  int* index;
  rank = (long double*) malloc(n*sizeof(long double));
  if (!rank) return NULL;
  ...
  while (i < n)
  { int m;
    long double value = data[index[i]];
    int j = i + 1;
...
static int
makedatamask(int nrows, int ncols, long double*** pdata, int*** pmask)
{ int i;
  long double** data;
  int** mask;
  data = (long double**) malloc(nrows*sizeof(long double*));
  if(!data) return 0;
  ...
  for (i = 0; i < nrows; i++)
  { data[i] = (long double*) malloc(ncols*sizeof(long double));
    if(!data[i]) break;
...
static void
freedatamask(int n, long double** data, int** mask)
{ int i;
  for (i = 0; i < n; i++)
  { free(mask[i]);
    free(data[i]);
...
static
long double find_closest_pair(int n, long double** distmatrix, int* ip, int* jp)
/*
...
{ int i, j;
  long double temp;
  long double distance = distmatrix[1][0];
  *ip = 1;
  *jp = 0;
  for (i = 1; i < n; i++)
  { for (j = 0; j < i; j++)
...
static int svd(int m, int n, long double** u, long double w[], long double** vt)
/*
...
{ int i, j, k, i1, k1, l1, its;
  ...
  long double* rv1 = (long double*) malloc(n*sizeof(long double));
  if (!rv1) return -1;
  if (m >= n)
  { /* Householder reduction to bidiagonal form */
    ...
    { l = i + 1;
      ...
      anorm = clustermax(anorm,fabs(w[i])+fabs(rv1[i]));
    }
    ...
    { if (i < n-1)
      { if (g != 0.0)
        { for (j = l; j < n; j++) vt[i][j] = (u[i][j] / u[i][l]) / g;
          /* long double division avoids possible underflow */
          for (j = l; j < n; j++)
          { s = 0.0;
...
int pca(int nrows, int ncolumns, long double** u, long double** v, long double* w)
/*
Purpose
=======
...
u          (input) long double[nrows][ncolumns]
...
v          (input) long double[n][n], where n = min(nrows, ncolumns)
Not used on input.

w          (input) long double[n], where n = min(nrows, ncolumns)
Not used on input.
...
*/
{
    ...
    long double* temp = (long double*) malloc(ncolumns*sizeof(long double));
    if (!index || !temp)
    {   if (index) free(index);
        if (temp) free(temp);
        return -1;
...
static
long double euclid (int n, long double** data1, long double** data2, int** mask1, int** mask2,
  const long double weight[], int index1, int index2, int transpose)
...
data1  (input) long double array
The data array containing the first vector.

data2  (input) long double array
...
{ long double result = 0.;
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[i][index1] && mask2[i][index2])
      { long double term = data1[i][index1] - data2[i][index2];
        result += weight[i]*term*term;
        tweight += weight[i];
      }
    }
...
static
long double cityblock (int n, long double** data1, long double** data2, int** mask1,
  int** mask2, const long double weight[], int index1, int index2, int transpose)
...
static
long double correlation (int n, long double** data1, long double** data2, int** mask1,
  int** mask2, const long double weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
{ long double result = 0.;
  ...
  { int i;
    for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      { long double term1 = data1[index1][i];
        long double term2 = data2[index2][i];
  ...
  { int i;
    for (i = 0; i < n; i++)
    { if (mask1[i][index1] && mask2[i][index2])
      { long double term1 = data1[i][index1];
        long double term2 = data2[i][index2];
        long double w = weight[i];
        sum1 += w*term1;
        sum2 += w*term2;
        result += w*term1*term2;
...
static
long double acorrelation (int n, long double** data1, long double** data2, int** mask1,
  int** mask2, const long double weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
long double ucorrelation (int n, long double** data1, long double** data2, int** mask1,
  int** mask2, const long double weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
long double uacorrelation (int n, long double** data1, long double** data2, int** mask1,
  int** mask2, const long double weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
static
long double spearman (int n, long double** data1, long double** data2, int** mask1,
  int** mask2, const long double weight[], int index1, int index2, int transpose)
/*
Purpose
=======
...
{ int i;
  ...
  long double* tdata1;
  long double* tdata2;
  tdata1 = (long double*) malloc(n*sizeof(long double));
  if(!tdata1) return 0.0; /* Memory allocation error */
  tdata2 = (long double*) malloc(n*sizeof(long double));
  ...
  for (i = 0; i < m; i++)
  { const long double value1 = rank1[i];
    const long double value2 = rank2[i];
...
static
long double kendall (int n, long double** data1, long double** data2, int** mask1, int** mask2,
  const long double weight[], int index1, int index2, int transpose)
/*
Purpose
=======
    ...
    { if (mask1[index1][i] && mask2[index2][i])
      { for (j = 0; j < i; j++)
        { if (mask1[index1][j] && mask2[index2][j])
          { long double x1 = data1[index1][i];
            long double x2 = data1[index1][j];
            long double y1 = data2[index2][i];
            long double y2 = data2[index2][j];
            if (x1 < x2 && y1 < y2) con++;
            if (x1 > x2 && y1 > y2) con++;
            if (x1 < x2 && y1 > y2) dis++;
            if (x1 > x2 && y1 < y2) dis++;
    ...
    { if (mask1[i][index1] && mask2[i][index2])
      { for (j = 0; j < i; j++)
        { if (mask1[j][index1] && mask2[j][index2])
          { long double x1 = data1[i][index1];
            long double x2 = data1[j][index1];
            long double y1 = data2[i][index2];
            long double y2 = data2[j][index2];
            if (x1 < x2 && y1 < y2) con++;
            if (x1 > x2 && y1 > y2) con++;
            if (x1 < x2 && y1 > y2) dis++;
            if (x1 > x2 && y1 < y2) dis++;
...
static long double(*setmetric(char dist)) 
  (int, long double**, long double**, int**, int**, const long double[], int, int, int)
{ switch(dist)
...
void setClusterFuncsSeed(unsigned int value) { seed = value; }

static long double uniform(void)
/*
Purpose
=======
...
A long double-precison number between 0.0 and 1.0.
============================================================================
*/
{ int z;
  static const int m1 = 2147483563;
  static const int m2 = 2147483399;
  const long double scale = 1.0/m1;

  static int s1 = 0;
  static int s2 = 0;

  if (s1==0 || s2==0) /* initialize */
  { // unsigned int initseed = (unsigned int) time(0); old line changed it so that it uses the seed set by the user
    // srand(initseed);
    srand(seed);
    s1 = rand();
    s2 = rand();
...
static int binomial(int n, long double p)
/*
Purpose
=======
...
{ const long double q = 1 - p;
  ...
  { const long double s = p/q;
    const long double a = (n+1)*s;
    long double r = exp(n*log(q)); /* pow() causes a crash on AIX */
    int x = 0;
    long double u = uniform();
    while(1)
    { if (u < r) return x;
      u-=r;
      x++;
  ...
  { /* Step 0 */
    const long double fm = n*p + p;
    const int m = (int) fm;
    const long double p1 = floor(2.195*sqrt(n*p*q) -4.6*q) + 0.5;
    const long double xm = m + 0.5;
    const long double xl = xm - p1;
    const long double xr = xm + p1;
    const long double c = 0.134 + 20.5/(15.3+m);
    const long double a = (fm-xl)/(fm-xl*p);
    const long double b = (xr-fm)/(xr*q);
    const long double lambdal = a*(1.0+0.5*a);
    const long double lambdar = b*(1.0+0.5*b);
    const long double p2 = p1*(1+2*c);
    const long double p3 = p2 + c/lambdal;
    const long double p4 = p3 + c/lambdar;
    while (1)
    { /* Step 1 */
      int y;
      int k;
      long double u = uniform();
      long double v = uniform();
      u *= p4;
      ...
      { /* Step 5.2 */
        long double rho = (k/(n*p*q))*((k*(k/3.0 + 0.625) + 0.1666666666666)/(n*p*q)+0.5);
        long double t = -k*k/(2*n*p*q);
        long double A = log(v);
        if (A < t-rho) return y;
        ...
        { /* Step 5.3 */
          ...
          long double z = n+1-m;
          long double w = n-y+1;
          long double x2 = x1*x1;
          long double f2 = f1*f1;
          long double z2 = z*z;
          long double w2 = w*w;
...
data       (input) long double[nrows][ncolumns]
...
cdata      (output) long double[nclusters][ncolumns] if transpose==0
                    long double[nrows][nclusters] if transpose==1
...
getclustermedians(int nclusters, int nrows, int ncolumns,
  long double** data, int** mask, int clusterid[], long double** cdata, int** cmask,
  int transpose, long double cache[])
/*
Purpose
=======
...
cache      (input) long double[nrows] if transpose==0
                   long double[ncolumns] if transpose==1
...
{ switch(method)
  { case 'm':
    { const int nelements = (transpose==0) ? nrows : ncolumns;
      long double* cache = (long double*) malloc(nelements*sizeof(long double));
      if (!cache) return 0;
...
distmatrix (input) long double array, ragged
...
errors     (output) long double[nclusters]
...
{ int i, j, k;
  ...
  { long double d = 0.0;
    ...
    { if (i==k || clusterid[k]!=j) continue;
      d += (i < k ? distmatrix.getDist(k,i) : distmatrix.getDist(i,k) );
      if (d > errors[j]) break;
    }
    if (d <= errors[j]) // Changed to <= so that if the error and d have the same value and
    { errors[j] = d;    // this value is the worst, the cluster is assigned (otherwise some
      centroids[j] = i; // clusters could not be assigned
    }
  }
}
...
static int
kmeans(int nclusters, int nrows, int ncolumns, long double** data, int** mask,
  long double weight[], int transpose, int npass, char dist,
  long double** cdata, int** cmask, int clusterid[], long double* error,
...
static int
kmedians(int nclusters, int nrows, int ncolumns, long double** data, int** mask,
  long double weight[], int transpose, int npass, char dist,
  long double** cdata, int** cmask, int clusterid[], long double* error,
  int tclusterid[], int counts[], int mapping[], long double cache[])
{ int i, j, k;
  const int nelements = (transpose==0) ? nrows : ncolumns;
  const int ndata = (transpose==0) ? ncolumns : nrows;
  int ifound = 1;
...
void kcluster (int nclusters, int nrows, int ncolumns,
  long double** data, int** mask, long double weight[], int transpose,
...
void kmedoids (int nclusters, int nelements, DistMatrix& distmatrix,
  ...
  *ifound = -1;
  ...
  errors = (long double*) malloc(nclusters * sizeof(long double));
  if (!errors) {
    free(saved);
    free(centroids);
    return;
  ...
  *error = DBL_MAX;
  ...
  // These have been added to the library to take into account the scenarios where an individual has the same distance
  // to several medoids. Without this modification, that individual could be always be assigned to the first medoid
  int samedistclustdist[nclusters];
  int samedistclustdistlastpos = -1; // Must be initialized later. If it is forget it, this value will raise an Error

  int indspercluster[nclusters]; // Added to register the number of individuals assigned to each cluster
  ...
  do /* Start the loop */ {
    ...
    while (1) {
      ...
      for (i=0; i<nclusters; i++) indspercluster[i] = 0;
      ...
      for (i = 0; i < nelements; i++) {
        ...
        for (icluster = 0; icluster < nclusters; icluster++) {
          ...
          if (i == j) {
            distance = 0.0;
            //tclusterid[i] = icluster; old line. Commented because now the assignment is made a few lines later (2905)
            //with the intention of distributing the individuals in case they have the same distance to the medoids
            samedistclustdist[0] = icluster;
            samedistclustdistlastpos = 1;

            break;
          }
          tdistance = (i > j) ? distmatrix.getDist(i,j) : distmatrix.getDist(j,i);

          // This part has been modified to take into account when an individual has the same distance to several medoids
          if (tdistance < distance) {
            distance = tdistance;
            ...
            samedistclustdist[samedistclustdistlastpos++] = icluster;
          }

        }
        assert(samedistclustdistlastpos > 0);
        assert(samedistclustdistlastpos <= nclusters);

        int sel_pos          = (samedistclustdistlastpos == 1) ? 0 : rand() % (samedistclustdistlastpos);
        int selected_cluster = samedistclustdist[sel_pos];
        tclusterid[i]        = selected_cluster; //assign the individual to a cluster
        assert(selected_cluster >=0 && selected_cluster <nclusters);

        indspercluster[selected_cluster] += 1;

        total += distance;
      }

      if (mode == KMEDSELBESTBALANCED) {
        // New part for computing the degree of balanced of the resulting sets
        int ideal_indspercluster = nelements/nclusters;
        long balance_measure = 0;
        int cluspos;
        for (cluspos=0; cluspos<nclusters; cluspos++) {
          balance_measure += pow( abs(indspercluster[cluspos]-ideal_indspercluster) ,2.0);
        }

         total = balance_measure;
      }
      ...
      if (i == nelements) break; /* Identical solution found; break out of this loop */
    }

    for (i = 0; i < nelements; i++) {
    // if (clusterid[i]!=centroids[tclusterid[i]]) old line
      if (clusterid[i] != tclusterid[i]) { // Modified so that in clusterid we only have numbers from 0..num clusters -1
        ...
        if (total < *error) {
          ...
          *error = total;
          ...
          for (j = 0; j < nelements; j++) {
            //clusterid[j] = centroids[tclusterid[j]]; old line
            clusterid[j] = tclusterid[j]; // Modified so that in clusterid we only have numbers from 0..num clusters -1
          }
        }
        break;
      }
  ...
  // New option added. It uses the centroids computed before but it assigns the individuals sequentially according to their
  // distance to the medoids so that at the end there is a balanced set of individuals per cluster.
  if (mode == KMEDFORCEBALANCE) {
    int    medoid;
    long double distance, tdistance;
    int    mindistpos = -1;
    int    numusedinds = 0;
    bool   usedinds[nelements]; 
    int ind;
    for (ind=0; ind<nelements; ind++) usedinds[ind] = false;


    while (numusedinds < nelements) {

      int icluster;
      for (icluster = 0; icluster < nclusters && numusedinds < nelements; icluster++) {
        medoid   = centroids[icluster];
        distance = DBL_MAX;

        int ind;
        for (ind=0; ind<nelements; ind++) {
          if (medoid == ind || usedinds[ind] ) continue;

          tdistance = (medoid > ind) ? distmatrix.getDist(medoid,ind) : distmatrix.getDist(ind,medoid);
        ...
        clusterid[mindistpos] = icluster;
        usedinds[mindistpos]  = true;
        numusedinds++;
      }
    }
...
long double** distancematrix (int nrows, int ncolumns, long double** data,
  int** mask, long double weights[], char dist, int transpose)
/*
Purpose
=======
...
{ /* First determine the size of the distance matrix */
  ...
  matrix = (long double**) malloc(n*sizeof(long double*));
  ...
  for (i = 1; i < n; i++)
  { matrix[i] = (long double*) malloc(i*sizeof(long double));
...
cutoff    (input) long double
The cutoff to be used to calculate the weights.

exponent  (input) long double
...
{ int i,j;
  ...
  long double* result = (long double*) malloc(nelements*sizeof(long double));
  if (!result) return NULL;
  memset(result, 0, nelements*sizeof(long double));

  for (i = 0; i < nelements; i++)
  { result[i] += 1.0;
    for (j = 0; j < i; j++)
    { const long double distance = metric(ndata, data, data, mask, mask, weights,
                                     i, j, transpose);
      if (distance < cutoff)
      { const long double dweight = exp(exponent*log(1-distance/cutoff));
...
static
Node* pclcluster (int nrows, int ncolumns, long double** data, int** mask,
  long double weight[], long double** distmatrix, char dist, int transpose)
...
weight     (input) long double[ncolumns] if transpose==0;
                   long double[nrows]    if transpose==1
The weights that are used to calculate the distance. The length of this vector
...
{ int i, j;
  ...
  { for (i = 0; i < nelements; i++)
    { memcpy(newdata[i], data[i], ndata*sizeof(long double));
...
{ const Node* node1 = (const Node*)a;
  ...
  const long double term1 = node1->distance;
  const long double term2 = node2->distance;
  if (term1 < term2) return -1;
  if (term1 > term2) return +1;
  return 0;
}
...
static
Node* pslcluster (int nrows, int ncolumns, long double** data, int** mask,
  long double weight[], long double** distmatrix, char dist, int transpose)
...
{ int i, j, k;
  ...
  int* index;
  Node* result;
  temp = (long double*) malloc(nnodes*sizeof(long double));
  if(!temp) return NULL;
...
static Node* pmlcluster (int nelements, long double** distmatrix)
/*
...
{ int j;
  ...
  { int is = 1;
    ...
    /* Fix the distances */
    for (j = 0; j < js; j++)
      distmatrix[js][j] = clustermax(distmatrix[is][j],distmatrix[js][j]);
    for (j = js+1; j < is; j++)
      distmatrix[j][js] = clustermax(distmatrix[is][j],distmatrix[j][js]);
    for (j = is+1; j < n; j++)
      distmatrix[j][js] = clustermax(distmatrix[j][is],distmatrix[j][js]);
...
static Node* palcluster (int nelements, long double** distmatrix)
/*
Purpose
=======
...
weight (input) long double array[n]
The weights that are used to calculate the distance.
...
static
void somworker (int nrows, int ncolumns, long double** data, int** mask,
  const long double weights[], int transpose, int nxgrid, int nygrid,
  long double inittau, long double*** celldata, int niter, char dist)

{ const int nelements = (transpose==0) ? nrows : ncolumns;
  const int ndata = (transpose==0) ? ncolumns : nrows;
  int i, j;
  long double* stddata = (long double*) calloc(nelements,sizeof(long double));
  int** dummymask;
  int ix, iy;
  int* index;
  int iter;
  /* Maximum radius in which nodes are adjusted */
  long double maxradius = sqrt(nxgrid*nxgrid+nygrid*nygrid);
  ...
  { for (iy = 0; iy < nygrid; iy++)
    { long double sum = 0.;
      for (i = 0; i < ndata; i++)
      { long double term = -1.0 + 2.0*uniform();
  ...
  { int ixbest = 0;
    ...
    if (transpose==0)
    { long double closest = metric(ndata,data,celldata[ixbest],
        mask,dummymask,weights,iobject,iybest,transpose);
      long double radius = maxradius * (1. - ((long double)iter)/((long double)niter));
      long double tau = inittau * (1. - ((long double)iter)/((long double)niter));
      ...
      { for (iy = 0; iy < nygrid; iy++)
        { if (sqrt((ix-ixbest)*(ix-ixbest)+(iy-iybest)*(iy-iybest))<radius)
          { long double sum = 0.;
            ...
            for (i = 0; i < ndata; i++)
            { long double term = celldata[ix][iy][i];
              term = term * term;
              sum += term;
            }
            if (sum>0)
    ...
    { long double closest;
      long double** celldatavector = (long double**) malloc(ndata*sizeof(long double*));
      long double radius = maxradius * (1. - ((long double)iter)/((long double)niter));
      long double tau = inittau * (1. - ((long double)iter)/((long double)niter));
...
static
void somassign (int nrows, int ncolumns, long double** data, int** mask,
  const long double weights[], int transpose, int nxgrid, int nygrid,
  long double*** celldata, char dist, int clusterid[][2])
/* Collect clusterids */
{ const int ndata = (transpose==0) ? ncolumns : nrows;
  int i,j;
...
void somcluster (int nrows, int ncolumns, long double** data, int** mask,
  const long double weight[], int transpose, int nxgrid, int nygrid,
  long double inittau, int niter, char dist, long double*** celldata, int clusterid[][2])
/*
...
weights    (input) long double[ncolumns] if transpose==0;
                   long double[nrows]    if transpose==1
The weights that are used to calculate the distance. The length of this vector
...
celldata (output) long double[nxgrid][nygrid][ncolumns] if transpose==0;
                  long double[nxgrid][nygrid][nrows]    if tranpose==1
...
{ const int nobjects = (transpose==0) ? nrows : ncolumns;
  ...
  if (lcelldata==0)
  { celldata = (long double***) malloc(nxgrid*nygrid*ndata*sizeof(long double**));
    for (i = 0; i < nxgrid; i++)
    { celldata[i] = (long double**) malloc(nygrid*ndata*sizeof(long double*));
      for (j = 0; j < nygrid; j++)
        celldata[i][j] = (long double*) malloc(ndata*sizeof(long double));
    }
  }
...
long double clusterdistance (int nrows, int ncolumns, long double** data,
  int** mask, long double weight[], int n1, int n2, int index1[], int index2[],
...
The routine returns the distance in long double precision.
  ...
  { case 'a':
    { /* Find the center */
      ...
      { long double distance;
        ...
        cdata[0] = (long double*) calloc(ncolumns,sizeof(long double));
        cdata[1] = (long double*) calloc(ncolumns,sizeof(long double));
      ...
      { long double distance;
        ...
        long double** cdata = (long double**) malloc(nrows*sizeof(long double*));
        int** cmask = (int**) malloc(nrows*sizeof(int*));
        for (i = 0; i < nrows; i++)
        { count[i] = (int*)    calloc(2,sizeof(int));
          cdata[i] = (long double*) calloc(2,sizeof(long double));
    ...
    { int i, j, k;
      if (transpose==0)
      { long double distance;
        long double* temp = (long double*) malloc(nrows*sizeof(long double));
        long double* cdata[2];
        int* cmask[2];
        for (i = 0; i < 2; i++)
        { cdata[i] = (long double*) malloc(ncolumns*sizeof(long double));
      ...
      { long double distance;
        ...
        int** cmask = (int**) malloc(nrows*sizeof(int*));
        for (i = 0; i < nrows; i++)
        { cdata[i] = (long double*) malloc(2*sizeof(long double));
          cmask[i] = (int*) malloc(2*sizeof(int));
        }
        for (j = 0; j < nrows; j++)
        { int count = 0;

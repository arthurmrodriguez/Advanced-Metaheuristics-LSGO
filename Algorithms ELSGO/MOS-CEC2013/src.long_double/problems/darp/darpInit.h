...
private:
  ...
  static void processRequestsAccordingToDecentrIndex(DistMatrix& dist, std::vector<Request>& requests, long double alpha);
  static long double decentralizationIndex(DistMatrix& dist, std::vector<Request>& requests, int request_pos);
  static long double sumOfDistancesToOtherRequests(DistMatrix& dist, std::vector<Request>& requests, int request_pos);
  static long double sumPickAndDelDistances(DistMatrix& dist, Request& req_orig, Request& req_dest);
  ...
  static long double computeReqRegretValue(DARPGenome& gen, Request& req, /*out*/ int& ins_route);
  ...
  static long double decentr_alpha__;
  static int    maxreqsforregret__;

public:
  static void DARPRegretInsertionInit (GAGenome& g, DistMatrix& dist, VerticesList& verticeslist);

  static void   setInitParameters(long double decen, int maxreqs) { decentr_alpha__ = decen; maxreqsforregret__ = maxreqs; }
  static long double decentrAlpha()        { return decentr_alpha__; }
...
private:
  ...
  static long double computeSlackCostOfInsertingReq(DARPGenome& gen, Request& req, int num_routes,
  ...
  static long double valueOfBestRouteAndPosInsertion(DARPGenome& gen, Vertex& newvert, int num_routes,
                                                DistMatrix& dist, VerticesList& verticelist,
                                               /*out*/ int& best_route, /*out*/ int& best_insert_pos);

  static long double valueOfInsertingVertexAtBestPos(DARPGenome& gen, int route, Vertex& newvert, DistMatrix& dist, VerticesList& verticelist,

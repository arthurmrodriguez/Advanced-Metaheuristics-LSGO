 ...
 ******************************************************************************************/
//long int 
 int long long evaluaRuta(std::vector <OrdenacionPeticiones> &solucion,long double DARP_ALPHA,long double DARP_BETA){
	...
	for(int i=0;i<solucion.size();i++){
		...
		if(solucion[i].getIdPeticion()>0){//subidas
			int long long retrasocuartaSubida=pow((long double)retraso,4);
            // cout<<" retraso cuarta subida"<<retrasocuartaSubida<<"---------";
			retrasoSubida+=retrasocuartaSubida;

		}else{//bajadas
			int long long retrasocuartaBajada=pow((long double)retraso,4);
  ...
  *
  */
  bool insertaPeticionNegativa(PeticionInsercion &candidata,std::vector<Vehiculo> &vehiculos,long double DARP_ALPHA,long double DARP_BETA,
  ...
  /**************************************************************************************************
    *
    */
   void heuristicoInicializacion(std::vector<Vehiculo> &vehiculos,long vehicleCapacity,long maxDelay, long double DARP_ALPHA, long double DARP_BETA, VerticesList& verticeslist, long nVehicles, DistMatrix& distmatrix) {

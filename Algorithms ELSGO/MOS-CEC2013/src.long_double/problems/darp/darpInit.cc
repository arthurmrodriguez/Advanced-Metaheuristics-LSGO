...
int routeMinCostCriterion(VerticesList& vertices,
                          ...
                          DARPGenome&   gen,
                          Request&      req,
                          long double        (*crit)(DistMatrix&, Vertex&, Vertex&, Vertex&, Vertex& ) ) {
  ...
  int best_route = -1;
  long double best_value = numeric_limits<long double>::max();
  ...
  for (int route=0; route<num_routes; route++) {
    ...
    long double route_value = crit(distmatrix, req_pick_vert, req_del_vert, route_pick_vert, route_del_vert);
...
long double OrigOrigCostCriterion(DistMatrix& distmatrix,
...
long double OrigDestCostCriterion(DistMatrix& distmatrix,
                          Vertex&    req_pick_vert,  Vertex& req_del_vert,
                          Vertex&    route_pick_vert, Vertex& route_del_vert) {
  return distmatrix.getDist(req_pick_vert.pos_,route_del_vert.pos_);
}

long double DestOrigCostCriterion(DistMatrix& distmatrix,
                          Vertex&    req_pick_vert,  Vertex& req_del_vert,
                          Vertex&    route_pick_vert, Vertex& route_del_vert) {
  return distmatrix.getDist(req_del_vert.pos_,route_pick_vert.pos_);
}

long double DestDestCostCriterion(DistMatrix& distmatrix,
                          Vertex&    req_pick_vert,  Vertex& req_del_vert,
                          Vertex&    route_pick_vert, Vertex& route_del_vert) {
...
vector< long double (*)(DistMatrix&, Vertex&, Vertex&, Vertex&, Vertex& ) > constructCriteriaList () {
  vector< long double (*)(DistMatrix&, Vertex&, Vertex&, Vertex&, Vertex& ) > criteria;
...
long double scoreForInsertingRequestInRoute(DARPGenome& gen, Request& req, int route, DARPObjFuncBasedInitC::objfuncbasedmethod method,
...
void bestScoreAndRouteForInsertion(DARPGenome& gen, Request& req, DARPObjFuncBasedInitC::objfuncbasedmethod method,
                                   /*out*/ long double &best_score, /*out*/int &best_route, /*out*/ int& pickins, /*out*/ int& delins) {
  best_route  = -1;
  ...
  for (int route=0; route<gen.length(); route++) {
    ...
    long double tmp_score = scoreForInsertingRequestInRoute(gen,req,route,method,tmp_pickins,tmp_delins);
...
//                                                /*out*/ long double &best_score, /*out*/ int &route_swap, /*out*/ int &route_ins ) {
//  route_swap = route_ins = -1;
//  best_score = GAGenome::worstPossibleScore();
//
//  long double tmp_score;
//  int    tmp_route_ins;
//
//  for (int route=0; route<gen.length(); route++) {
...
long double DARPRegretInsertionInitC::sumPickAndDelDistances(DistMatrix& dist, Request& req_orig, Request& req_dest) {
...
long double DARPRegretInsertionInitC::sumOfDistancesToOtherRequests(DistMatrix& dist, std::vector<Request>& requests, int request_pos) {
...
long double DARPRegretInsertionInitC::decentralizationIndex(DistMatrix& dist, std::vector<Request>& requests, int request_pos) {
  long double numerator = sumOfDistancesToOtherRequests(dist,requests,request_pos);

  long double denominator = numerator;
  for (int i=0; i<requests.size(); i++) {
    if (i==request_pos) continue;
 ...
 */
void DARPRegretInsertionInitC::processRequestsAccordingToDecentrIndex(DistMatrix& dist, std::vector<Request>& requests, long double alpha) {
...
bool DARPRegretInsertionInitC::isTravelingFromLateDeliveryGoingToArriveBeforeEarlyPickup(Request& req, Request& other, DistMatrix& dist) {
  long double distance = dist.getDist(req.delivery_vert->pos_,  other.pickup_vert->pos_);
...
void DARPRegretInsertionInitC::computeBestAndAllInsertionScores(DARPGenome& gen, Request& req,
                                     /*out*/ long double& best_score, /*out*/ int& best_ins_route, /*out*/ vector<long double>& ins_scores) {
  ins_scores.clear();
  ...
  for (int route=0; route<gen.length(); route++) {
    long double ins_score  = scoreForInsertingRequestInRoute(gen, req, route, DARPObjFuncBasedInitC::BEST, pickins, delins);
...
long double DARPRegretInsertionInitC::computeReqRegretValue(DARPGenome& gen, Request& req, /*out*/ int& ins_route) {
  vector<long double> ins_scores;
  long double         best_score;

  computeBestAndAllInsertionScores(gen, req, best_score, ins_route, ins_scores);

  long double regret_value = 0;
...
void DARPRegretInsertionInitC::findReqWithHighestRegret(DARPGenome& gen, std::list<Request*>& requests_toins, int maxreqsforregret,
  ...
  long double worst_regret_value = -1;

  int i=0;
  for (list<Request*>::iterator it=requests_toins.begin(); it!=requests_toins.end() && i<maxreqsforregret; it++, i++) {
    Request& req_tmp      = **it;
    int      ins_route_tmp;
    long double   regret_value = computeReqRegretValue(gen,req_tmp,ins_route_tmp);
    assert(regret_value >= 0);
...
long double DARPRegretInsertionInitC::decentr_alpha__ = 0.5;
...
void DARPSlackInitC::constructRoutesAccordingToSlackValue(DARPGenome& gen,std::vector<Request>& requests,
  ...
  cout << "Objective based insertion method calls = " << oldinscalls << " (" << (long double)oldinscalls/num_req_toins << " %)" << endl;
  cout << gen << endl;
}
...
void DARPSlackInitC::findReqWithSmallestSlackCost(DARPGenome& gen, std::list<Request*>& requests_toins, int maxreqs,
  ...
  for (list<Request*>::iterator it=requests_toins.begin(); it!=requests_toins.end() && i<maxreqs; it++, i++) {
    ...
    long double value = computeSlackCostOfInsertingReq(gen, req_tmp, num_routes, dist, verticeslist, feasible_tmp,
 ...
 */
long double DARPSlackInitC::computeSlackCostOfInsertingReq(DARPGenome& gen, Request& req, int num_routes,
...
long double DARPSlackInitC::valueOfBestRouteAndPosInsertion(DARPGenome& gen, Vertex& newvert, int num_routes,
                                                   DistMatrix& dist, VerticesList& verticeslist,
                                                  /*out*/ int& best_route, /*out*/ int& best_insert_pos) {
  best_route        = -1;
  best_insert_pos   = -1;
  long double best_value = -1;

  for (int route = 0; route<num_routes; route++) {
    int    tmp_pos   = -1;
    long double tmp_value = valueOfInsertingVertexAtBestPos(gen,route,newvert,dist,verticeslist,tmp_pos);
    assert(tmp_value >= -1);
 ...
 */
long double DARPSlackInitC::valueOfInsertingVertexAtBestPos(DARPGenome& gen, int route, Vertex& newvert, DistMatrix& dist,
  ...
  best_insert_pos          = -1;
  long double best_insert_value = -1;
  ...
  for (int i=start_pos; i<=end_pos; i++) {
    long double tmp_insert_value = computeSlackCostOfInsertion(gen,route,i,newvert,dist,verticeslist,ctimes);
...
void constructGenomeSkybus(DARPGenome& gen, DistMatrix& distmatrix, VerticesList& verticeslist,
              long vehicleCapacity,long maxDelay, long double DARP_ALPHA, long double DARP_BETA, long nVehicles) {
...
void DARPSkybusInit(GAGenome& g, DistMatrix& dist,VerticesList& verticeslist) {
  ...
  long double DARP_ALPHA = gen.loadVWeight();
  long double DARP_BETA = gen.TWVWeight();

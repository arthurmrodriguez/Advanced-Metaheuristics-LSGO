...
protected:
  ...
  static int selectPos(vector<long double>& probs);

  static string stringOfSeq(list<int>& seq);

  struct RouteAndScore { int route; long double score;};
  ...
  static vector<long double>        getSelectionProbs                (const vector<RouteAndScore>& routescores, GAGenome::OptCriterion optcrit=GAGenome::MAXIMIZATION);
...
protected:
  ...
  static int bestRoutePosForInsertion (const DARPGenome& gen, int orig_route, long double orig_score, const list<int>& vertices);
...
protected:
  ...
  virtual long double scoreOfSwappingSeqs(const DARPGenome& gen, int route1, list<int>& seq1, int route2, list<int>& seq2);
...
protected: 
  ...
  static long double scoreFromSwapNaturalSequences(const DARPGenome&               gen,

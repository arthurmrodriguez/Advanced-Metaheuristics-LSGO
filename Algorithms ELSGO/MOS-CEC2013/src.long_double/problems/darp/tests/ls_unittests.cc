...
extern "C" long double LSDummyObjFunc(GAGenome& g) {
...
TEST_F(LocalSearchTest,LSRestartTest) {
   ...
   */
  {
    ...
    long double start_score = gen->score();          // vertex 2 at position 2 and -2 at position 3
   ...
   */
  {
      ...
      long double start_score = gen->score();                // trying all the insert position for vertex -2. Then, it repeats the same
  ...
  // For this test we check if the ls works with two routes by giving the exact evals to be applied to the remaining two routes
  {
      ...
      long double start_score = gen->score();                        // with the next position: 2 (since 1 corresponds to a delivery vertex)

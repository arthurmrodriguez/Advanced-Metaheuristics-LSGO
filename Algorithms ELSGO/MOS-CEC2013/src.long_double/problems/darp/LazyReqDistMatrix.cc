...
long double LazyReqDistMatrix::getDist(int ori, int dest) const {
...
long double LazyReqDistMatrix::computeValue(int ori, int dest) {
  assert(dest<ori);
  assert(reqs_.size() > 0);
...
long double LazyReqDistMatrix::computedValuesRatio() {
  ...
  return (long double) computedvalues/(long double)allvalues;
}

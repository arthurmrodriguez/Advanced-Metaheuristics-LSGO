...
class DistMatrix {
private:
  std::vector< std::vector<long double> > data_;
...
public:
  ...
  DistMatrix(int size) : data_(size+1, std::vector<long double>(size+1, 0.0)) {}

  DistMatrix(std::string distFile, int size);

  virtual void setDist(int ori, int dest, long double value) {
    assert(data_.size() > 0);
    assert(ori >= 0);
    assert(dest >= 0);

...
protected:
  ...
  virtual long double bestScoreFromAllCombs(Request& req1, Request& req2);
  long double         reqSequenceScore     (Request& req1, Request& req2, int req1d, int req2p, int req2d);
...
public:
  ...
  long double getDistOfReqsPickupIds(const int reqid1, const int reqid2) {

...
const long double MAXSEQSIZERATIO = 0.7;
...
void DARPVNSShaker::getRandomBadGoodRoutes(const DARPGenome& gen, /*out*/ int& badroute, /*out*/ int& goodroute) {
  ...
  vector<long double>        probs       = getSelectionProbs(routescores);
...
\
  long double best_score  = gen.scoreOf##option##ingVertices(route,*(allseqs[best_pos])); \
  long double worst_score = best_score; \
...
vector< list<int>* >* DARPVNSShaker::getAllSequencesOfSize(const DARPGenome& gen, int route, int max_size, bool userandomsize) {
  ...
  max_size = GAMin(allpickupvertices.size(),(int) round((long double)max_size/2.0));  // /2.0 since later we complete it with the siblings
...
int DARPVNSShaker::selectPos(vector<long double>& probs) {
  long double value = GARandomDouble();
...
//  }
//
//  max_size = GAMin(allseqs.size(),round((long double)max_size/2.0));  // /2.0 since later we complete it with the siblings
//
...
vector<long double> DARPVNSShaker::getSelectionProbs (const vector<RouteAndScore>& routescores, GAGenome::OptCriterion optcrit) {
  vector<long double> probs_sum (routescores.size());
  ...
  if (optcrit == GAGenome::MAXIMIZATION) {
    ...
    long double minscore = routescores[0].score;
    long double maxscore = minscore;
 ...
 */
int GreedyMoveNeighborhood::bestRoutePosForInsertion(const DARPGenome& gen, int orig_route, long double orig_score, const list<int>& vertices) {
  // Extra greedy approach: we try all the possible insertions
  int    best_ins_route = -1;
  long double best_score     = GAGenome::worstPossibleScore();
  long double tmp_score;
...
long double GreedySwapNeighborhood::scoreOfSwappingSeqs(const DARPGenome& gen, int route1, list<int>& seq1, int route2, list<int>& seq2) {
  DARPGenome tmp_gen(gen);

  tmp_gen.swapSeqs(route1,seq1,route2,seq2);

  long double score = tmp_gen.score();
...
long double CheckAllNaturalSeqsCombsNeighborhood::scoreFromSwapNaturalSequences(const DARPGenome&               gen,
                                                                           vector< vector< list<int>* > >& allnatseqs,
                                                                           int                             fromroute,
                                                                           int                             from_natseq_pos,
                                                                           int                             toroute,
...
VNSOpAction* CheckAllNaturalSeqsCombsNeighborhood::operator() (GAGenome& g) {
  ...
  for (int fromroute=0; fromroute<allnatseqs.size()-1; fromroute++) {
    ...
    long double best_swap_score = gen.score();

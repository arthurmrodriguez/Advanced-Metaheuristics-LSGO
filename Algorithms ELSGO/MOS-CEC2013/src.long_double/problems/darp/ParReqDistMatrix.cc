...
void ParReqDistMatrix::initializeDistMatrix(vector<Request>& reqs, CommManager& comm) {
  ...
  vector<long double> computed_data = computeCorrespondingVectorData(reqs, comm);
...
//  /*DEBUG*/       long double exp_value = bestScoreFromAllCombs(reqs[i],reqs[j]);
...
inline vector<long double> ParReqDistMatrix::computeCorrespondingVectorData(vector<Request>& reqs, CommManager& comm) {
  ...
  vector<long double> computed_data (vector_nelems+3); // we are also storing the row, column, and number of elements
  int datapos = 0;
...
inline void ParReqDistMatrix::parallelfillOfData(CommManager& comm, vector<long double>& data) {
  if (comm.isIslandMaster()) {
    ...
    for (int i=1;i<=comm.getLastRank(); i++) {
      ...
      vector<long double> received_data = comm.receiveDoubleVectorFromIsland(i,data.size()+comm.getNumIslands());
...
inline void ParReqDistMatrix::fillMatrixWithSerData(vector<long double>& data) {
  int row    = data[0];
  int col    = data[1];

...
static long double maxRideValue;
...
static long double minEvalWeightsUpdateValue;
static long double maxEvalWeightsUpdateValue;

static long double loadVWeight;
static long double TWVWeight;
static long double rideVWeight;

static long double initloadVWeight = -1;
static long double initTWVWeight   = -1;
static long double initrideVWeight = -1;
...
static long double          kmedoidswaitingpenconstant  = 0;
...
bool parse_config (char* fich) {
   ...
   try {
      ...
      if (genType.compare("VNS") == 0) {
        minEvalWeightsUpdateValue = (long double) cfg.lookup("minEvalsWeightUpdateValue");
        maxEvalWeightsUpdateValue = (long double) cfg.lookup("maxEvalsWeightUpdateValue");
      }
      else if (genType.compare("TS")  != 0) {throw runtime_error("Unrecognized Genome Type");}

      maxRideValue    = (long double) cfg.lookup("maxRideValue");
      ...
      loadVWeight = (long double) cfg.lookup("loadVWeight");
      TWVWeight   = (long double) cfg.lookup("TWVWeight");
      rideVWeight = (long double) cfg.lookup("rideVWeight");
      ...
      if (initfunc == DARPRegretInsertionInitC::DARPRegretInsertionInit) {
        long double dencentr_alpha   = (long double) cfg.lookup("decentrAlpha");
        long double maxreqsperinitit = (int)    cfg.lookup("maxRequestsPetInitIt");
        DARPRegretInsertionInitC::setInitParameters(dencentr_alpha,maxreqsperinitit);
      }
      if (initfunc == DARPSlackInitC::DARPSlackInit) {
        long double maxreqsperinitit = (int) cfg.lookup("maxRequestsPerInitIt");
   ...
   if (cfg.exists("initloadVWeight") )           initloadVWeight            = (long double)       cfg.lookup("initloadVWeight");
   if (cfg.exists("initTWVWeight") )             initTWVWeight              = (long double)       cfg.lookup("initTWVWeight");
   if (cfg.exists("initrideVWeight") )           initrideVWeight            = (long double)       cfg.lookup("initrideVWeight");
   ...
   if (kmedoidsusewaitingpen) {
     ...
     kmedoidswaitingpenconstant  = (long double) cfg.lookup("waitingpenconstant");
   }
...
extern "C" void configureAlg (Algorithm& alg) {
  ...
  DARPObjFuncBasedInitC::setAlg(alg);

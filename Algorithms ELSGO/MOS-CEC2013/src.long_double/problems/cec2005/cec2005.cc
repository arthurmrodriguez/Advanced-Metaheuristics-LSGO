...
long double ALLELE_MIN = 0.0;
long double ALLELE_MAX = 0.0;

const long double MIN_F7     =   0.0;
const long double MAX_F7     = 600.0;
const long double MIN_F25    =   2.0;
const long double MAX_F25    =   5.0;

unsigned function = 1;
std::vector<long double> opt_genome;
long double opt_genome_score = 0.0;

GA1DArrayAlleleGenome<long double>* genome_opt = NULL;
...
extern "C" GAGenome* defineProblem () {
   ...
   GAAlleleSet<long double> alleles (ALLELE_MIN, ALLELE_MAX);
   ...
   genome_opt = new GA1DArrayAlleleGenome<long double> (cfg->getProblemSize (), alleles, objective);
...
extern "C" bool postprocess (GAPopulation* pop, int rank, int repetition) {
   ...
   if (rank == 0) {
      long double best_score = (1 / pop->best ().score ()) - 1;

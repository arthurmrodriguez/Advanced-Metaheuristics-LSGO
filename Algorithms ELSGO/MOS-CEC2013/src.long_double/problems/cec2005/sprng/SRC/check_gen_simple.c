...
/* Hack to avoid scanf warnings with gcc 4.3+ */
int res;
...
int check_gen()			/* Check generator with correct parameters   */
{
  ...
  for(i=0; i<50; i++)		/* check float generator                   */
  {
    res=scanf("%d\n", &tempi);
#ifdef USE_MPI
    tempd = (long double) get_rn_flt_simple_mpi();
#else
    tempd = (long double) get_rn_flt_simple();
#endif
    /* printf("%d. %d %f\n", i, tempi, tempd);*/
  ...
  correct = YES;
  for(i=0; i<50; i++)		/* check long double precision generator          */
  {
    res=scanf("%d", &tempi);
    tempd = sprng();

...
/*      lagged-Fibonacci generators to produce integer, float and long double */
...
/*      to get the necessary precision for the long double value.             */
/*                                                                       */
...
/*      FLT_MULT is used in converting to float and long double values; the   */
...
#ifdef __STDC__
static void si_long double(unsigned *a,  unsigned *b, int length)
#else
static void si_long double(a,b, length)
unsigned *a,*b;
int length;
#endif
{
  ...
  if (b[length-2]&(1<<MAX_BIT_INT))
    errprint("WARNING","si_long double",TOOMANY);
...
#ifdef __STDC__
long double get_rn_dbl(int *genptr)
#else
long double get_rn_dbl(genptr)
int *genptr;
#endif
/*      returns value put into new position                              */
{
  ...
  *hp = (--hptr < 0) ? lval-1 : hptr;
        ...
        new_val = ((unsigned int) temp2*(long double)FLT_MULT + (unsigned int) temp1)*FLT_MULT;
        return (new_val);
}

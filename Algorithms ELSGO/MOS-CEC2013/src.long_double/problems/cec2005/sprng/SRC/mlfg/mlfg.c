...
static void si_long double(uint64 *a,  uint64 *b, int length)
{
  int i;
  uint64 mask1, temp1;
  ...
  if (notzero(temp1))
    fprintf(stderr,"WARNING: si_long double -- RNG has branched maximum number of times.\n\t Independence of generators no longer guaranteed\n");
...
/*           contained in nstart array                                   */
  ...
  si_long double(q[0]->si,nstart,length);
  ...
  if (ngen>1) 
	...
	si_long double(nindex,nindex, length);

...
struct rngen
{
  ...
  long double state[3], multiplier[3];/* simulate 64 bit arithmetic */
  int s0, s1, s2, s3, s4;
#endif
};
...
#else
  genptr->multiplier[0] = (long double) (PARAMLIST[param][0]&0x3fffff);
  genptr->multiplier[1] = (long double) 
    (PARAMLIST[param][0]>>22 | (PARAMLIST[param][1]&0xfff)<<10);
  genptr->multiplier[2] = (long double) (PARAMLIST[param][1]>>12);
  genptr->state[0] = (long double) ((INIT_SEED0^gennum)&0x3fffff);
  genptr->state[1] = (long double) 
    ((INIT_SEED0^gennum)>>22 | ((INIT_SEED1 ^ (unsigned)seed<<1)&0xfff)<<10);
  genptr->state[2] = (long double) ((INIT_SEED1 ^ (unsigned)seed<<1)>>12);
#endif  
...
/* Returns a long double precision random number */

#ifdef __STDC__
long double get_rn_dbl(int *igenptr)
#else
long double get_rn_dbl(igenptr)
int *igenptr;
#endif
{
...
#endif
  static long double dtemp[1] = {0.0};
...
#endif
{
  ...
  q->state[0] = (long double) (m[1]&0x3fffff);
  q->state[1] = (long double) ((m[1]>>22) | (m[0]&0xfff)<<10);
  q->state[2] = (long double) (m[0]>>12);
  p += load_intarray(p,2,4,m);
  q->multiplier[0] = (long double) (m[1]&0x3fffff);
  q->multiplier[1] = (long double) ((m[1]>>22) | (m[0]&0xfff)<<10);
  q->multiplier[2] = (long double) (m[0]>>12);

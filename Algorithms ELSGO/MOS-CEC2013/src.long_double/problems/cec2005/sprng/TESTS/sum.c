...
long double test_normal(long double *array, int n);
...
main(int argc, char *argv[])
{
  ...
  long double result, rn, *temparray, *sumarray;
  ...
  temparray = (long double *) mymalloc(n*sizeof(long double));
  sumarray = (long double *) mymalloc(n*sizeof(long double));
...
long double test_normal(long double *array, int n)
{
  long double mean1, sd1, mean2, sd2, result, percent;
  int i;
  long double result_min, result_max, error;
  
  mean2 = NTESTS*(group_size/2.0);
  sd2 = sqrt( (long double) NTESTS*(group_size/12.0) );
  for(i=0; i<n; i++)
  ...
  error = 0.0275/NTESTS/group_size*sqrt((long double) n);
  printf("\tKS range = [%f, %f]%%\n", 100*KSpercent(max(0.0,result-error),n),
	 100*KSpercent(min(sqrt((long double) n),result+error),n) );

...
static long double  *oneGrp;
/* Array of bins */
static long    *bins;
/* Array of corresponding probabilities */
static long double  *probs;
/* Array of chi-squares */
static long double  *chiSqrs;
...
void initTest(argc, argv) int argc; char *argv[]; {
   ...
   oneGrp = mymalloc(sizeof(long double)*grpSize);
   bins = mymalloc(sizeof(long)*numBin);
   probs = mymalloc(sizeof(long double)*numBin);
   chiSqrs = mymalloc(sizeof(long double)*NTESTS);

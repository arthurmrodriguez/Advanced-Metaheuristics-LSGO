...
void main (int argc, char *argv[])
{
  ...
  FFTcoeffs = (long double *) malloc(lda*nstreams*sizeof(long double)); /* FFT coeffs */
  means = (long double *) malloc(lda*nstreams*sizeof(long double)); /* average coeffs */
...
{
  int i, j, count;
  long double StdDev1, StdDev2, mean1, mean2, StdDev, mult, percentile;
  ...
  mean1 = (long double)nstreams*(long double)n/2.0; /* Expected value of constant term*/
  mean2 = 0.0;			/* Expected values of all other terms       */
  StdDev1 = sqrt((long double)nstreams*(long double)n/(long double)nruns/12.0);
  StdDev2 = sqrt((long double)nstreams*(long double)n/(long double)nruns/24.0);
...
void FFTCalc(int nstreams, int nruns, int n) 
{
  ...
  for(k=0; k<nruns; k++)	/* Repeat calculation 'nruns' times         */
  {
    ...
    dscal2d(n,nstreams,1.0/(long double)(n*nstreams),FFTCoeffs,lda);
    for(i=0; i<(lda*nstreams); i++) 
      printf("array[%d] = %g\n", i, FFTcoeffs[i]); 
#endif
  } /*end loop for nruns                                                    */

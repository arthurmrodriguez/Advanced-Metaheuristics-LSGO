...
long double identity ANSI_ARGS((long double d));
void compute_probability ANSI_ARGS((long double *A, int n, int m));
...
#ifdef __STDC__
long double identity(long double d)
#else
long double identity(d)
long double d;
#endif
{
  return d;
...
#endif
{
  ...
  probability = (long double *) mymalloc((n+1)*sizeof(long double));
...
#ifdef __STDC__
void compute_probability(long double *A, int n, int m)
#else
...
#endif
{
  ...
  for(i=0; i<n-1; i++)
  {
    ...
    for(j=j1; j>=j0; j--)
    {
      A[j] = (long double) j/(long double) m *A[j] + 
	( (1.0+1.0/(long double)m) - (long double)j/(long double)m )*A[j-1];
  ...
  if(NTESTS < MIN_CHISQ_NTESTS && sqrt((long double)NTESTS/(long double)(j1-j0+1)) > 0.5)
  {
    fprintf(stderr,"WARNING: Error in KS estimate may be ~ %f\n", 
	    sqrt((long double)NTESTS/(long double)(j1-j0+1)));

...
/* This returns the Kolmogorov-Smirnov 'error' value. V is an array of long doubles, which contains the values of the samples. n is the size of the array. F is the pointer to the cumulative distribution function from which it is expected that the samples are drawn. */
...
long double KSF ANSI_ARGS((long double value));
...
void getKSdata ANSI_ARGS((long double *V, long ntests));
...
void reduce_sum ANSI_ARGS((long double *array,int n));
...
long double get_rn ANSI_ARGS((void));

...
long double prob;
long double Energy[10][10], Cv[10][10], J=0.4406868;
long double expJarray[17], *expJ;
long double exact_energy=-1.4530649029, exact_Cv=1.4987048885; /***** This is correct ONLY for a 16x16 lattice!!****** */
...
{
  int j;
  long double average_energy, average_Cv, energy_error, Cv_error;
...
void metropolis(int block_size, int use_blocks)
{
  ...
  long double average_E, average_Cv;
...
printf("         \tEnergy\t\tEnergy_error\tSigma_Energy\tCv\t\tCv_error\tSigma_Cv\n");
  ...
  for(i=old_row=row=0,divisor=1; i<use_blocks; i++)
  {
    ...
    average_E /= (long double) block_size*nsites; /*compute average of quantities */
    average_Cv /= (long double) block_size; 

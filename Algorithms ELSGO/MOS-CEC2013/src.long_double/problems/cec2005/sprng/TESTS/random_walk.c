...
void random_walk ANSI_ARGS((int walk_length, long double *quadrant));
...
#endif
{
  ...
  long double result, expected, chisum, quadrant[4];
...
#ifdef __STDC__
void random_walk(int walk_length, long double *quadrant)
#else
void random_walk(walk_length, quadrant)
int walk_length;
long double *quadrant;
#endif
{
  int x, y, i, j;
  long double rn;

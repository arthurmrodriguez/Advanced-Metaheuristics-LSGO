...
long double Objective::objectiveFunction(VesicleInfo* info) {
  ...
  // Compute the intensity averages
    edgeAvgIntensity = (  edgePx > 0) ? (long double)  edgeIntensity /   edgePx : 0;
  centerAvgIntensity = (centerPx > 0) ? (long double)centerIntensity / centerPx : 0;

...
std::vector<VesicleGenome<long double>*> found_vesicles;

bool getEdgeInfo(const Vesicle& ves, long double& distAllOUT, long double& distAllIN, long double& distNOUT, long double& distNIN);

long double fitness (VesicleGenome<long double>& genome) {
  long double res = 9999, aux = 9999;
  VesicleInfo info;
...
extern "C" GAGenome* defineProblem () {
  ...
  VesicleGenome<long double>* genome = new VesicleGenome<long double> (alleles, objective);
...
extern "C" bool postprocess (GAPopulation* pop, int rank, int repetition) {
  ...
  VesicleGenome<long double>& genome = dynamic_cast<VesicleGenome<long double>&>(*g.getGenome(GAID::RealEncoding));
  ...
  VesicleGenome<long double>* new_vesicle = dynamic_cast<VesicleGenome<long double>*>(genome.clone());
  ...
  if (repetition == GAEDAConfig::handle()->getRepetitions() - 1) {
    ...
    for (unsigned i = 0; i < found_vesicles.size(); i++) {
      VesicleGenome<long double>& vgenome = *found_vesicles[i];
      ...
      long double distAllOUT, distAllIN, distNOUT, distNIN;
      ...
      std::vector<long double> fits = vgenome.fits();
...
bool getEdgeInfo(const Vesicle& ves, long double& distAllOUT, long double& distAllIN, long double& distNOUT, long double& distNIN) {
  ...
  avgAllOUT /= (long double) points_out.size();
  ...
  avgNeighborhoodIN /= (long double) closestToClearestIN.size();

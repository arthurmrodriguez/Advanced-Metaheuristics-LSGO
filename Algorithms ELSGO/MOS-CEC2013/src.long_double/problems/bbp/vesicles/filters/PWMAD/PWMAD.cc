...
int N = 3;
int K = 1;
const long double INVALID = -9999;

// Aux functions
long double median(const matrix<long double>&);
void createWindowMatrix(const matrix<long double>& origMatrix, matrix< matrix<long double> >& windowMatrix, int Wsize);
...
int main(int argc, char** argv) {
  ...
  matrix<long double> noisyImage (cols, rows);
  matrix<long double> medianImage(cols, rows);
  matrix<long double> devImage   (cols, rows);

  matrix< matrix<long double> > noiseWindows (cols, rows);
  matrix< matrix<long double> > medianWindows(cols, rows);
  matrix< matrix<long double> > devWindows   (cols, rows);
  ...
  std::vector< matrix<long double>* > dij(N);
  std::vector< matrix< matrix<long double> >* > dijW(N);

  // Compute dij0
  Magick::Image noise0(Magick::Geometry(cols, rows), Magick::ColorGray(1));

  dij[0] = new matrix<long double>(cols, rows);
  matrix<long double>& dij0 = *(dij[0]);
  ...
  dijW[0] = new matrix< matrix<long double> >(cols, rows);
  matrix< matrix<long double> >& dijW0 = *(dijW[0]);

  createWindowMatrix(dij0, dijW0, K);

  // Conduct iterative process
  for (unsigned n = 1; n < N; n++) {
    dij[n]  = new matrix<long double>          (cols, rows);
    dijW[n] = new matrix< matrix<long double> >(cols, rows);

    matrix<long double>& dijn_old = *(dij[n-1]);
    matrix<long double>& dijn     = *(dij[n  ]);

    matrix< matrix<long double> >& dijWn_old = *(dijW[n-1]);
    matrix< matrix<long double> >& dijWn     = *(dijW[n  ]);
  ...
  matrix<long double>& dijFinal = *(dij[N-1]);
...
long double median(const matrix<long double>& m) {
...
void createWindowMatrix(const matrix<long double>& origMatrix, matrix< matrix<long double> >& windowMatrix, int Wsize) {
  ...
  for (int i = 0; i < cols; i++) {
    for (int j = 0; j < rows; j++) {
      matrix<long double> xij(2*Wsize+1, 2*Wsize+1);

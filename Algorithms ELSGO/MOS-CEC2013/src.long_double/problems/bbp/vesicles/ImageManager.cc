...
void ImageManager::drawSelectedVesicles(char* _path) const {
  ...
  if (len>0) {
    /* NOT USED
    long double minValue = (std::min_element(selected_vesicles.begin(), selected_vesicles.end(), vesiclesCmpLesser ))->getValue();
    long double maxValue = (std::max_element(selected_vesicles.begin(), selected_vesicles.end(), vesiclesCmpGreater))->getValue();
    */
...
long double ImageManager::objectiveFunction(int _x, int _y, int _radius, int _width, VesicleInfo* info) {
  NeighborhoodIteratorType::RadiusType rt;
  rt.Fill(_radius);
  NeighborhoodIteratorType* nit = new NeighborhoodIteratorType(rt, reader->GetOutput(), reader->GetOutput()->GetRequestedRegion());

  Objective obj(_x, _y, _radius, _width, nit, selected_pixels, dimX, dimY);
  long double res = obj.objectiveFunction(info);
  delete nit;

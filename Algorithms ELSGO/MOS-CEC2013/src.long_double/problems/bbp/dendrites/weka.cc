...
void LinearRegression::Print( void ){
	boost::unordered_map<string, long double>::iterator it;
...
long double LinearRegression::Predict( boost::unordered_map<string, long double>& data ){
	/*
	 * data: map de string -> long double, que representa la instancia a partir de la cual se quiere predecir la clase del modelo
	 *
	 * Devuelve la predicción realizada por el modelo a partir de la instancia "data".	 *
	 */

	long double acumul = 0.0, error = 0.0, res;

	for(boost::unordered_map<string, long double>::iterator it=data.begin() ; it != data.end(); it++ ){
	...
	int loops = 10;
	boost::normal_distribution<long double> paramND(0, error);
	boost::variate_generator<boost::mt19937&, boost::normal_distribution<long double> > nD(randgen, paramND);
...
NormalDistribution::NormalDistribution( long double mean, long double stdv, string attribute )
...
long double NormalDistribution::Predict( boost::unordered_map<string, long double>& data  ){

	long double p = boost::math::cdf( *_terminationD, data[_factorIndex] );
	long double rnd = rand_unif01();

...
void print_matrix (void) {
   ...
   for (int i = 0; i < problem.n; i++) {
      ...
      for (int j = 0; j < problem.n; j++) {
         ...
         double weight = problem.matrix [i][j];
...
bool parse_data (void) {
   ...
   while (!arch.eof ()) {
      ...
      if (token == "NAME:") {
         ...
         problem.matrix = (double**) calloc ((size_t) sizeof (double) * problem.n, (size_t) sizeof (double));

         for (int i = 0; i < problem.n; i++)
            problem.matrix [i] = (double*) calloc ((size_t) sizeof (double) * problem.n, (size_t) sizeof (double));
...
extern "C" double objectiveInt (GAGenome& g) {
   ...
   double totaldist = problem.matrix [0][gen.gene (0)];
...
extern "C" double objectiveReal (GAGenome& g) {
   ...
   double totaldist = problem.matrix [0][keys[0]];
...
extern "C" GAGenome* defineProblem () {
   ...
   GA1DArrayAlleleGenome<double>* genomeReal = new GA1DArrayAlleleGenome<double> (problem.n - 1, real_alleles, objectiveReal);

...
double scoreOfRoutes(GAGenome& g1, int route1, GAGenome& g2, int route2, ReqDistMatrix& reqdistmatrix) {
  assert(&g1);
  assert(&g2);
  ...
  for (int i=0; i<gen1.routeLength(route1); i++) {
    ...
    for (int j=0; j<gen2.routeLength(route2); j++) {
      ...
      double tmp_score = reqdistmatrix.getDistOfReqsPickupIds( gen1.gene(route1,i),gen2.gene(route2,j) );
...
double scoreOfRoutes(vector<GAGenome*>& routes, int route1pos, int route2pos, ReqDistMatrix& reqdistmatrix) {

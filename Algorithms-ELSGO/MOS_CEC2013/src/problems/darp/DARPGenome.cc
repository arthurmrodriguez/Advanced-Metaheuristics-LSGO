...
DARPGenome::DARPGenome(unsigned x, double evalWeightsUpdateValue, GAGenome::Evaluator f, void* u)
...
int DARPGenome::insertVertexInBestPos(int route, int vert_id, int start_pos, int end_pos, double scoretoimprove, long maxevals) {
...
int DARPGenome::insertVertexInBestPosFromStartPos(int route, int vert_id, int start_pos_requested,
                                                  double scoretoimprove, long maxevals) {
...
double DARPGenome::scoreOfInsertingVertices(int route, const list<int> vertices_ids) const {
  assert(route >= 0 && route < length() );
  ...
  double score = tmpgen.score(); // To force the computation of the score and the increase in the nevals
...
//TODO: Analyze this method for a possible loss in performance due to the multiple DARPGenome copies
double DARPGenome::scoreOfInsertingVertex(int route, int pos, int vert_id) const {
  assert(route >= 0 && route < length() );
...
double DARPGenome::scoreOfRemovingVertices(int route, const list<int>& vertices_ids) const {
  assert(route >= 0 && route < length() );
...
#define WRITEVECTOR(name,type)  \
  ...
  WRITEVECTOR(pickupDelayPerRoute_  ,double);
  WRITEVECTOR(deliveryDelayPerRoute_,double);
...
#define READVECTOR(name,type) \
  ...
  READVECTOR(pickupDelayPerRoute_  ,double);
  READVECTOR(deliveryDelayPerRoute_,double);
...
double DARPGenome::nonPenalizedScore() const {

...
inline bool tmpCompareScores(double s1, double s2) {
  return GAGenome::compareScores(s1,s2) == GAGenome::BETTER;
}

double ReqDistMatrix::bestScoreFromAllCombs(Request& req1, Request& req2) {
  ...
  double res = * min_element(scores.begin(),scores.end(),tmpCompareScores);
...
inline void computeCostValues(vector<Vertex*>& vertices, vector<long>& arrivalTimes, vector<long>& departureTimes,
                             ...
                             /*out*/ double& pickupDelay, /*out*/ double& deliveryDelay, /*out*/ long& cost) {
...
inline double computeWaitingTimePenalization(vector<long>& waitingTimes,long threshold, double penconstant) {
  double res=0;
...
double ReqDistMatrix::reqSequenceScore(Request& req1, Request& req2, int req1d, int req2p, int req2d) { // Positions of the inserted vertices.
  ...
  double nonpenscore = eval_.nonPenalizedScore(cost,pickupDelay,deliveryDelay);

  double resvalue = (useconstrpen_ == true)
...
// double (KMedoidsReqDistributor::*scorefunc)(Request& req1, Request& req2); attribute to select between the old and the new opt way

//inline double ReqDistMatrix::reqsBestScore(Request& req1, Request& req2) {
//  vector<double> scores;
//

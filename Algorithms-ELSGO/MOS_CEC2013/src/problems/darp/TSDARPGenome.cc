...
pair<RoutingGenome*,int> TSDARPGenome::bestFromNeighborhood() {
  ...
  double        bestScore    = GAGenome::worstPossibleScore(); // Needed for moving always in case the score has the same value
  int           usedEvals    = 0;
  ...
  for (int route=0; route<length(); route++) {
    for (int pos=0; pos<routeLength(route); pos++) {
      ...
      for (int newroute=0; newroute<length(); newroute++) {
        ...
        double neighborScore = neighbor->score();
...
double TSDARPGenome::divPenalization(int newroute, int vertid) {
  int routesNum = length();
  pair<int,int> key = routeVertIdKey(newroute,vertid);

  int freq_value = opFreq_[key];

  return lackOfDivPenalization_ * nonPenalizedScore() * sqrt( (double) requestsNum() * routesNum ) * freq_value;
}

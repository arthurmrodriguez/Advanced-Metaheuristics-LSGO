...
protected:
  ...
  vector<double> pickupDelayPerRoute_;
  vector<double> deliveryDelayPerRoute_;
  ...
  double evalWeightsUpdateValue_;    // delta  in Parragh's or Cordeau's paper
...
public:
  ...
  DARPGenome (unsigned x, double evalWeightsUpdateValue=0, GAGenome::Evaluator f = (GAGenome::Evaluator) 0, void* u = (void*) 0);
  ...
  double deliveryDelayPerRoute(int route) const;
  double deliveryDelayPerRoute(int route, double value);
  double deliveryDelay() const;

  double pickupDelayPerRoute(int route) const;
  double pickupDelayPerRoute(int route, double value);
  double pickupDelay() const;
  ...
  int       insertVertexInBestPos             (int route, int vert_id, int start_pos, int end_pos,
                                               double scoretoimprove=GAGenome::bestPossibleScore(), long maxevals= std::numeric_limits<int>::max() ) ;
  ...
  double scoreOfInsertingVertex  (int route, int pos, int vert_id) const;
  double scoreOfInsertingVertices(int route, const list<int> vertices_ids) const;

  double scoreWithoutIncRouteEvals() { double sc=score(); darpeval__->decrementEvalRouteCalls(); return sc;}
  ...
  double    scoreOfRemovingVertices(int route, const list<int>& vertices_ids) const;
  ...
  virtual double nonPenalizedScore() const;
  ...
  double loadVWeight() const { return darpeval__->loadVWeight(); }
  double TWVWeight()   const { return darpeval__->TWVWeight();   }
  double rideVWeight() const { return darpeval__->rideVWeight(); }

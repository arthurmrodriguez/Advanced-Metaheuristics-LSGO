...
TEST_F(scoreOfInsertingReqTest,BasicInsertions) {
  ...
  for (int k=0; k<insert_positions.size(); k++) {
    ...
    double right_addition = dummymatrix->getCost( newreqidins, insert_pos_id);

    new_score = orig_score + right_addition;

    if (insert_pos > 0) {
      int prev_pos_id       = gen->gene(route,insert_pos-1);
      double left_addition  = dummymatrix->getCost( prev_pos_id, newreqidins  );
      double removal        = dummymatrix->getCost( prev_pos_id, insert_pos_id );

...
struct DummyCostPoint {
  ...
  DummyCostPoint(int o, int d, double c) : orig(o), dest(d), cost(c) {}
...
double dummyRouteScore(const DARPGenome& gen, int route) {
...
extern "C" double DummyObjFunc(GAGenome& g) {
...
extern "C" double DummyObjFuncWithCriticalVertexConstraint(GAGenome& g) {
...
public:
  ...
  double nonPenalizedScore(const DARPGenome& gen, int route) const {
    return dummyRouteScore(gen,route);
  }

  double score(DARPGenome& gen) {
...
extern "C" double originalobjective(GAGenome& g) {
  DARPGenome& gen = dynamic_cast<DARPGenome&>(g);
...
struct DummyDistMatrix : public DistMatrix {
  ...
  void setCost(int orig, int dest, double cost) { 
  ...
  void setCostOneSide(int orig, int dest, double cost) {
    assert(orig >= 0 && orig < data_.size() );
    assert(dest >= 0 && dest < data_[orig].size() );
    data_[orig][dest] = cost;
  }
...
void addRequestRoute(VerticesList& list, int id,int origin, int dest, int wbegin, int wend, vector<DummyCostPoint>& costinf, double cost) {
...
void printRoute(DARPGenome& gen, int route, VerticesList& list, CostMatrix& costmatrix) {
  ...
  for (int i=0; i<gen.routeLength(route)-1; i++) {
    ...
    double cost     = costmatrix.getCost(req,next_req);
...
DARPGenome* createGen(int num_routes,int route, vector<int>& seqv) {
  ...
  for (int i=0; i<seqv.size(); i++) {
    ...
    for (int j=1; j<num_routes; j++) {
      ...
      gen->pushBackVertex( next_route , (int) pow((double)10,j) *seqv[i]);
    }
  }

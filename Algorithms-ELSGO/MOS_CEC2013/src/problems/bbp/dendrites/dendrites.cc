...
typedef struct MetricsT {
  std::vector<double> numBranches;
  std::vector<double> maxEucDistance;
...
boost::unordered_map<std::string, double> incRec_avg, incEuc_avg, incRec_dev, incEuc_dev;

double avgDev_mean_avg, avgDev_stddev_avg, avgDev_mean_dev, avgDev_stddev_dev;
...
void csvline_populate(std::vector<std::string> &record, const std::string& line, char delimiter) {
  ...
  while(line[linepos] != 0 && linepos < linemax) {
    ...
    if (!inquotes && curstring.length() == 0 && c == '"') { //beginquotechar
      ...
      if ((linepos+1 < linemax) && (line[linepos+1] == '"')) { //encountered 2 double quotes in a row (resolves to 1 double quote)
...
extern "C" double objective (GAGenome& g) {
  ...
  double pvalue_euc = 0, pvalue_path = 0, pvalue_avg = 0, pvalue_euc_res = 0, pvalue_path_res = 0, pvalue_avg_res = 0;
  ...
  for (unsigned i = 0; i < bsIters + 1; i++) {
    ...
    boost::unordered_map<std::string, double> perturbation;
    boost::unordered_map<std::string, double>::iterator it;
    unsigned pos = 0;
    ...
    double pvalueBranches = 0.0;
    double pvalueEuc      = 0.0;
    double pvalueLength   = 0.0;
    ...
    for (unsigned k = 0; k < valIters; k++) {
      ...
      std::vector<double> newNumBranches(nMetrics);
      std::vector<double> newMaxEucDist (nMetrics);
      std::vector<double> newTotalLength(nMetrics);

      for (unsigned j = 0; j < nMetrics; j++) {
        boost::unordered_map<std::string, double> metrics;
...
extern "C" GAGenome* defineProblem () {
  ...
  GAAlleleSet<double> alleles (-1, 1);
  ...
  GA1DArrayAlleleGenome<double>* genome = new GA1DArrayAlleleGenome<double> (sz, alleles, objective);

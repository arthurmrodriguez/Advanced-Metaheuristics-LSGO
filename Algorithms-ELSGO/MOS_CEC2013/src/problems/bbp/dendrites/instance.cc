...
Instance::Instance( boost::unordered_map<string, double>& data/*, ??? models*/, Dendrite* d )
	:_data(data),
	 _dendrite(d)
{
...
void Instance::createNexts( void ){
	 ...
	 * de termina)
	 */
	boost::unordered_map<string, double> dt;
	Instance* inst;
	double incRec, incEuc;
	double termina = _dendrite->predict( _data, str_termina );
	unsigned count = 0;
...
void Instance::storeInstance(){
	...
	//for( boost::unordered_map<string, double>::const_iterator it = _data.begin(); it != _data.end(); ++it )
	//	msg << (*it).second << ", ";

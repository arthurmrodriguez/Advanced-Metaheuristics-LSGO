...
vector<LogStat*>* createLogStats (const GAEDAConfig& cfg, const Algorithm& alg, CommManager& comm, double startruntime) {
...
static void runAndDisplayResults (Algorithm& alg, GALogger& logger, CommManager& comm_manager, double startruntime, GAEDAConfig& cfg) {
   alg.run();
   double runtime = comm_manager.getTime () - startruntime;
   ...
   if (comm_manager.isIslandMaster ()) {
      ...
      std::cout << "-> EVALS:   " << cfg.getEvals()  << std::endl;
      if (cfg.getPrintBestSol() ) {
        std::cout << "BEST:  " <<endl << *best   << std::endl;
...
int main (int argc, char **argv) {
  ...
  double startruntime = comm_manager.getTime ();
  ...
  try {
    ...
    int size = ((cfg->getProblemSize()%2) ==0) ? cfg->getProblemSize(): cfg->getProblemSize()-1;
    RLImprovPerDimManager impmanager (size);

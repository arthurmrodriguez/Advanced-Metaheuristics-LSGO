...
public:
  ...
  struct PairComp {
    bool operator() (const std::pair<unsigned, double>& p1,
		     const std::pair<unsigned, double>& p2) const {
  ...
  // Constructores
  MOSEARL(const GAGenome&         genome, const double elitismPercent);
  MOSEARL(const GAPopulation& population, const double elitismPercent);
  MOSEARL(const GAGeneticAlgorithm&  alg, const double elitismPercent);
 ...
 protected:
  ...
  double scaleFitness(double fit) const;
 ...
 protected:
  ...
  double           _elitismPercent;   // Percent of the old Pop brought
                                      // into the new one
  ...
  std::vector<double> _avgFit;        // Fitness of individuals generated by each technique (per generation)
  ...
  std::vector< std::vector<double> > _QLearner; // Matrix with the QL
                                                // information

  std::vector< std::vector<double> > _actionProb; // Matrix with the probability
                                                  // for selecting a technique

  std::vector< std::vector<double> > _averagePolicyEstimate;

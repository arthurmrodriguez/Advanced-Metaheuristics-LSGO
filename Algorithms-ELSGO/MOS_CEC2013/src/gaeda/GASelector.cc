...
static void GAQuickSort(unsigned int *, double *, int, int);
#endif
...
GARouletteWheelSelector::update() {
  ...
  if (which == GASelectionScheme::RAW) {
    ...
    if (pop->max() == pop->min()) {
      for (i=0; i<n; i++) psum[i] = (double)(i+1)/(double)n; // equal likelihoods
    }
    ...
    if (pop->fitmax() == pop->fitmin()) {
      for (i=0; i<n; i++)
        psum[i] = (double)(i+1)/(double)n; // equal likelihoods
    }
...
void GATournamentSelector::copy(const GASelectionScheme& orig) {
  GASelectionScheme::copy(orig);

  const GATournamentSelector& ori = DYN_CAST(const GATournamentSelector&,orig);

  size_ = ori.size_;
  selector_->copy(* ori.selector_);
}
...
static void
GAQuickSort(unsigned int *c, double *s, int l, int r) {
  int i,j;
  double v;
  unsigned int tc;
  double ts;

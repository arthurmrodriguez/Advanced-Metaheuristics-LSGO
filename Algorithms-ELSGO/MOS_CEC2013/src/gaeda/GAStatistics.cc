...
GAStatistics::GAStatistics() {
  ...
  nconv=0;
  Nconv = 10;
  cscore = new double[Nconv]; memset(cscore, 0, Nconv*sizeof(double));

  nscrs=0;
  Nscrs = gaDefFlushFrequency;
  gen = new int[Nscrs]; memset(gen, 0, Nscrs*sizeof(int));
  aveScore = new double[Nscrs]; memset(aveScore, 0, Nscrs*sizeof(double));
  maxScore = new double[Nscrs]; memset(maxScore, 0, Nscrs*sizeof(double));
  minScore = new double[Nscrs]; memset(minScore, 0, Nscrs*sizeof(double));
  devScore = new double[Nscrs]; memset(devScore, 0, Nscrs*sizeof(double));
  divScore = new double[Nscrs]; memset(divScore, 0, Nscrs*sizeof(double));
...
GAStatistics::copy(const GAStatistics & orig){
  ...
  delete [] cscore;
  cscore = new double [Nconv]; memcpy(cscore, orig.cscore, Nconv*sizeof(double));
  ...
  delete [] aveScore;
  aveScore = new double [Nscrs];
  memcpy(aveScore, orig.aveScore, Nscrs*sizeof(double));
  delete [] maxScore;
  maxScore = new double [Nscrs];
  memcpy(maxScore, orig.maxScore, Nscrs*sizeof(double));
  delete [] minScore;
  minScore = new double [Nscrs];
  memcpy(minScore, orig.minScore, Nscrs*sizeof(double));
  delete [] devScore;
  devScore = new double [Nscrs];
  memcpy(devScore, orig.devScore, Nscrs*sizeof(double));
  delete [] divScore;
  divScore = new double [Nscrs];
  memcpy(divScore, orig.divScore, Nscrs*sizeof(double));
...
void GAStatistics::updateHallOfFame(const GAGenome& best_ind){
  if (use_hall_of_fame_) {
    ...
    double   best_ind_score = best_ind.score();
...
GAStatistics::reset(const GAPopulation & pop){
  ...
  memset(aveScore, 0, Nscrs*sizeof(double));
  memset(maxScore, 0, Nscrs*sizeof(double));
  memset(minScore, 0, Nscrs*sizeof(double));
  memset(devScore, 0, Nscrs*sizeof(double));
  memset(divScore, 0, Nscrs*sizeof(double));
  nscrs = 0;
  setScore(pop);
  if(Nscrs > 0) flushScores();

  memset(cscore, 0, Nconv*sizeof(double));
...
//  setConvergence(maxScore[0]);
  ...
  divCur = divInit = ((dodiv == gaTrue) ? pop.div() : (double)-1.0);
...
GAStatistics::setScore(const GAPopulation& pop){
  ...
  minCur = pop.min();
  devCur = pop.dev();
  divCur = ((dodiv == gaTrue) ? pop.div() : (double)-1.0);
...
void
GAStatistics::setConvergence(double s){
  nconv++;
...
GAStatistics::nConvergence(unsigned int n){
  ...
  if(Nconv < n){
    if(nconv < Nconv){
      memcpy(cscore, tmp, (nconv+1) * sizeof(double));
    }
    else{
      memcpy(&(cscore[Nconv-(nconv%Nconv)-1]), tmp,
	     ((nconv%Nconv)+1) * sizeof(double));
      memcpy(cscore, &(tmp[(nconv%Nconv)+1]),
	     (Nconv-(nconv%Nconv)-1) * sizeof(double));
    }
  }
  else{
    if(nconv < n){
	...
	memcpy(&(cscore[n-(nconv%Nconv)-1]), tmp,
	       ((nconv%Nconv)+1) * sizeof(double));
	memcpy(cscore, &(tmp[Nconv-(1+n-(nconv%Nconv))]), sizeof(double));
      }
      else{
	memcpy(cscore, &(tmp[1+(nconv%Nconv)-n]), n * sizeof(double));
      }
    }
  }
...
GAStatistics::resizeScores(unsigned int n){
  ...
  if(n == 0){
    ...
    tmpf = aveScore;
    aveScore = new double [n];
    memcpy(aveScore, tmpf, (n < Nscrs ? n : Nscrs)*sizeof(double));
    delete [] tmpf;

    tmpf = maxScore;
    maxScore = new double [n];
    memcpy(maxScore, tmpf, (n < Nscrs ? n : Nscrs)*sizeof(double));
    delete [] tmpf;

    tmpf = minScore;
    minScore = new double [n];
    memcpy(minScore, tmpf, (n < Nscrs ? n : Nscrs)*sizeof(double));
    delete [] tmpf;

    tmpf = devScore;
    devScore = new double [n];
    memcpy(devScore, tmpf, (n < Nscrs ? n : Nscrs)*sizeof(double));
    delete [] tmpf;

    tmpf = divScore;
    divScore = new double [n];
    memcpy(divScore, tmpf, (n < Nscrs ? n : Nscrs)*sizeof(double));
    delete [] tmpf;
...
double GAStatistics::current(int w) const {
  double val = 0.0;
  switch(w){
...
void GAStatistics::updateFitnessIncrement(const GAPopulation&      old_pop,
  ...
  old_pop.sort();
  double    max_score          = old_pop.best().score();
  double    min_score          = old_pop.worst().score();
  double    best_parent_score, best_child_score;
  double    fit_inc_sum_tmp    = 0.0;
...
double GAStatistics::getScaledFitness(double max_score, double min_score, double genome_score) {
  double result;
...
double GAStatistics::fitInc() const { return fit_inc_;}

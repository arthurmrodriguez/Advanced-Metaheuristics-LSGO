...
GABoolean GAGeneticAlgorithm::TerminateUponPopConvergenceOrEvals (GAGeneticAlgorithm & ga) {
  GABoolean val;
  if (TerminateUponPopConvergenceDiscrete(ga) || TerminateUponFitnessEvaluations(ga)) val = gaTrue;
  else                                                                                val = gaFalse;
  return val;
}
...
GABoolean GAGeneticAlgorithm::TerminateUponPopConvergence (GAGeneticAlgorithm & ga) {
   ...
   double ratio = ga.statistics ().current (GAStatistics::Minimum) /
...
GABoolean GAGeneticAlgorithm::TerminateUponPopConvergenceDiscrete (GAGeneticAlgorithm & ga) {
   GABoolean val = gaFalse;
   ...
   for (unsigned i = 0; i < pop.size(); i++)
     if (pop.individual(i).score() == best)
       nBest++;

   if (((double) nBest / (double) pop.size()) >= ga.pConvergence())
     val = gaTrue;

   std::cout << "Converged " << (double) nBest / (double) pop.size() << "%, over threshold " << ga.pConvergence() << "%" << std::endl;
...
GABoolean GAGeneticAlgorithm::TerminateUponFitnessEvaluations(GAGeneticAlgorithm & ga) {
  if (GAEDAConfig::handle()->usePrecission())
    return (ga.statistics().indEvals() >= ga.nEvaluations() || ga.population().best().precissionReached()) ? gaTrue : gaFalse;
  else
...
// This is a pretty ugly little hack to make doubles/doubles work transparently.
...
inline void GAGeneticAlgorithm::evolve () {

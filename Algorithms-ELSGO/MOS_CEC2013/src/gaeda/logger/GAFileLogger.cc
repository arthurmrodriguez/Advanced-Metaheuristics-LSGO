...
GAFileLogger::GAFileLogger (char*    path,
                            ...
                            LogLevel log_level): GALogger(stats_display_freq,log_level), path_(path) {
...
void GAFileLogger::appendMedoidsStats(string title, const GAPopulation& pop, vector<GAGenome*>& medoids, GAGenome& my_medoid){
  if (level_ <= GALogger::debug ) {
    double min_dist_medoid_to_pop = my_medoid.compare( pop.individual(0) );
    double max_dist_medoid_to_pop = my_medoid.compare( pop.individual(0) );
    double avg_dist_medoid_to_pop = my_medoid.compare( pop.individual(0) );

    for (unsigned i=1; i<pop.size(); i++){
      if ( my_medoid == pop.individual(i) ) continue;
      double dist_medoid_to_ind = my_medoid.compare( pop.individual(i) );
    ...
    double min_dist_between_medoids = ( medoids[0] )->compare( *( medoids[1] ) );
    double max_dist_between_medoids = ( medoids[0] )->compare( *( medoids[1] ) );
    double avg_dist_between_medoids = 0.0;

    for (unsigned int i=0; i<medoids.size(); i++){
      for (unsigned int j=i+1; j<medoids.size(); j++){
        double dist_between_medoids = medoids[i]->compare( *( medoids[j] ) );
...
void GAFileLogger::appendStats (string title) {
...
void GAFileLogger::appendExecTime(double time){

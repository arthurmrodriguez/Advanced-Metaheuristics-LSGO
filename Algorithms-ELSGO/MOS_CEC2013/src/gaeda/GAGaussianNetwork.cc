...
void GAGaussianNetwork::initialize () {
   ...
   mMin = new double [getIndividualSize ()];
   mMax = new double [getIndividualSize ()];
   ...
   mA = new double* [getIndividualSize ()];

   for (i = 0; i < getIndividualSize (); i++) {

      mA [i] = new double [getIndividualSize ()];
...
void GAGaussianNetwork::learn (const GAPopulation& cases, double sel_pctg) {
   ...
   for (i = getIndividualSize () - 2; i >= 0; i--) {
      ...
      for (j = 0; j < getIndividualSize (); j++)
         ...
         if (!linked [j]) {
            ...
            double entropy_j = entropy (j, head);
...
double GAGaussianNetwork::entropy (int node) {
...
double GAGaussianNetwork::entropy (int node, int parent) {
...
double GAGaussianNetwork::BGe (int node) {
...
double GAGaussianNetwork::BGe (CSet& nodes) {
   ...
   double detWY_0 = WY_0.Determinant ();
   double detWY_m = WY_m.Determinant ();

   double log_gh = 0;

   log_gh += -(double) l * m / 2 * log (M_PI);
   log_gh +=  (double) l / 2 * log ((double) alpha_m / (alpha_m + m));

   log_gh += logC (l, alphaY_W + m);
   log_gh -= logC (l, alphaY_W);

   log_gh +=  (double) alphaY_W / 2 * log (detWY_0);
   log_gh += -(double) (alphaY_W + m) / 2 * log (detWY_m);
...
double GAGaussianNetwork::logC (int l, int alpha) {

   int i;
   double value = 0;

   for (i = 1; i <= l; i++)
      value += logGamma (double (alpha + 1 - i) / 2);
...
double GAGaussianNetwork::logGamma (double num) {
...
double GAGaussianNetwork::rejectionRegionBoundary () {
...
double GAGaussianNetwork::F (double x) {

   double b = -0.5 * (2 * getIndividualSize () + 1) * x;
...
double GAGaussianNetwork::f (double x) {
   ...
   double a = 0.25 * (x - 1) * (2 * getIndividualSize () + 1);
...
double GAGaussianNetwork::BIC (int node) {
   ...
   double gh_node_parents = BIC (node, Y);
...
double GAGaussianNetwork::BIC (int node, CSet& ParentNodes) {
   ...
   // Compute initial log
   double log2pi = log ((double) sqrt (2 * M_PI) * mV [node]);

   // Compute penalization
   double penal = 0.5 * log ((double) mSelectionSize) * (/*2*getIndividualSize()+*/ ParentNodes.size ());
...
void GAGaussianNetwork::EMNAAdapt (const GAPopulation& cases, const GAGenome& genes_new,
   ...
   int i, j;
   std::vector <double> Cov_tmp (getIndividualSize ());

...
MOSTechniqueLS::MOSTechniqueLS(techIdType id, std::string description, LocalSearch ls, GAGenome::Comparator comparator,
  ...
  GA1DArrayAlleleGenome<double>& g = dynamic_cast<GA1DArrayAlleleGenome<double>&>(*genomeBase);
  _SR = (double) (g.alleleset(0).upper() - g.alleleset(0).lower()) / 2.0;
  _currentGenome = NULL;
...
unsigned MOSTechniqueLS::offspring (unsigned maxEvals, unsigned& usedEvals, GAPopulation* pop, GAPopulation* auxPop, bool& converged) {
  ...
  do {
    double fit_inc_acum = 0.0;
    unsigned evals = 0;
    double grade = (*_ls)(*g.getGenome(_encoding), fitBest, evals, fit_inc_acum, _times_improved, _SR, maxEvals-usedEvals);
    g.score(g.getGenome(_encoding)->score());
    fit_inc_acum_total += fit_inc_acum;
    usedEvals+=evals;

...
GASteadyStateGA::GASteadyStateGA(const GAGenome& c) : GAGeneticAlgorithm(c) {
  ...
  double n = ((pRepl*(double)pop->size() < 1) ? 1 : pRepl*(double)pop->size());
...
// measure instead.
double
GASteadyStateGA::pReplacement(double value){
  ...
  double n = ((value*(double)pop->size() < 1) ? 1 : value*(double)pop->size());
  nRepl = (unsigned int)n;
  params.set(gaNnReplacement, (unsigned int)nRepl);
...
GASteadyStateGA::nReplacement(unsigned int value){
  ...
  pRepl = (double)nRepl / (double)pop->size();

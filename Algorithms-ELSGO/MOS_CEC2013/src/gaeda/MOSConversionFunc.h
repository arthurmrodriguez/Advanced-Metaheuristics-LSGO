...
extern "C" bool convertIntToReal (GAGenome* intGen, GAGenome* realGen) {
  ...
  GA1DArrayAlleleGenome<double>* _realGen = DYN_CAST(GA1DArrayAlleleGenome<double>*,realGen);

  unsigned len = _realGen->length();
  double   inc = (_realGen->alleleset().upper () - _realGen->alleleset().lower ()) / (double) len;

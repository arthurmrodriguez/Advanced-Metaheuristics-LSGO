...
class GAPopulation : public GAID {
	public:
		...
		typedef void (*Initializer)(GAPopulation &, double perc);
		typedef void (*Evaluator)(GAPopulation &);

		static void DefaultInitializer(GAPopulation &, double perc);
		static void DefaultEvaluator(GAPopulation &);

    static void PercInitializer(GAPopulation & p, double perc);
	...
	public:
		...
		void destroy(int w=WORST, SortBasis b=RAW) { delete remove(w,b); }
                ...
                double    SumOfDistanceToPop(unsigned int ind_pos) const;
                double*   genDistsMatrix    (                    ) {diversity(); return indDiv;}
                double    distAvgPoint      (                    ) const;
                double    entropy           (                    ) const;
                double    grefenstetteBias  (                    ) const;
                double    nativePrcnt       ( unsigned rank      ) const;


    double initPercentage(double _initPerc)       {return initPerc = _initPerc;}
    double initPercentage(                ) const {return initPerc;            }

#ifdef GALIB_USE_STREAMS
		virtual void read(STD_ISTREAM &){}
		virtual void write (STD_OSTREAM & os, SortBasis basis=RAW) const;
                double avgGenDiv() const ;
#endif

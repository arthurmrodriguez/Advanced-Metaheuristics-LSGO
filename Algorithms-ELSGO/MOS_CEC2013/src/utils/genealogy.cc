...
int main (int argc, char** argv) {
  ...
  rule<iterator_type, std::vector<double>(), space_type> stats_techs;
  ...
  std::vector< std::vector<double> > participations (lastgen + 1, std::vector<double> (nTechs, 0.0));
  std::vector< std::vector<double> > inc_fit        (lastgen + 1, std::vector<double> (nTechs, 0.0));
  std::vector<double> entropy  (lastgen + 1, 0.0);
  std::vector<double> endogamy (lastgen + 1, 0.0);
  ...
  do {
    ...
    for (unsigned i = 0; i < current_indiv.nparents_; i++) {
      ...
      endogamy[current_indiv.firstgen_] += (current_indiv.technique_ == genealogy[current_indiv.parents_ids_[i]]->technique_) ? (double) 1.0 / (double) current_indiv.nparents_ : 0.0;
  ...
  for (unsigned i = 1; i <= lastgen; i++) {
    ...
    for (unsigned j = 0; j < nTechs; j++) {
      double prop = (total > 0) ? (double) participations[i][j] / (double) total : 0.0;
...
//    entropy_f << "  => entropy = " << entropy << std::endl;
    ...
    endogamy_f << ((new_genomes[i] == 0) ? 0 : endogamy[i] / (double) new_genomes[i]) << std::endl;

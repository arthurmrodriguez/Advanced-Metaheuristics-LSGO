// $Header: /nfs/dsi/cvs/galib/ga/GASelector.h,v 1.1.1.1 1999/11/11 18:56:03 mbwall Exp $
/* ----------------------------------------------------------------------------
  selector.h
  mbwall 10aug94
...
public:
  ...
  GARouletteWheelSelector(int w=GASelectionScheme::SCALED) :  GASelectionScheme(w) { psum = (long double*)0; n = 0;                  }
  GARouletteWheelSelector(const GARouletteWheelSelector& orig)                     { psum = (long double*)0; n = 0; copy(orig);      }

  GARouletteWheelSelector& operator=(const GASelectionScheme& orig)                { if(&orig != this) copy(orig); return *this;}

  virtual ~GARouletteWheelSelector()                                               { delete [] psum;                            }

  virtual GASelectionScheme* clone() const                                         { return new GARouletteWheelSelector;        }

  virtual void copy(const GASelectionScheme& orig) {
    GASelectionScheme::copy(orig);

    const GARouletteWheelSelector& sel = DYN_CAST(const GARouletteWheelSelector&,orig);

    delete [] psum;

    n    = sel.n;
    psum = new long double[n];
    memcpy(psum, sel.psum, n * sizeof(long double));
  }
...
/**
 * This version of the tournament selector does n roulette wheel selections
 * then picks the better of the two.  We derive from the roulette wheel class so
 * that we can use its update and select method.
 */
#if USE_TOURNAMENT_SELECTOR == 1
class GATournamentSelector : public GASelectionScheme {
  unsigned           size_;
  GASelectionScheme* selector_;


public:
  GADefineIdentity("GATournamentSelector", GAID::TournamentSelection);

  GATournamentSelector(unsigned size,const GASelectionScheme& selector,int w=GASelectionScheme::SCALED);
  GATournamentSelector(const GATournamentSelector& orig);

  virtual ~GATournamentSelector();

  virtual void               copy     (const GATournamentSelector & orig);
  GATournamentSelector&      operator=(const GATournamentSelector& orig );
...
public:
  ...
  GASRSSelector(int w=GASelectionScheme::SCALED) : GASelectionScheme(w)
    { fraction = (long double*)0; choices = (unsigned int *)0; n = 0; }
  GASRSSelector(const GASRSSelector& orig)
    { fraction = (long double*)0; choices = (unsigned int *)0; n = 0; copy(orig); }
  ...
  virtual void copy(const GASelectionScheme& orig) {
    ...
    memcpy(fraction, sel.fraction, n * sizeof(long double));

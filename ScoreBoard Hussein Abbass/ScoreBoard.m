...
//Calculates score of the SO problem regarding the baseline value
if(MorS=='S')
    if(length(scalefile)>0)
       s = load(scalefile);
       solutions(:,2) = s(3) + (s(4)-s(3))*(solutions(:,2)-s(1))/(s(2)-s(1));
    end

    baseline = load(strcat('baseline/',problemtype,'S.txt'));
    sol = min(solutions(:,1)); //Valor de F1+F2 del conjunto de soluciones
    if(sol<baseline)
        Score = baseline - sol;
    else
